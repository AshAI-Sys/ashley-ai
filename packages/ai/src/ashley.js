...
class Ashley {
    ...
    async validateProduction(workspaceId, userId, productionData) {
        const validatedData = validation_1.ProductionDataSchema.parse(productionData);
        const context = {
            workspaceId,
            userId,
    ...
    async getOrCreateAnalysis(context) {
        const validatedContext = validation_1.ValidationContextSchema.parse(context);

...
async function deleteConflicts() {
  ...
  try {
    ...
    if (deletedUser.count > 0) {
      console.log(
        `✅ Deleted ${deletedUser.count} user(s) with email "kelvinmorfe17@gmail.com"`
      );
    } else {
      console.log(
        'ℹ️  No user found with email "kelvinmorfe17@gmail.com" (already clean)'
      );
    }
    ...
    if (deletedWorkspace.count > 0) {
      console.log(
        `✅ Deleted ${deletedWorkspace.count} workspace(s) with slug "reefer"`
      );
    } else {
    ...
    console.log("      - Email: kelvinmorfe17@gmail.com");
    console.log(
      "      - Password: (make sure it has 8+ chars, uppercase, lowercase, number)"
    );

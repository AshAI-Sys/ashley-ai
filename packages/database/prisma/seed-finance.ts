...
async function main() {
  ...
  for (const order of orders) {
    ...
    let invoiceStatus: 'DRAFT' | 'SENT' | 'PAID' | 'PARTIAL' | 'OVERDUE' | 'CANCELLED' = 'DRAFT'
    let paidAmount = 0

    if (order.status === 'COMPLETED' || order.status === 'DELIVERED') {
      invoiceStatus = Math.random() > 0.5 ? 'PAID' : 'PARTIAL'
      paidAmount = invoiceStatus === 'PAID' ? order.total_amount : order.total_amount * 0.5
    } else if (order.status === 'IN_PRODUCTION' || order.status === 'QC_PASSED') {
      invoiceStatus = 'SENT'
      ...
      for (const lineItem of order.line_items) {
        await prisma.invoiceItem.create({
          data: {
            ...
            description: lineItem.description || `${lineItem.garment_type} - ${lineItem.printing_method}`,
    ...
    if (invoiceStatus === 'PAID' || invoiceStatus === 'PARTIAL') {
      ...
      paymentDate.setDate(paymentDate.getDate() + (invoiceStatus === 'PAID' ? 15 : 10))
      ...
      const payment = await prisma.payment.create({
        data: {
          ...
          payment_method: ['BANK_TRANSFER', 'CHECK', 'CASH', 'CARD'][Math.floor(Math.random() * 4)],
          status: 'completed',
  ...
  const expenseTypes = [
    { description: 'Office Rent - January 2025', amount: 25000, category: 'RENT' },
    { description: 'Electricity Bill', amount: 8500, category: 'UTILITIES' },
    { description: 'Internet & Phone Services', amount: 3200, category: 'UTILITIES' },
    { description: 'Fabric Purchase - Cotton', amount: 125000, category: 'MATERIALS' },
    { description: 'Printing Supplies', amount: 18000, category: 'OFFICE' },
    { description: 'Employee Salaries - Production', amount: 95000, category: 'LABOR' },
    { description: 'Delivery Service - J&T', amount: 12500, category: 'MARKETING' },
    { description: 'Equipment Maintenance', amount: 7800, category: 'UTILITIES' },
  ]
  ...
  for (const expenseData of expenseTypes) {
    ...
    await prisma.expense.create({
      data: {
        ...
        payment_method: ['BANK_TRANSFER', 'CHECK', 'CASH'][Math.floor(Math.random() * 3)],
        approved: approved,
        currency: 'PHP',

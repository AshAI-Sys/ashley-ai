...
const orderIntakeFormSchema = zod_2.z.object({
    ...
    sizeCurve: zod_2.z
        .record(zod_2.z.number().nonnegative())
        .refine(curve => Object.values(curve).reduce((sum, qty) => sum + qty, 0) > 0, "Size curve must have at least one size with quantity > 0"),
    ...
    specifications: zod_2.z
        .object({
        printingMethod: zod_2.z
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-6">
                  ...
                  <form_1.FormField control={form.control} name="priority" render={({ field }) => (<form_1.FormItem>
                        ...
                        <select_1.Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <select_1.SelectContent>
                            ...
                            <select_1.SelectItem value="NORMAL">
                              Normal Priority
                            </select_1.SelectItem>
                ...
                <form_1.FormField control={form.control} name="description" render={({ field }) => (<form_1.FormItem>
                      ...
                      <form_1.FormControl>
                        <textarea {...field} className="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring min-h-[80px] w-full rounded-md border px-3 py-2 text-sm focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50" placeholder="Detailed description of the garment requirements..."/>
                      </form_1.FormControl>
                      <form_1.FormMessage />
                    </form_1.FormItem>)}/>
              ...
              <card_1.CardContent className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <badge_1.Badge variant="outline" className="px-3 py-1 text-lg">
                    {Object.values(watchedSizeCurve || {}).reduce((sum, qty) => sum + qty, 0)}{" "}
                    pieces
                  </badge_1.Badge>
                </div>

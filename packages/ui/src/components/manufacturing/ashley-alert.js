...
const AshleyAlert = React.forwardRef(({ className, risk, issues = [], recommendations = [], confidence = 0, showDetails = true, ...props }, ref) => {
    ...
    return (<div ref={ref} className={(0, utils_1.cn)("rounded-lg border p-4", config.bgColor, className)} {...props}>
        <div className="flex items-start gap-3">
          <Icon className={(0, utils_1.cn)("mt-0.5 h-5 w-5", config.iconColor)}/>
          ...
          <div className="flex-1 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <badge_1.Badge variant={config.badgeVariant}>{risk}</badge_1.Badge>
                {confidence > 0 && (<span className="text-muted-foreground text-xs">
            ...
            {showDetails && (<>
                {issues.length > 0 && (<div className="space-y-2">
                    ...
                    <ul className="space-y-1">
                      {issues.map((issue, index) => (<li key={index} className={(0, utils_1.cn)("flex items-start gap-2 text-sm", config.textColor)}>
                ...
                {recommendations.length > 0 && (<div className="space-y-2">
                    ...
                    <ul className="space-y-1">
                      {recommendations.map((recommendation, index) => (<li key={index} className={(0, utils_1.cn)("flex items-start gap-2 text-sm", config.textColor)}>
                          <lucide_react_1.Info className="mt-0.5 h-3 w-3 flex-shrink-0"/>
                          {recommendation}
                        </li>))}
                    </ul>
                  </div>)}

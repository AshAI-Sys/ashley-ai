...
const getStockTrendIcon = (projected, current) => {
    if (projected > current * 1.1)
        return <lucide_react_1.ArrowUp className="h-4 w-4 text-ash-green-600"/>;
            ...
            return (<table_1.TableRow key={material.id} className={(0, utils_1.cn)("cursor-pointer hover:bg-gray-50", selectedMaterial === material.id && "bg-ash-blue-50")} onClick={() => onMaterialSelect?.(material.id)}>
                        ...
                        <table_1.TableCell>
                          {material.urgentOrders.length > 0 && (<div className="space-y-1">
                              ...
                              <div className="text-xs text-muted-foreground">
                                Next: {(0, date_fns_1.format)(material.urgentOrders[0].requiredDate, "MMM dd")}
                              </div>
                            </div>)}
                        </table_1.TableCell>
                ...
                <table_1.Table>
                  ...
                  <table_1.TableBody>
                    {projections.map((projection, index) => (<table_1.TableRow key={index} className={projection.shortfall > 0 ? "bg-red-50" : ""}>
                        ...
                        <table_1.TableCell>{projection.beginningStock.toLocaleString()}</table_1.TableCell>
                        ...
                        <table_1.TableCell>
                          <span className={(0, utils_1.cn)("font-medium", projection.endingStock < 0 ? "text-red-600" : "text-gray-900")}>
                            {projection.endingStock.toLocaleString()}
                          </span>
                        </table_1.TableCell>
                        <table_1.TableCell>
                          {projection.shortfall > 0 && (<span className="text-red-600 font-medium">

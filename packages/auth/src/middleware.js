...
async function handleApiRoute(request) {
  ...
  const routeConfig = exports.PROTECTED_API_ROUTES.find(config =>
  ...
  if (routeConfig.permissions) {
    const hasRequiredPermission = routeConfig.permissions.some(permission =>
      (0, utils_1.hasPermission)(userRole, permission)
    );
...
async function handleWebRoute(request) {
  ...
  // Find matching route configuration
  const routeConfig = exports.PROTECTED_ROUTES.find(config =>
    pathname.startsWith(config.path)
  );
  if (!routeConfig) {
    return server_1.NextResponse.next();
  ...
  // Check role requirements
  if (routeConfig.roles && !routeConfig.roles.includes(userRole)) {
    return server_1.NextResponse.redirect(

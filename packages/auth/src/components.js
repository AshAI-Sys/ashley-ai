...
function AuthGuard({ children, fallback = null, loading = <div>Loading...</div>, requireAuth = true, }) {
...
function RoleGuard({ children, roles, requireAll = false, fallback = null, loading = <div>Loading...</div>, }) {
...
function PermissionGuard({ children, permissions, requireAll = false, fallback = null, loading = <div>Loading...</div>, }) {
    const { isLoading } = (0, hooks_1.useAuth)();
...
function AdminGuard({ children, fallback = null, loading, }) {
    return (<RoleGuard roles={types_1.UserRole.ADMIN} fallback={fallback} loading={loading}>
      {children}
    </RoleGuard>);
}
// Manager+ guard (Admin or Manager)
function ManagerGuard({ children, fallback = null, loading, }) {
    return (<RoleGuard roles={[types_1.UserRole.ADMIN, types_1.UserRole.MANAGER]} fallback={fallback} loading={loading}>
      {children}
    </RoleGuard>);
}
// User profile display component
function UserProfile({ showRole = true, showWorkspace = true, className = "", }) {
    const { user } = (0, hooks_1.useAuth)();
    if (!user)
        return null;
    return (<div className={`flex items-center space-x-3 ${className}`}>
      {user.avatar_url ? (<img src={user.avatar_url} alt={user.name} className="h-8 w-8 rounded-full"/>) : (<div className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-300">
      ...
      <div className="flex flex-col">
        ...
        {showRole && (<span className="text-xs text-gray-500">
            {user.role.replace("_", " ").toLowerCase()}
          </span>)}

        {showWorkspace && (<span className="text-xs text-gray-400">{user.workspace_name}</span>)}
      </div>
    </div>);
}
...
function PermissionIndicator({ permission, showIcon = true, }) {
    ...
    return (<span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${hasPermission
            ? "bg-green-100 text-green-800"
            : "bg-red-100 text-red-800"}`} title={`Permission: ${permission}`}>
...
function RoleBadge({ role, className = "", }) {
    ...
    return (<span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getRoleColor(displayRole)} ${className}`}>
...
function UnauthorizedMessage({ message = "You don't have permission to access this content.", showContactSupport = true, }) {
    return (<div className="py-8 text-center">
      ...
      {showContactSupport && (<p className="mt-2 text-xs text-gray-400">
...
function withAuth(Component, options) {
    const WrappedComponent = (props) => {
        ...
        return (<AuthGuard fallback={fallback}>
        <Component {...props}/>
      </AuthGuard>);
    };

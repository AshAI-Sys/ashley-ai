...
function UserProfile({ showRole = true, showWorkspace = true, className = "" }) {
    ...
    return (<div className={`flex items-center space-x-3 ${className}`}>
      {user.avatar_url ? (<img src={user.avatar_url} alt={user.name} className="w-8 h-8 rounded-full"/>) : (<div className="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center">
      ...
      <div className="flex flex-col">
        ...
        {showWorkspace && (<span className="text-xs text-gray-400">
            {user.workspace_name}
          </span>)}
      </div>
    </div>);
...
function PermissionIndicator({ permission, showIcon = true }) {
    ...
    return (<span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${hasPermission
...
function RoleBadge({ role, className = "" }) {
    ...
    return (<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getRoleColor(displayRole)} ${className}`}>

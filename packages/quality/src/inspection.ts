...
export interface InspectionRequest {
  ...
  batchNumber?: string;
  sampleSize: number;
  photos?: string[]; // photo URLs or base64 data
  notes?: string;
}

export class QualityInspectionSystem extends EventEmitter {
  ...
  async createInspection(request: InspectionRequest): Promise<QCInspection> {
    ...
    const qcInspection: QCInspection = {
      id: inspectionId,
  ...
  async completeInspection(
    ...
    for (const criteriaResult of results.criteriaResults) {
      await db.qCCriteriaResult.create({
        data: {
  ...
  async getInspectionsByOrder(orderId: string): Promise<QCInspection[]> {
  ...
  private async analyzeIndividualPhoto(photoUrl: string): Promise<{
    defectsDetected: PhotoAnalysisResult["defectsDetected"];
    qualityScore: number;
  }> {
    try {
  ...
  private async checkForQualityAlerts(inspection: QCInspection): Promise<void> {
  ...
  private async getInspectionById(inspectionId: string): Promise<QCInspection> {
  ...
  private mapDatabaseToInspection(dbInspection: any): QCInspection {
    return {
      id: dbInspection.id,
      ...
      defects: (dbInspection.defects || []).map((defect: any) => ({
        id: defect.id,

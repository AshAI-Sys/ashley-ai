...
export class ProductionWorkflowEngine extends EventEmitter {
  ...
  async createWorkflow(
    ...
    const estimatedEndDate = addHours(startDate, estimatedDuration);
  ...
  async assignWorker(stepId: string, workerId: string): Promise<void> {
    const step = await this.getStep(stepId);
    if (!step) {
      throw new Error("Step not found");
    }
  ...
  async detectBottlenecks(workflowId: string): Promise<BottleneckAnalysis[]> {
  ...
  private getStageDisplayName(stage: ProductionStage): string {
    const names = {
      INTAKE: "Order Intake",
  ...
  private async getWorkflow(workflowId: string): Promise<ProductionWorkflow | null> {
    ...
    return {
      ...
      status: schedule.status as WorkflowStatus,
  ...
  private async getStep(stepId: string): Promise<WorkflowStep | null> {
    // In a real implementation, you'd have a steps table
    // For now, this is a placeholder
    return null;
  }

  private async getNextStep(workflowId: string): Promise<WorkflowStep | null> {
    // In a real implementation, you'd query for the next pending step
    return null;
  }

  private async startStep(stepId: string): Promise<void> {
  ...
  private async getDelayedSteps(workflowId: string): Promise<WorkflowStep[]> {
  ...
  private numberToPriority(priority: number): Priority {

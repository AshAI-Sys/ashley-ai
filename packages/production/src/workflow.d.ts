...
import type { ProductionWorkflow, Priority, ProductionAlert, BottleneckAnalysis } from "./types";
export declare class ProductionWorkflowEngine extends EventEmitter {
    private workspaceId;
    private userId;
    constructor(workspaceId: string, userId: string);
    createWorkflow(orderId: string, priority?: Priority, metadata?: Record<string, unknown>): Promise<ProductionWorkflow>;
    startWorkflow(workflowId: string): Promise<void>;
    completeStep(stepId: string, qualityScore?: number, notes?: string): Promise<void>;
    ...
    createAlert(workflowId: string, type: ProductionAlert["type"], severity: ProductionAlert["severity"], title: string, message: string, data?: Record<string, unknown>): Promise<ProductionAlert>;

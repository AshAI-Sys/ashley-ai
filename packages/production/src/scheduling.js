...
class ProductionScheduler {
  ...
  async assignWorkerToTask(request) {
    ...
    const isAvailable = await this.checkWorkerAvailability(
      ...
      (0, date_fns_1.addDays)(request.preferredStartTime, 1) // Simple 1-day assignment
    );
    if (!isAvailable) {
      return {
        success: false,
    ...
    const scheduledEnd = (0, date_fns_1.addDays)(
      request.preferredStartTime,
  ...
  async optimizeProductionSchedule(
    scheduleIds,
    optimizationGoals = { minimizeTime: true }
  ) {
  ...
  async generateProductionMetrics(date, productionLineId, workerId) {
    ...
    const totalPlannedHours = schedules.reduce(
      (sum, s) =>
        (0, date_fns_1.differenceInHours)(s.planned_end, s.planned_start),
      0
    );

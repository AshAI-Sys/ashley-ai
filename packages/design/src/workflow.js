...
class DesignApprovalWorkflow extends eventemitter3_1.EventEmitter {
    ...
    async processApproval(approvalId, status, feedback, approverId) {
        try {
            ...
            const stageApprovals = approval.workflow.approvals.filter(a => a.stageNumber === approval.stageNumber);
            const allApproved = stageApprovals.every(a => a.status === "APPROVED" || !a.isRequired);

...
export class PredictiveAnalyticsService {
  ...
  async validateCapacityVsDeadline(workspaceId: string, orderId: string): Promise<PredictionResult> {
    try {
      ...
      const historicalData = await prisma.routingStep.findMany({
        where: {
      ...
      const currentCapacity = await prisma.employee.findMany({
        where: {
          workspace_id: workspaceId,
          is_active: true,
...
`
  ...
  async predictQualityIssues(workspaceId: string, bundleId: string): Promise<PredictionResult> {
    try {
      ...
      const qualityHistory = await prisma.qcCheck.findMany({
        where: {
          workspace_id: workspaceId,
          bundle: {
            order: {
...
`
  ...
  async analyzeEmployeeFatigue(workspaceId: string): Promise<PredictionResult> {
    try {
...
`
  ...
  async predictMaintenance(workspaceId: string): Promise<PredictionResult> {
    try {
...
`
  ...
  async generateInsightsDashboard(workspaceId: string): Promise<any> {
    try {
      const [
        capacityInsights,
        qualityPredictions,
  ...
  private async getCapacityInsights(workspaceId: string): Promise<any> {
  ...
  private async getQualityInsights(workspaceId: string): Promise<any> {
    const recentQC = await prisma.qcCheck.findMany({
      where: {
        workspace_id: workspaceId,
  ...
  private extractHighPriorityAlerts(insights: any[]): string[] {
    const alerts: string[] = []
  ...
  private generateActionItems(insights: any[]): string[] {

...
exports.predictionsRouter = router;
// Get demand forecast
router.get('/demand', [
    (0, express_validator_1.query)('workspace_id').isUUID(),
    (0, express_validator_1.query)('time_range').optional().isIn(['week', 'month', 'quarter']),
    (0, express_validator_1.query)('product_category').optional().isString()
], async (req, res) => {
    try {
...
router.post('/timeline', [
    ...
    (0, express_validator_1.body)('order_ids').isArray(),
    (0, express_validator_1.body)('order_ids.*').isUUID()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const predictions = order_ids.map((orderId, index) => ({
            order_id: orderId,
        ...
        logger_1.logger.error('Timeline prediction error:', error);
        res.status(500).json({
            error: 'Internal Server Error',
...
router.post('/materials', [
    (0, express_validator_1.body)('workspace_id').isUUID(),
    (0, express_validator_1.body)('time_horizon').isInt({ min: 1, max: 365 })
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        logger_1.logger.error('Material prediction error:', error);
        res.status(500).json({
            error: 'Internal Server Error',

...
router.get('/reprints', [
  ...
  query('time_range').optional().isIn(['month', 'quarter', 'year']),
  ...
  try {
    ...
    for (const [key, frequency] of itemFrequency.entries()) {
      if (frequency < 100) continue // Minimum threshold
      ...
      if (confidence >= parseFloat(min_confidence as string)) {
    ...
    logger.error('Reprint recommendations error:', error)
    res.status(500).json({
...
router.post('/themes', [
  ...
  body('style_preference').optional().isString()
], async (req, res) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
    ...
    const suggestions = themeCategories[currentSeason as keyof typeof themeCategories].map((theme, index) => ({
      theme_name: theme,
      description: `A ${currentSeason} collection inspired by ${theme.toLowerCase()}`,
      color_palettes: colorPalettes[currentSeason as keyof typeof colorPalettes],
    ...
    res.json({
      ...
      recommendations: {
        ...
        trending_colors: colorPalettes[currentSeason as keyof typeof colorPalettes][0].colors,
    ...
    logger.error('Theme suggestions error:', error)
    res.status(500).json({
      error: 'Internal Server Error',
...
router.post('/optimize', [
  body('workspace_id').isUUID(),
  body('optimization_type').isIn(['workflow', 'capacity', 'quality', 'cost']),
  body('department').optional().isString(),
  body('time_range').optional().isIn(['week', 'month', 'quarter'])
], async (req, res) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
    ...
    const selectedOptimizations = optimizations[optimization_type as keyof typeof optimizations] || []

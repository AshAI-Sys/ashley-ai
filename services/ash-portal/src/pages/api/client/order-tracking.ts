...
export default async function handler(
  ...
  try {
    ...
    const totalSteps = bundles.reduce((sum: number, bundle: any) => sum + (bundle.sewing_runs?.length || 0), 0)
    const completedSteps = bundles.reduce((sum: number, bundle: any) => 
      sum + (bundle.sewing_runs?.filter((step: any) => step.status === 'completed')?.length || 0), 0
    )

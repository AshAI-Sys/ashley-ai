...
async function handler(req, res) {
    ...
    try {
        ...
        const order = await database_1.db.order.findFirst({
            where: {
        ...
        const completedSteps = bundles.reduce((sum, bundle) => sum +
            (bundle.sewing_runs?.filter((step) => step.status === "completed")
                ?.length || 0), 0);
        ...
        bundles.forEach((bundle, bundleIndex) => {
            ...
            sewingRuns.forEach((sewingRun) => {
                timeline.push({
                    ...
                    timestamp: new Date(sewingRun.completed_at ||
                        sewingRun.started_at ||
                        sewingRun.created_at).toISOString(),

...
function FeedbackForm({ designName, version, onSubmit, submitting = false, disabled = false, className = '' }) {
    ...
    return (<card_1.Card className={className}>
      ...
      <card_1.CardContent className="space-y-6">
        {/* Rating */}
        <div>
          <label_1.Label className="text-base font-medium mb-3 block">
            How do you feel about this design? (Optional)
          </label_1.Label>
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((star) => (<button key={star} type="button" onClick={() => setRating(rating === star ? undefined : star)} className={`p-2 rounded transition-colors ${rating && rating >= star
                ? 'text-yellow-500 hover:text-yellow-600'
                : 'text-gray-300 hover:text-yellow-400'}`} disabled={disabled}>
                <lucide_react_1.Star className={`w-6 h-6 ${rating && rating >= star ? 'fill-current' : ''}`}/>
              </button>))}
          </div>
          {rating && (<p className="text-sm text-muted-foreground mt-2">
        ...
        <div>
          <label_1.Label className="text-sm font-medium mb-2 block">Quick Feedback</label_1.Label>
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            {changeRequests.length > 0 && (<div className="space-y-2">
                {changeRequests.map((request, index) => (<div key={index} className="flex items-center justify-between p-2 bg-yellow-50 border border-yellow-200 rounded">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {PRIORITY_OPTIONS.map((option) => (<button key={option.value} type="button" onClick={() => setPriority(option.value)} disabled={disabled} className={`
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            {attachments.length > 0 && (<div className="space-y-2">
                {attachments.map((file, index) => (<div key={index} className="flex items-center justify-between p-2 bg-gray-50 border rounded">
                    ...
                    <button_1.Button type="button" size="sm" variant="ghost" onClick={() => removeAttachment(index)} disabled={disabled}>
                      <lucide_react_1.X className="w-4 h-4 text-red-500"/>
                    </button_1.Button>
                  </div>))}
              </div>)}
          </div>
        ...
        <div className="pt-4 border-t">
          ...
          {selectedPriority && priority !== 'normal' && (<div className={`mt-3 p-3 rounded-lg ${selectedPriority.color}`}>
              <div className="flex items-center gap-2">
                {priority === 'urgent' && <lucide_react_1.AlertTriangle className="w-4 h-4"/>}

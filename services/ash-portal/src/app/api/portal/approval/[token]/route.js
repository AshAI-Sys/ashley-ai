...
async function GET(request, { params }) {
    try {
        ...
        const tokenValidation = await tokenService_1.tokenService.validateApprovalToken(params.token);
        if (!tokenValidation.valid) {
            // Log security event
            await tokenService_1.tokenService.logSecurityEvent({
                type: 'invalid_access',
                ipAddress: clientIp,
                userAgent: userAgent,
                details: {
        ...
        if (!approval) {
            return server_1.NextResponse.json({ success: false, message: 'Invalid or expired approval link' }, { status: 404 });
        }

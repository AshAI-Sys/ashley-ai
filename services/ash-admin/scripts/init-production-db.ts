...
async function main() {
  try {
    ...
    if (existingWorkspace) {
      ...
      log(`   Existing workspace: ${existingWorkspace.name} (${existingWorkspace.slug})`, 'yellow')
    ...
    if (!adminEmail || !adminPassword || !workspaceName || !workspaceSlug) {
      ...
      if (!workspaceName) {
        workspaceName = await promptUser('Enter workspace name (e.g., "Acme Manufacturing"): ')
      ...
      if (!workspaceSlug) {
        const defaultSlug = workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')
        workspaceSlug = await promptUser(`Enter workspace slug (default: "${defaultSlug}"): `) || defaultSlug
        while (!validateSlug(workspaceSlug)) {
          log('   ✖ Slug must be lowercase alphanumeric with hyphens (3-50 chars)', 'red')
      ...
      if (!adminPassword) {
        adminPassword = await promptUser('Enter admin password (min 8 chars, uppercase, lowercase, number, special): ')

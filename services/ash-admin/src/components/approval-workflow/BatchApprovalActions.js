...
function BatchApprovalActions({ approvals, onRefresh, className = "", }) {
    ...
    const handleBatchAction = async () => {
        ...
        try {
            ...
            if (response.ok) {
                react_hot_toast_1.toast.success(`Successfully ${actionType === "send_reminder"
                    ? "sent reminders"
                    : actionType === "extend_expiry"
    ...
    if (actionableApprovals.length === 0) {
        ...
        <card_1.CardContent className="py-8 text-center">
          <lucide_react_1.Users className="mx-auto mb-4 h-12 w-12 text-muted-foreground"/>
    ...
    return (<card_1.Card className={className}>
      <card_1.CardHeader>
        <card_1.CardTitle className="flex items-center gap-2">
          <lucide_react_1.CheckSquare className="h-5 w-5"/>
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="max-h-60 overflow-y-auto rounded-lg border">
            ...
            return (<div key={approval.id} className={`flex items-center gap-3 border-b p-3 last:border-b-0 hover:bg-gray-50 ${isOverdue
                    ? "bg-red-50"
                    : isExpiringSoon
                        ? "bg-yellow-50"
                        : ""}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {isOverdue && (<lucide_react_1.AlertCircle className="h-4 w-4 text-red-500"/>)}
                      {isExpiringSoon && (<lucide_react_1.Clock className="h-4 w-4 text-yellow-500"/>)}
                    </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <select_1.Select value={actionType} onValueChange={value => setActionType(value)}>
              ...
              <select_1.SelectContent>
                <select_1.SelectItem value="send_reminder">
                  Send Reminder Email
                </select_1.SelectItem>
                <select_1.SelectItem value="extend_expiry">
                  Extend Expiry Date
                </select_1.SelectItem>
                <select_1.SelectItem value="cancel_approval">
          ...
          {/* Reminder Options */}
          {actionType === "send_reminder" && (<div className="space-y-4 rounded-lg border bg-blue-50 p-4">
              <div>
          ...
          {actionType === "extend_expiry" && (<div className="space-y-4 rounded-lg border bg-yellow-50 p-4">
              <div>
          ...
          {/* Cancel Confirmation */}
          {actionType === "cancel_approval" && (<div className="rounded-lg border bg-red-50 p-4">
        ...
        <button_1.Button onClick={handleBatchAction} disabled={submitting || selectedApprovals.length === 0 || !actionType} className="w-full" size="lg">
          {submitting ? (<>
              <lucide_react_1.Clock className="mr-2 h-4 w-4 animate-spin"/>
              Processing...
            </>) : (<>
              {actionType === "send_reminder" && (<lucide_react_1.Mail className="mr-2 h-4 w-4"/>)}
              {actionType === "extend_expiry" && (<lucide_react_1.Clock className="mr-2 h-4 w-4"/>)}
              {actionType === "cancel_approval" && (<lucide_react_1.AlertCircle className="mr-2 h-4 w-4"/>)}

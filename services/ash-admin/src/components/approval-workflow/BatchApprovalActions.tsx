...
export function BatchApprovalActions({ approvals, onRefresh, className = '' }: BatchApprovalActionsProps) {
  const [selectedApprovals, setSelectedApprovals] = useState<string[]>([])
  const [actionType, setActionType] = useState<'send_reminder' | 'extend_expiry' | 'cancel_approval' | ''>('')
  ...
  const handleBatchAction = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          `Successfully ${
            actionType === 'send_reminder' ? 'sent reminders' :
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Checkbox
                checked={selectedApprovals.length === actionableApprovals.length}
              ...
              return (
                <div 
                  ...
                  className={`flex items-center gap-3 p-3 border-b last:border-b-0 hover:bg-gray-50 ${
                    isOverdue ? 'bg-red-50' : isExpiringSoon ? 'bg-yellow-50' : ''
                  }`}
                >
                  <Checkbox
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isExpiringSoon && <Clock className="w-4 h-4 text-yellow-500" />}
                    </div>
          ...
          <div>
            ...
            <Select value={actionType} onValueChange={(value) => setActionType(value as any)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="extend_expiry">Extend Expiry Date</SelectItem>
                <SelectItem value="cancel_approval">Cancel Approval Request</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        >
          {submitting ? (
            ...
            <>
              {actionType === 'send_reminder' && <Mail className="w-4 h-4 mr-2" />}
              {actionType === 'extend_expiry' && <Clock className="w-4 h-4 mr-2" />}
              {actionType === 'cancel_approval' && <AlertCircle className="w-4 h-4 mr-2" />}

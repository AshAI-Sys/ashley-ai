...
function QualityControlInterface({ runId, bundleId, operatorId, onRejectSubmitted, className = "", }) {
    ...
    return (<div className={`space-y-6 ${className}`}>
      <card_1.Card className="border-2 border-red-200 bg-red-50">
        ...
        <card_1.CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            <label_1.Label className="font-medium text-red-900">Defect Type *</label_1.Label>
            ...
            {selectedDefect && (<div className="flex items-center gap-2">
                ...
                <badge_1.Badge variant="outline">{selectedDefect.category}</badge_1.Badge>
              </div>)}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <label_1.Label className="font-medium text-red-900">
                Quantity Rejected *
              </label_1.Label>
          ...
          <div className="space-y-2">
            ...
            <textarea_1.Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Detailed description of the defect, location, and severity..." className="min-h-20 border-red-300"/>
          </div>
          ...
          <div className="space-y-3">
            ...
            {photos.length > 0 && (<div className="grid grid-cols-3 gap-2">
                {photos.map((photo, index) => (<div key={index} className="relative">
                    <img src={URL.createObjectURL(photo)} alt={`Defect photo ${index + 1}`} className="h-24 w-full rounded border object-cover"/>
                    <button_1.Button size="sm" variant="destructive" onClick={() => removePhoto(index)} className="absolute right-1 top-1 h-6 w-6 p-0">
                      ×
                    </button_1.Button>
                  </div>))}
              </div>)}
          ...
          {/* Corrective Action */}
          <div className="space-y-2">
            <label_1.Label className="text-gray-700">
          ...
          <div className="flex gap-3 border-t pt-4">
            <button_1.Button onClick={handleSubmitReject} disabled={isSubmitting ||
            !selectedDefect ||
            !quantityRejected ||
            !description.trim()} className="bg-red-600 text-white hover:bg-red-700">
              {isSubmitting ? (<lucide_react_1.RefreshCw className="mr-2 h-4 w-4 animate-spin"/>) : (<lucide_react_1.Save className="mr-2 h-4 w-4"/>)}
      ...
      {/* Camera Modal */}
      {showCamera && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">

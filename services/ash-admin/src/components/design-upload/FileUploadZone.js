...
function FileUploadZone({ files, onFilesChange, fileType, title, description, accept, multiple = false, maxFiles = 10, maxSize = 50, className = '' }) {
    ...
    return (<div className={`space-y-4 ${className}`}>
      <div>
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-blue-400 transition-colors cursor-pointer" onDrop={handleDrop} onDragOver={handleDragOver} onClick={() => document.getElementById(`file-input-${fileType}`)?.click()}>
      ...
      {files.length > 0 && (<div className="space-y-2">
          {files.map((fileUpload) => (<div key={fileUpload.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <lucide_react_1.File className="w-4 h-4 flex-shrink-0"/>
                <div className="min-w-0 flex-1">
                  <p className="text-sm truncate">
                    {fileUpload.file?.name || 'Unknown file'}
                  </p>
                  {fileUpload.uploading && (<progress_1.Progress value={50} className="h-1 mt-1"/>)}
                  {fileUpload.error && (<p className="text-xs text-red-600 mt-1">{fileUpload.error}</p>)}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {fileUpload.error && (<div className="flex gap-1">
                    <button_1.Button variant="ghost" size="sm" onClick={() => retryUpload(fileUpload.id)} className="text-xs px-2 py-1 h-6">
                      Retry
                    </button_1.Button>
                  </div>)}
      ...
      {/* Upload Summary */}
      {files.length > 0 && (<div className="text-xs text-muted-foreground flex justify-between">
          <span>

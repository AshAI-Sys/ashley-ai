...
function Calendar({ mode = "single", selected, onSelect, disabled, initialFocus, className, ...props }) {
    ...
    const renderCalendarDays = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(<button key={day} onClick={() => selectDate(day)} disabled={dayIsDisabled} className={(0, utils_1.cn)("h-9 w-9 text-center text-sm p-0 font-normal rounded-md", "hover:bg-accent hover:text-accent-foreground", "focus:bg-accent focus:text-accent-foreground focus:outline-none", dayIsSelected && "bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground", dayIsToday && !dayIsSelected && "bg-accent text-accent-foreground", dayIsDisabled && "text-muted-foreground opacity-50 cursor-not-allowed")}>
          {day}
        </button>);
        }
        return days;
    ...
    return (<div className={(0, utils_1.cn)("p-3", className)} {...props}>
      ...
      <div className="grid grid-cols-7 gap-1">
        {dayNames.map(dayName => (<div key={dayName} className="h-9 w-9 text-center text-muted-foreground text-xs font-normal flex items-center justify-center">
            {dayName}
          </div>))}
        {renderCalendarDays()}
      </div>

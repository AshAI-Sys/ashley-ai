...
function SilkscreenWorkflow({ runId, onUpdate, readOnly = false }) {
    ...
    const [data, setData] = (0, react_1.useState)({
        ...
        ink_setup: {
            ink_type: 'PLASTISOL',
            colors: [
                { name: 'Black', pantone: '19-0303', location: 'Station 1', estimated_grams: 15 }
            ],
            coats: 1,
    ...
    const getStepStatus = (step) => {
        switch (step) {
            ...
            case 'ink_setup':
                return data.ink_setup.colors.every(c => c.name && c.pantone) ? 'complete' : 'pending';
            case 'printing':
    ...
    return (<div className="space-y-6">
        ...
        <card_1.CardHeader>
          <div className="flex items-center gap-2">
            <div className="p-2 bg-blue-100 rounded-full">
              <lucide_react_1.AlertCircle className="w-5 h-5 text-blue-600"/>
            </div>
            <div>
              <card_1.CardTitle className="text-lg">Ashley AI Optimization</card_1.CardTitle>
              <card_1.CardDescription>Smart recommendations for this silkscreen run</card_1.CardDescription>
            </div>
          </div>
        </card_1.CardHeader>
        <card_1.CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-3 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{data.ashley_recommendations.ink_calculation}g</div>
            ...
            <div className="text-center p-3 bg-orange-50 rounded-lg">
              <div className="text-2xl font-bold text-orange-600">{data.ashley_recommendations.registration_drift_prediction}mm</div>
      ...
      {activeStep === 'prep' && (<card_1.Card>
          <card_1.CardHeader>
            ...
            <card_1.CardDescription>Prepare screens with proper mesh and emulsion</card_1.CardDescription>
          </card_1.CardHeader>
          <card_1.CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input_1.Input value={data.screen_prep.emulsion_batch} onChange={(e) => updateData('screen_prep', { emulsion_batch: e.target.value })} placeholder="EMB-20250101-001" readOnly={readOnly}/>
              </div>
              <div className="space-y-2">
                <label_1.Label>Exposure Time (seconds)</label_1.Label>
                <input_1.Input type="number" value={data.screen_prep.exposure_seconds} onChange={(e) => updateData('screen_prep', { exposure_seconds: parseInt(e.target.value) })} readOnly={readOnly}/>
              </div>
            </div>

            <div className="space-y-2">
              <label_1.Label>Registration Notes</label_1.Label>
              <textarea_1.Textarea value={data.screen_prep.registration_notes} onChange={(e) => updateData('screen_prep', { registration_notes: e.target.value })} placeholder="Record registration marks, alignment notes..." readOnly={readOnly}/>
            </div>
      ...
      {activeStep === 'setup' && (<card_1.Card>
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center gap-2">
              <lucide_react_1.Droplet className="w-5 h-5"/>
              Ink Setup
            </card_1.CardTitle>
            <card_1.CardDescription>Configure inks, colors, and printing parameters</card_1.CardDescription>
          </card_1.CardHeader>
          <card_1.CardContent className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <select_1.Select value={data.ink_setup.ink_type} onValueChange={(value) => updateData('ink_setup', { ink_type: value })} disabled={readOnly}>
                  ...
                  <select_1.SelectContent>
                    ...
                    <select_1.SelectItem value="ANTI_MIGRATION">Anti-migration</select_1.SelectItem>
                  </select_1.SelectContent>
                </select_1.Select>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <select_1.Select value={data.ink_setup.squeegee_durometer.toString()} onValueChange={(value) => updateData('ink_setup', { squeegee_durometer: parseInt(value) })} disabled={readOnly}>
                  <select_1.SelectTrigger>
                    <select_1.SelectValue />
                  </select_1.SelectTrigger>
                  <select_1.SelectContent>
            ...
            <div className="space-y-4">
              ...
              {data.ink_setup.colors.map((color, index) => (<div key={index} className="grid grid-cols-4 gap-4 p-4 border rounded-lg">
                  <div className="space-y-2">
      ...
      {activeStep === 'print' && (<card_1.Card>
          <card_1.CardHeader>
            ...
            <card_1.CardDescription>Monitor printing quality and registration</card_1.CardDescription>
          </card_1.CardHeader>
          <card_1.CardContent className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                <label_1.Label>Test Prints</label_1.Label>
                <input_1.Input type="number" min="0" value={data.printing.test_prints} onChange={(e) => updateData('printing', { test_prints: parseInt(e.target.value) })} readOnly={readOnly}/>
              </div>
              <div className="space-y-2">
                <label_1.Label>Registration Check Interval</label_1.Label>
                <input_1.Input type="number" value={data.printing.registration_check_interval} onChange={(e) => updateData('printing', { registration_check_interval: parseInt(e.target.value) })} readOnly={readOnly}/>
                <p className="text-xs text-muted-foreground">Check every X pieces</p>
              </div>
              <div className="space-y-2">
      ...
      {activeStep === 'cure' && (<card_1.Card>
          <card_1.CardHeader>
            ...
            <card_1.CardDescription>Set proper curing temperature and time</card_1.CardDescription>
          </card_1.CardHeader>
          <card_1.CardContent className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                <label_1.Label>Dryer Temperature (°C)</label_1.Label>
                <input_1.Input type="number" min="120" max="200" value={data.curing.dryer_temp} onChange={(e) => updateData('curing', { dryer_temp: parseInt(e.target.value) })} readOnly={readOnly}/>
              ...
              <div className="space-y-2">
                ...
                <input_1.Input type="number" min="30" max="120" value={data.curing.cure_time} onChange={(e) => updateData('curing', { cure_time: parseInt(e.target.value) })} readOnly={readOnly}/>
              </div>
            </div>

            <div className="flex items-center space-x-2">
              <input type="checkbox" id="washTest" checked={data.curing.wash_test_completed} onChange={(e) => updateData('curing', { wash_test_completed: e.target.checked })} disabled={readOnly}/>

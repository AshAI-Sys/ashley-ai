    ...
    const addMaterial = () => {
        ...
        if (existingIndex >= 0) {
            ...
            updated[existingIndex] = {
                ...
                cost: (updated[existingIndex].quantity + quantity) *
      ...
      <card_1.Card className="border-l-4 border-l-orange-500">
        <card_1.CardHeader>
          <div className="flex items-center gap-2">
            <lucide_react_1.Package2 className="h-5 w-5 text-orange-600"/>
            <div>
      ...
      <card_1.Card>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {!readOnly && (<dialog_1.Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
                  ...
                  <dialog_1.DialogContent className="max-w-2xl">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <select_1.Select value={selectedType} onValueChange={setSelectedType}>
                            ...
                            <select_1.SelectContent>
                              ...
                              {materialTypes.map(type => (<select_1.SelectItem key={type} value={type}>
                                  {type}
                                </select_1.SelectItem>))}
                            </select_1.SelectContent>
                          </select_1.Select>
                      ...
                      <div className="space-y-2">
                        ...
                        <select_1.Select value={newConsumption.material_id} onValueChange={value => setNewConsumption({
                ...newConsumption,
                material_id: value,
            })}>
                          <select_1.SelectTrigger>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <input_1.Input type="number" step="0.01" min="0" value={newConsumption.quantity} onChange={e => setNewConsumption({
                ...newConsumption,
                quantity: e.target.value,
            })} placeholder="0.00"/>
                          {newConsumption.material_id && (<p className="text-xs text-muted-foreground">
                              Available:{" "}
                              {materials.find(m => m.id === newConsumption.material_id)?.available_stock}{" "}
                              {materials.find(m => m.id === newConsumption.material_id)?.unit}
                            </p>)}
                        </div>
                        <div className="space-y-2">
                          <label_1.Label>Notes (optional)</label_1.Label>
                          <input_1.Input value={newConsumption.notes} onChange={e => setNewConsumption({
                ...newConsumption,
                ...
                newConsumption.quantity && (<div className="rounded-lg bg-green-50 p-3">
                            <div className="flex items-center justify-between">
                              <span>Estimated Cost:</span>
                              <span className="font-bold text-green-600">
                                ₱
                                {(parseFloat(newConsumption.quantity || "0") *
                    (materials.find(m => m.id === newConsumption.material_id)?.cost_per_unit || 0)).toFixed(2)}
                              </span>
                            </div>
                          </div>)}
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        <button_1.Button onClick={addMaterial} disabled={!newConsumption.material_id ||
        ...
        <card_1.CardContent>
          {consumedMaterials.length > 0 ? (<div className="space-y-3">
              {consumedMaterials.map((material, index) => (<div key={material.material_id} className="flex items-center justify-between rounded-lg border p-4">
              ...
              <lucide_react_1.Package2 className="mx-auto mb-2 h-8 w-8 text-muted-foreground"/>
...
function MethodMaterialGuidelines({ method }) {
    ...
    const methodGuidelines = guidelines[method] || [

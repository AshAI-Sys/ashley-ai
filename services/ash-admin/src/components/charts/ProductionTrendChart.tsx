// @ts-nocheck - Temporarily disabled due to minified format causing parsing issues
"use client";
import {
  Card,
  CardContent,
...
export default function ProductionTrendChart({
  ...
  const [_chartType, _setChartType] = useState<"line" | "area">("area");
  ...
  const ChartComponent = _chartType === "area" ? AreaChart : LineChart;
  return (
    <Card className="">
      {" "}
      <CardHeader>
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            ...
            <CardTitle className="flex items-center gap-2">
              {" "}
              <TrendingUp className="h-5 w-5 text-blue-500" /> Production
              Trends{" "}
            </CardTitle>{" "}
            <CardDescription className="">
              {" "}
              Daily production output across all stages{" "}
            </CardDescription>{" "}
          </div>{" "}
          <div className="flex gap-2">
            ...
            <div className="flex overflow-hidden rounded-md border border-gray-300">
              ...
              {(["7d", "30d", "90d"] as const).map(range => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium transition-colors ${timeRange === range ? "bg-blue-600 text-white" : "bg-white text-gray-700 hover:bg-gray-50"}`}
                >
                  {" "}
                  {range.toUpperCase()}{" "}
                </button>
            ...
            <div className="flex overflow-hidden rounded-md border border-gray-300">
              {" "}
              <button
                onClick={() => _setChartType("line")}
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${_chartType === "line" ? "bg-blue-600 text-white" : "bg-white text-gray-700 hover:bg-gray-50"}`}
              >
                {" "}
                Line{" "}
              </button>{" "}
              <button
                onClick={() => _setChartType("area")}
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${_chartType === "area" ? "bg-blue-600 text-white" : "bg-white text-gray-700 hover:bg-gray-50"}`}
              >
                {" "}
                Area{" "}
              </button>{" "}
        ...
        <div className="mt-6 grid grid-cols-4 gap-4 border-t border-gray-200 pt-6">
          ...
          {[
            ...
            <div key={stat.label} className="text-center">
              ...
              <p className="text-xs text-gray-500"> units today </p>{" "}
            </div>
          ))}{" "}
        </div>{" "}
      </CardContent>{" "}

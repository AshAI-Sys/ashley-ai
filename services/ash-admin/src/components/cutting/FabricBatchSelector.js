...
function FabricBatchSelector({ availableBatches, onSelectionChange, orderRequirement, allowMultiSelect = true }) {
    ...
    const performAshleyAnalysis = async () => {
        ...
        const analysis = {
            ...
            fabricEfficiency: orderRequirement ?
                Math.min(100, (totalFabric / orderRequirement.estimatedFabricNeeded) * 100) : 0,
            recommendations: [],
            riskLevel: 'low',
        ...
        const gsmValues = selectedBatches.map(sel => sel.batch.gsm).filter(gsm => gsm !== undefined);
        ...
        const widthValues = selectedBatches.map(sel => sel.batch.width_cm).filter(w => w !== undefined);
    ...
    return (<div className="space-y-6">
      ...
      {orderRequirement && (<card_1.Card>
          ...
          <card_1.CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <span className="font-medium">Garment:</span><br />
                {orderRequirement.garmentType}
              </div>
              <div>
                <span className="font-medium">Target Pieces:</span><br />
                <span className="text-lg font-bold text-blue-600">
                  {orderRequirement.totalPieces}
                </span>
              </div>
              <div>
                <span className="font-medium">Est. Fabric Needed:</span><br />
                <span className="text-lg font-bold text-green-600">
                  {orderRequirement.estimatedFabricNeeded} {orderRequirement.unitType}
                </span>
              </div>
              <div>
                <span className="font-medium">Selected:</span><br />
                <span className="text-lg font-bold">
                  {selectedBatches.reduce((sum, sel) => sum + sel.qty_to_use, 0).toFixed(1)} {orderRequirement.unitType}
                </span>
              </div>
            </div>
          </card_1.CardContent>
      ...
      <card_1.Card>
        <card_1.CardHeader>
          <card_1.CardTitle>Available Fabric Batches ({filteredBatches.length})</card_1.CardTitle>
          <card_1.CardDescription>Select fabric batches for cutting operation</card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent className="space-y-4">
          {filteredBatches.map((batch) => {
            const selected = isSelected(batch.id);
            const selectedBatch = selectedBatches.find(sel => sel.batch.id === batch.id);
            return (<div key={batch.id} className={`border rounded-lg p-4 transition-all ${selected ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:bg-gray-50'}`}>
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-muted-foreground">
                        ...
                        <div>
                          <span className="font-medium">Est. Yield:</span><br />
                          ~{calculateEstimatedPieces(batch, 1)} pcs/{batch.uom}
                        </div>
                      </div>
                      
                      {selected && selectedBatch && (<div className="mt-4 p-3 bg-white rounded border">
                          <div className="flex items-center gap-4">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-sm text-muted-foreground">{batch.uom}</span>
                                <button_1.Button type="button" variant="outline" size="sm" onClick={() => updateQuantity(batch.id, selectedBatch.qty_to_use + 5)} disabled={selectedBatch.qty_to_use >= batch.qty_on_hand}>
                            ...
                            <div className="text-right">
                              <div className="text-sm text-muted-foreground">Est. Pieces:</div>
                  ...
                  <lucide_react_1.Package className={`w-5 h-5 ${selected ? 'text-blue-600' : 'text-gray-400'}`}/>
                </div>
              </div>);
        })}

          {filteredBatches.length === 0 && (<div className="text-center py-8 text-muted-foreground">
              <lucide_react_1.Package className="w-12 h-12 mx-auto mb-4 text-gray-300"/>
      ...
      {ashleyAnalysis && (<card_1.Card>
          ...
          <card_1.CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div className={`text-center p-3 rounded-lg ${getRiskColor(ashleyAnalysis.riskLevel)}`}>
            ...
            {ashleyAnalysis.recommendations.length > 0 && (<div className="space-y-2">
                ...
                {ashleyAnalysis.recommendations.map((rec, index) => (<div key={index} className="flex items-start gap-2 p-2 bg-gray-50 rounded">

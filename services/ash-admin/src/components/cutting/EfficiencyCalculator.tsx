...
export default function EfficiencyCalculator({ 
  ...
  const getEfficiencyColor = (value: number, thresholds: { good: number, warning: number }) => {
  ...
  const getAshleyScoreLevel = (score: number) => {
    if (score >= 90) return { level: 'Excellent', color: 'text-green-600 bg-green-50', icon: CheckCircle }
    if (score >= 80) return { level: 'Good', color: 'text-blue-600 bg-blue-50', icon: Target }
    if (score >= 70) return { level: 'Average', color: 'text-yellow-600 bg-yellow-50', icon: TrendingUp }
    return { level: 'Needs Improvement', color: 'text-red-600 bg-red-50', icon: AlertTriangle }
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge className={ashleyLevel.color.replace('bg-', 'bg-').replace('text-', 'text-')}>
                  {ashleyLevel.level}
                </Badge>
              </div>

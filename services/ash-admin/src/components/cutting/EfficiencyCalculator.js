...
function EfficiencyCalculator({ data, onResultsChange, }) {
    ...
    const performCalculations = () => {
        ...
        const expectedPiecesFromFabric = data.uom === "KG"
            ? Math.floor((grossUsed * 1000) /
                ((estimatedPatternArea / 10000) * fabricDensity)) // KG to pieces
            : Math.floor((grossUsed * 100 * (markerWidth || 160)) / estimatedPatternArea); // M to pieces
        const actualYield = (totalPiecesCut / Math.max(expectedPiecesFromFabric, totalPiecesCut)) *
            100;
        const pieceVariance = ((totalPiecesCut - expectedPiecesFromFabric) /
    ...
    if (loading) {
        ...
        <card_1.CardHeader>
          <card_1.CardTitle className="flex items-center gap-2">
            <lucide_react_1.Calculator className="h-5 w-5 animate-pulse"/>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="mb-2 flex items-center gap-2">
                ...
                <badge_1.Badge className={ashleyLevel.color
            .replace("bg-", "bg-")
            .replace("text-", "text-")}>
                  {ashleyLevel.level}
                </badge_1.Badge>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {/* Marker Efficiency */}
            <div className={`rounded-lg p-3 text-center ${getEfficiencyColor(results.markerEfficiency, { good: 85, warning: 75 })}`}>
            ...
            {/* Material Yield */}
            <div className={`rounded-lg p-3 text-center ${getEfficiencyColor(results.materialYield, { good: 92, warning: 88 })}`}>
              <div className="text-2xl font-bold">{results.materialYield}%</div>
              <div className="text-sm">Material Yield</div>
            </div>

            {/* Waste Percentage */}
            <div className={`rounded-lg p-3 text-center ${getEfficiencyColor(100 - results.wastePercentage, { good: 95, warning: 92 })}`}>
              <div className="text-2xl font-bold">
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="space-y-2">
            {results.recommendations.map((recommendation, index) => (<div key={index} className="flex items-start gap-2 rounded bg-gray-50 p-2">

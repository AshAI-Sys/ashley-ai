...
export default function FabricBatchSelector({
  ...
  const calculateEstimatedPieces = (batch: FabricBatch, quantity: number): number => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Available Fabric Batches ({filteredBatches.length})</CardTitle>
          <CardDescription>Select fabric batches for cutting operation</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {filteredBatches.map((batch) => {
            ...
            return (
              <div key={batch.id} className={`border rounded-lg p-4 transition-all ${
                ...
                <div className="flex items-start justify-between">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <Button
                                  ...
                                  variant="outline"
                                  size="sm"
                                  onClick={() => updateQuantity(batch.id, selectedBatch.qty_to_use - 5)}
                                ...
                                <Input
                                  ...
                                  onChange={(e) => updateQuantity(batch.id, parseFloat(e.target.value) || 0.1)}
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() => updateQuantity(batch.id, selectedBatch.qty_to_use + 5)}
                                  disabled={selectedBatch.qty_to_use >= batch.qty_on_hand}
                                >
                  ...
                  <Package className={`w-5 h-5 ${selected ? 'text-blue-600' : 'text-gray-400'}`} />
                </div>
              </div>
            )
          })}
          ...
          <CardContent>
            ...
            {ashleyAnalysis.recommendations.length > 0 && (
              <div className="space-y-2">
                ...
                {ashleyAnalysis.recommendations.map((rec: string, index: number) => (

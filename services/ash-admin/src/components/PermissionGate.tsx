...
export default function PermissionGate({
  ...
  if (department && user.department && user.department !== 'Administration') {
    if (user.department !== department) {
      ...
      const allowedDepts = crossDepartmentAccess[user.department as keyof typeof crossDepartmentAccess] || []
  ...
  if (allPermissions.length > 0) {
    ...
    const hasRequiredPermission = hasAnyPermission(userPermissions as Permission[], allPermissions)
...
export function usePermissions() {
  ...
  const canAccessDepartment = (department: string): boolean => {
    ...
    if (user.role === 'admin' || user.department === 'Administration') return true
    ...
    const allowedDepartments = crossDepartmentAccess[user.department as keyof typeof crossDepartmentAccess] || []

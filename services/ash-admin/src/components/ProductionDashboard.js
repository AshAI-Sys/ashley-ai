      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {dashboardData?.overview.active_orders.find(item => item.status === "in_progress")?._count.id || 0}{" "}
              in progress
            </p>
          </card_1.CardContent>
        </card_1.Card>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            ? Math.round((dashboardData.quality.pass_rate.passed /
      ...
      {dashboardData?.alerts.upcoming_deadlines.length > 0 && (<alert_1.Alert className="border-red-200 bg-red-50">
          ...
          <alert_1.AlertDescription>
            ...
            {dashboardData.alerts.upcoming_deadlines
                .slice(0, 3)
                .map((alert, index) => (<span key={alert.order_number}>
                  {index > 0 && ", "}
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-4">
                {floorStatus?.bundles.slice(0, 10).map(bundle => (<div key={bundle.id} className="flex items-center space-x-4 rounded-lg border p-3">
                    ...
                    <div className="text-right">
                      ...
                      <progress_1.Progress value={(bundle.progress.cutting +
                bundle.progress.printing +
                bundle.progress.sewing) /
                3} className="mt-1 w-24"/>
                    </div>
                  </div>))}
              </card_1.CardContent>
            </card_1.Card>
            ...
            return acc;
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {employees.slice(0, 5).map((emp, index) => (<div key={index} className="flex items-center justify-between rounded bg-gray-50 p-2">
                          <div>
                          ...
                          <div className="flex items-center space-x-2">
                            <badge_1.Badge variant={emp.status === "working"
                    ? "default"
                    : "secondary"}>
                              {emp.status}
                            </badge_1.Badge>
                            {emp.current_task && (<span className="text-xs text-blue-600">
                                {emp.current_task}
                              </span>)}
                          </div>
                        </div>))}
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                {dashboardData?.overview.active_orders.map(orderGroup => (<div key={orderGroup.status} className="rounded-lg border p-4 text-center">
                    <div className="text-2xl font-bold">
                    ...
                    <div className={`mt-2 h-2 w-full rounded ${getStatusColor(orderGroup.status)}`}/>
                  </div>))}
              </div>
            </card_1.CardContent>
          </card_1.Card>
          ...
          <card_1.Card>
            ...
            return acc;
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="w-20">
                          <progress_1.Progress value={stats.total > 0
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {dashboardData?.overview.machine_utilization.map(machine => (<div key={machine.asset_name} className="rounded-lg border p-4">
                    <div className="mb-2 flex items-center justify-between">
                      <span className="font-medium">{machine.asset_name}</span>
                      <badge_1.Badge variant={machine.status === "operational"
                ? "default"
                : "destructive"}>
                        {machine.status}
                      </badge_1.Badge>

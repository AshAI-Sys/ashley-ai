...
function ProductionRouteSection({ printingMethod, totalQuantity, deliveryDate, selectedRoute, onRouteChange, onRouteOptimized }) {
    ...
    const [routeVisualization, setRouteVisualization] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
    ...
    const getAvailableRoutes = () => {
        return PRODUCTION_ROUTES.filter(route => route.printingMethod === printingMethod || route.printingMethod === 'any');
    };
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="grid gap-3">
            {availableRoutes.map((route) => (<div key={route.id} className={`border-2 rounded-lg p-4 cursor-pointer transition-colors ${selectedRoute === route.id
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-1">
                    <lucide_react_1.Route className="w-4 h-4"/>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
          ...
          {ashleyOptimization && (<div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <badge_1.Badge variant="outline" className="text-purple-700 border-purple-300">
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-1">
                  {ashleyOptimization.reasoning.map((reason, i) => (<li key={i} className="text-sm text-purple-800 flex items-start gap-1">
                      <lucide_react_1.CheckCircle className="w-3 h-3 mt-0.5 flex-shrink-0 text-green-600"/>
                      {reason}
                    </li>))}
                </ul>
              </div>
              ...
              {ashleyOptimization.warnings.length > 0 && (<div className="space-y-2">
                  ...
                  <ul className="space-y-1">
                    {ashleyOptimization.warnings.map((warning, i) => (<li key={i} className="text-sm text-amber-800 flex items-start gap-1">
                        <lucide_react_1.AlertCircle className="w-3 h-3 mt-0.5 flex-shrink-0"/>
                        {warning}
                      </li>))}
                  </ul>
                </div>)}
        ...
        {routeVisualization && currentRoute && (<div>
            ...
            <div className="space-y-3">
              {currentRoute.steps.map((step, index) => (<div key={step.id} className="relative">
                  {index < currentRoute.steps.length - 1 && (<div className="absolute left-4 top-8 bottom-0 w-0.5 bg-gray-300"/>)}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 pb-4">
                      ...
                      {step.dependencies.length > 0 && (<div className="text-xs text-muted-foreground">
                          Depends on: {step.dependencies.map(dep => currentRoute.steps.find(s => s.id === dep)?.name).filter(Boolean).join(', ')}
                        </div>)}
                    </div>
                  </div>
                </div>))}

...
exports.ProductionRouteSection = ProductionRouteSection;
...
function ProductionRouteSection({ printingMethod, totalQuantity, deliveryDate, selectedRoute, onRouteChange, onRouteOptimized, }) {
    const [optimizing, setOptimizing] = (0, react_1.useState)(false);
    const [ashleyOptimization, setAshleyOptimization] = (0, react_1.useState)(null);
    const [customSteps, setCustomSteps] = (0, react_1.useState)([]);
    const [routeVisualization, setRouteVisualization] = (0, react_1.useState)(true);
    const [expandedStep, setExpandedStep] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
    ...
    const getAvailableRoutes = () => {
        return PRODUCTION_ROUTES.filter(route => route.printingMethod === printingMethod ||
    ...
    const optimizeWithAshley = async () => {
        if (!printingMethod || totalQuantity === 0) {
            react_hot_toast_1.toast.error("Please select printing method and quantity first");
            return;
        }
        setOptimizing(true);
        try {
            ...
            const optimization = {
                recommendedRoute: availableRoutes.find(r => r.isRecommended)?.id ||
            ...
            react_hot_toast_1.toast.success("Ashley AI route optimization completed");
        }
        catch (error) {
    ...
    const applyOptimizedRoute = () => {
        if (ashleyOptimization?.recommendedRoute) {
            ...
            react_hot_toast_1.toast.success("Applied optimized production route");
        }
    };
    const calculateTotalEstimatedHours = (route) => {
        return route.steps.reduce((total, step) => total + step.estimatedHours, 0);
    };
    const getRouteEfficiencyColor = (efficiency) => {
        if (efficiency >= 85)
    ...
    const addCustomStep = () => {
        ...
        react_hot_toast_1.toast.success("Custom step added");
    };
    const removeCustomStep = (stepId) => {
        setCustomSteps(customSteps.filter(s => s.id !== stepId));
        react_hot_toast_1.toast.success("Custom step removed");
    };
    const toggleStepCustomization = (stepId, event) => {
        if (event) {
    ...
    const updateCustomStep = (stepId, field, value) => {
        setCustomSteps(prev => prev.map(step => step.id === stepId ? { ...step, [field]: value } : step));
    };
    const availableRoutes = getAvailableRoutes();
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            ...
            <button_1.Button variant="outline" size="sm" onClick={() => setRouteVisualization(!routeVisualization)}>
              {routeVisualization ? "Hide" : "Show"} Steps
            </button_1.Button>
          </div>

          {!printingMethod ? (<div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-center">
              <lucide_react_1.AlertCircle className="mx-auto mb-2 h-6 w-6 text-amber-600"/>
            ...
            </div>) : availableRoutes.length === 0 ? (<div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-center">
              <lucide_react_1.AlertCircle className="mx-auto mb-2 h-6 w-6 text-gray-700"/>
              <p className="text-gray-600">
                No production routes available for {printingMethod}
              </p>
            </div>) : (<div className="grid gap-3">
              {availableRoutes.map(route => (<div key={route.id} className={`cursor-pointer rounded-lg border-2 p-4 transition-colors ${selectedRoute === route.id
                    ? "border-blue-500 bg-blue-50"
                    : "border-gray-200 hover:border-gray-300"}`} onClick={() => onRouteChange(route.id)}>
                  <div className="mb-2 flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-medium">{route.name}</h4>
                        {route.isRecommended && (<badge_1.Badge variant="default" className="bg-green-600">
                            Recommended
                          </badge_1.Badge>)}
                        {route.isCustom && (<badge_1.Badge variant="outline">Custom</badge_1.Badge>)}
                      </div>
        ...
        <div className="rounded-lg border border-purple-200 bg-purple-50 p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button_1.Button variant="outline" size="sm" onClick={optimizeWithAshley} disabled={optimizing || !printingMethod || totalQuantity === 0}>
              {optimizing ? (<>
                  <lucide_react_1.Sparkles className="mr-2 h-4 w-4 animate-pulse"/>
                  Optimizing...
                </>) : (<>
          ...
          {ashleyOptimization && (<div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="font-medium text-purple-800">
                  Optimization Results
                </span>
                <badge_1.Badge variant="outline" className="border-purple-300 text-purple-700">
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-1">
                  {ashleyOptimization.reasoning.map((reason, i) => (<li key={i} className="flex items-start gap-1 text-sm text-purple-800">
                      <lucide_react_1.CheckCircle className="mt-0.5 h-3 w-3 flex-shrink-0 text-green-600"/>
                      {reason}
                    </li>))}
                </ul>
              </div>

              {ashleyOptimization.warnings.length > 0 && (<div className="space-y-2">
                  <h5 className="font-medium text-amber-900">
                    Considerations:
                  </h5>
                  <ul className="space-y-1">
                    {ashleyOptimization.warnings.map((warning, i) => (<li key={i} className="flex items-start gap-1 text-sm text-amber-800">
              ...
              <button_1.Button onClick={applyOptimizedRoute} size="sm" className="w-full" disabled={selectedRoute === ashleyOptimization.recommendedRoute}>
        ...
        {routeVisualization && currentRoute && (<div>
            <div className="mb-4 flex items-center justify-between">
              <h4 className="flex items-center gap-2 font-medium">
                <lucide_react_1.MapPin className="h-4 w-4"/>
                Production Steps - {currentRoute.name}
              </h4>
              <button_1.Button variant="outline" size="sm" onClick={addCustomStep}>
            ...
            <div className="space-y-3">
              {currentRoute.steps.map((step, index) => (<div key={step.id} className="relative">
                  {index < currentRoute.steps.length - 1 && (<div className="absolute bottom-0 left-4 top-8 w-0.5 bg-gray-300"/>)}
                    ...
                    <div className="flex-1 pb-4">
                      <div className="mb-2 flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={e => toggleStepCustomization(step.id, e)} title="Customize step">
                      ...
                      <div className="mb-2 flex flex-wrap gap-1">
                        {step.resources.map((resource, i) => (<badge_1.Badge key={i} variant="outline" className="text-xs">
                            {resource}
                          </badge_1.Badge>))}
                      </div>

                      {step.dependencies.length > 0 && (<div className="text-xs text-muted-foreground">
                          Depends on:{" "}
                          {step.dependencies
                        .map(dep => currentRoute.steps.find(s => s.id === dep)?.name)
                        .filter(Boolean)
                        .join(", ")}
                        </div>)}

                      {expandedStep === step.id && (<div className="mt-3 space-y-2 rounded-lg border border-blue-200 bg-blue-50/50 p-3">
                          <div className="mb-2 flex items-center justify-between">
                            <h6 className="flex items-center gap-2 text-sm font-medium">
                              <lucide_react_1.Settings className="h-4 w-4"/>
                              Customize Step
                            </h6>
                            <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={e => toggleStepCustomization(step.id, e)}>
                          ...
                          <div>
                            ...
                            <input type="text" placeholder="Add special instructions for this step" className="mt-1 w-full rounded-md border px-3 py-2 text-sm" onChange={e => {
                        ...
                        step.customNotes = e.target.value;
                          ...
                          <div>
                            ...
                            <input type="number" placeholder={`Default: ${step.estimatedHours}h`} className="mt-1 w-full rounded-md border px-3 py-2 text-sm" onChange={e => {
                        step.customDuration =
              ...
              {customSteps.map((step, index) => {
                ...
                return (<div key={step.id} className="relative">
                    {index < customSteps.length - 1 && (<div className="absolute bottom-0 left-4 top-8 w-0.5 bg-green-300"/>)}
                        ...
                        <div className="rounded-lg border-2 border-green-200 bg-green-50/50 p-3">
                          <div className="mb-2 flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                <badge_1.Badge variant="default" className="bg-green-600 text-xs">
                                  Custom
                                </badge_1.Badge>
                                <input type="text" value={step.name} onChange={e => updateCustomStep(step.id, "name", e.target.value)} className="flex-1 border-b border-green-300 bg-transparent px-1 font-medium" placeholder="Step name"/>
                              </div>
                              <input type="text" value={step.department} onChange={e => updateCustomStep(step.id, "department", e.target.value)} className="mt-1 w-full border-b border-green-200 bg-transparent px-1 text-sm text-muted-foreground" placeholder="Department"/>
                            </div>
                            <div className="flex items-center gap-2">
                              <div className="text-right">
                                <input type="number" value={step.estimatedHours} onChange={e => updateCustomStep(step.id, "estimatedHours", parseFloat(e.target.value) || 1)} className="w-12 border-b border-green-300 bg-transparent text-right text-sm font-medium" min="0.5" step="0.5"/>
                                <span className="text-sm">h</span>
                              </div>
                              <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={e => toggleStepCustomization(step.id, e)} title="Edit details">
                                {isExpanded ? (<lucide_react_1.Minus className="h-4 w-4"/>) : (<lucide_react_1.Plus className="h-4 w-4"/>)}
                              </button_1.Button>
                              <button_1.Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-red-600" onClick={() => removeCustomStep(step.id)} title="Remove step">
                                <lucide_react_1.Minus className="h-4 w-4"/>
                              </button_1.Button>
                            </div>
                          </div>

                          {isExpanded && (<div className="mt-3 space-y-2 border-t border-green-200 pt-3">
                              <div>
                                <label className="text-xs font-medium">
                                  Resources Required
                                </label>
                                <input type="text" placeholder="e.g., Machine, Operator (comma separated)" className="mt-1 w-full rounded border px-2 py-1 text-sm" onChange={e => updateCustomStep(step.id, "resources", e.target.value
                            .split(",")
                            .map(r => r.trim()))}/>
                              </div>
                              ...
                              <div>
                                ...
                                <input type="text" placeholder="Special instructions" value={step.customNotes || ""} onChange={e => updateCustomStep(step.id, "customNotes", e.target.value)} className="mt-1 w-full rounded border px-2 py-1 text-sm"/>
                              </div>
                            </div>)}
                        </div>
                      </div>

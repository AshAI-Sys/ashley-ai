...
const ACCEPTED_FILE_TYPES = [
    ...
    '.gif', '.svg', '.ai', '.eps', '.psd', '.txt', '.zip', '.rar'
];
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${dragActive
            ...
            <lucide_react_1.Upload className="w-10 h-10 mx-auto mb-4 text-gray-400"/>
            <p className="text-lg font-medium mb-2">Upload Additional Files</p>
            ...
            <p className="text-xs text-muted-foreground mt-3">
              Supported: {ACCEPTED_FILE_TYPES.slice(0, 8).join(', ')}, and more (Max 100MB each)
            </p>
          </div>
            ...
            {Object.entries(getFilesByCategory()).map(([category, files]) => {
                ...
                return (<div key={category} className="mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h5 className="font-medium">{categoryData?.label || 'Other'}</h5>
                  ...
                  <div className="space-y-3">
                    {files.map((file) => (<div key={file.id} className="border rounded-lg p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex items-start gap-3 flex-1">
                            {file.preview ? (<img src={file.preview} alt={file.name} className="w-16 h-16 object-cover rounded border"/>) : (<div className="w-16 h-16 bg-gray-100 rounded border flex items-center justify-center">
                                <lucide_react_1.FileText className="w-6 h-6 text-gray-400"/>
                              </div>)}
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {!file.uploaded && file.uploadProgress !== undefined && (<div className="mt-2">
                              ...
                              {file.uploaded && analyzingFiles.includes(file.id) && (<div className="flex items-center gap-2 mt-2 text-sm text-purple-600">
                                  <lucide_react_1.Sparkles className="w-4 h-4 animate-pulse"/>
                                  Ashley AI is analyzing...
                                </div>)}
                              
                              {file.ashleyAnalysis && (<div className="mt-3 space-y-2">
                                  <div className="flex items-center gap-2">
                                    {file.ashleyAnalysis.feasible ? (<lucide_react_1.CheckCircle className="w-4 h-4 text-green-500"/>) : (<lucide_react_1.AlertCircle className="w-4 h-4 text-red-500"/>)}
                                    <span className={`text-sm font-medium ${file.ashleyAnalysis.feasible ? 'text-green-700' : 'text-red-700'}`}>
                                      Ashley AI: {file.ashleyAnalysis.feasible ? 'Analysis Complete' : 'Issues Found'}
                                    </span>
                                  </div>
                                  
                                  {file.ashleyAnalysis.requirements.length > 0 && (<div className="text-sm">
                                      <p className="font-medium text-blue-700 mb-1">Requirements Identified:</p>
                                      <ul className="space-y-1">
                                        {file.ashleyAnalysis.requirements.map((req, i) => (<li key={i} className="text-blue-600 flex items-start gap-1">
                                            <lucide_react_1.CheckCircle className="w-3 h-3 mt-0.5 flex-shrink-0"/>
                                            {req}
                                          </li>))}
                                      </ul>
                                    </div>)}
                                  
                                  {file.ashleyAnalysis.suggestions.length > 0 && (<div className="text-sm">
                                      <p className="font-medium text-green-700 mb-1">Suggestions:</p>
                                      <ul className="space-y-1">
                                        {file.ashleyAnalysis.suggestions.map((suggestion, i) => (<li key={i} className="text-green-600 flex items-start gap-1">
                                  ...
                                  {file.ashleyAnalysis.warnings.length > 0 && (<div className="text-sm">
                                      ...
                                      <ul className="space-y-1">
                                        {file.ashleyAnalysis.warnings.map((warning, i) => (<li key={i} className="text-amber-600 flex items-start gap-1">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mb-3">
            Provide any specific requirements, color matching details, placement instructions, or other important notes
          </p>

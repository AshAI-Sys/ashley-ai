...
export function DatesSLAsSection({
  ...
  const [timelineValidation, setTimelineValidation] =
    useState<TimelineValidation | null>(null);
  ...
  const validateTimeline = async () => {
    ...
    try {
      ...
      const feasible =
        workingDays >= Math.ceil(slaInfo.expressDays * quantityMultiplier);
      ...
      if (isRush && feasible) {
        suggestions.push(
          `Rush order detected. Consider express production (${slaInfo.expressDays} days base)`
        );
        suggestions.push(
          `Additional ${slaInfo.rushSurcharge}% surcharge will apply`
        );
      }

      if (!feasible) {
        const earliestDate = addWorkingDays(
          today,
          Math.ceil(slaInfo.expressDays * quantityMultiplier)
        );
        suggestions.push(
          `Earliest possible delivery: ${format(earliestDate, "MMM dd, yyyy")}`
        );
      ...
      if (workingDays > requiredDays * 1.5) {
        suggestions.push(
          "Extended timeline allows for cost optimization opportunities"
        );
      }

      if (quantityMultiplier > 1) {
        suggestions.push(
          `Large quantity (${totalQuantity}) extends base timeline by ${Math.round((quantityMultiplier - 1) * 100)}%`
        );
      }
  ...
  const generateSuggestedDates = (slaInfo: SLAInfo) => {
    ...
    // Express delivery
    const expressDate = addWorkingDays(
      today,
      Math.ceil(slaInfo.expressDays * quantityMultiplier)
    );
    ...
    // Standard delivery
    const standardDate = addWorkingDays(
      today,
      Math.ceil(slaInfo.standardDays * quantityMultiplier)
    );
    suggested.push(standardDate);

    // Safe delivery (standard + 2 days buffer)
    const safeDate = addWorkingDays(
      today,
      Math.ceil(slaInfo.standardDays * quantityMultiplier) + 2
    );
        ...
        {timelineValidation && (
          ...
          >
            <div className="flex items-start gap-3">
              {timelineValidation.feasible ? (
                timelineValidation.isRush ? (
                  <Zap className="mt-1 h-5 w-5 text-amber-600" />
                ) : (

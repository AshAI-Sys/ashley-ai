...
function ProductDesignSection({ garmentType, printingMethod, designFiles, onGarmentTypeChange, onPrintingMethodChange, onDesignFilesChange }) {
    ...
    const uploadFile = async (file) => {
        ...
        for (let progress = 0; progress <= 100; progress += 10) {
            ...
            const updatedFiles = designFiles.map(f => f.id === file.id ? { ...f, uploadProgress: progress } : f);
            onDesignFilesChange(updatedFiles);
        }
        // Mark as uploaded
        const uploadedFiles = designFiles.map(f => f.id === file.id ? { ...f, uploaded: true } : f);
    ...
    const validateWithAshley = async (file) => {
        ...
        try {
            ...
            const validatedFiles = designFiles.map(f => f.id === file.id ? { ...f, ashleyValidation: mockValidation } : f);
    ...
    return (<card_1.Card>
      ...
      <card_1.CardContent className="space-y-6">
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label_1.Label>Garment Type *</label_1.Label>
          ...
          <div>
            <label_1.Label>Printing Method *</label_1.Label>
        ...
        {selectedPrintingMethod && (<div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          ...
          <div className="space-y-3">
            ...
            : 'border-gray-300 hover:border-gray-400'}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
              ...
              <button_1.Button onClick={triggerFileSelect} variant="outline">
                <lucide_react_1.FileImage className="w-4 h-4 mr-2"/>
                Choose Files
              </button_1.Button>
            ...
            {designFiles.length > 0 && (<div className="space-y-3">
                ...
                {designFiles.map((file) => (<div key={file.id} className="border rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3 flex-1">
                        {file.preview ? (<img src={file.preview} alt={file.name} className="w-16 h-16 object-cover rounded border"/>) : (<div className="w-16 h-16 bg-gray-100 rounded border flex items-center justify-center">
                            <lucide_react_1.FileImage className="w-6 h-6 text-gray-400"/>
                          </div>)}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {file.uploaded && validatingWithAshley === file.id && (<div className="flex items-center gap-2 mt-2 text-sm text-blue-600">
                          ...
                          {file.ashleyValidation && (<div className="mt-3 space-y-2">
                              <div className="flex items-center gap-2">
                                {file.ashleyValidation.feasible ? (<lucide_react_1.CheckCircle className="w-4 h-4 text-green-500"/>) : (<lucide_react_1.AlertCircle className="w-4 h-4 text-red-500"/>)}
                                <span className={`text-sm font-medium ${file.ashleyValidation.feasible ? 'text-green-700' : 'text-red-700'}`}>
                              ...
                              {file.ashleyValidation.suggestions.length > 0 && (<div className="text-sm">
                                  ...
                                  <ul className="space-y-1">
                                    {file.ashleyValidation.suggestions.map((suggestion, i) => (<li key={i} className="text-blue-600 flex items-start gap-1">
                              ...
                              {file.ashleyValidation.warnings.length > 0 && (<div className="text-sm">
                                  ...
                                  <ul className="space-y-1">
                                    {file.ashleyValidation.warnings.map((warning, i) => (<li key={i} className="text-amber-600 flex items-start gap-1">

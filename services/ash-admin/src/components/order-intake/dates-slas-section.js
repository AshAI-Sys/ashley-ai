...
function DatesSLAsSection({ deliveryDate, printingMethod, totalQuantity, onDeliveryDateChange, onTimelineValidation, onRushSurcharge, }) {
    ...
    return (<card_1.Card>
      ...
      <card_1.CardContent className="space-y-6">
          ...
          <div className="relative flex gap-2">
            <popover_1.Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
              <popover_1.PopoverTrigger asChild>
                <button_1.Button variant="outline" className="w-full justify-start text-left font-normal" type="button">
                  <lucide_react_1.CalendarIcon className="mr-2 h-4 w-4"/>
                  {selectedDate ? ((0, date_fns_1.format)(selectedDate, "PPP")) : (<span>Select delivery date</span>)}
                </button_1.Button>
        ...
        {/* SLA Information */}
        {slaInfo && (<div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
            <div className="mb-3 flex items-center gap-2">
              <lucide_react_1.Clock className="h-5 w-5 text-blue-600"/>
              <h4 className="font-medium text-blue-900">
                {printingMethod.charAt(0).toUpperCase() +
                printingMethod.slice(1)}{" "}
                SLA
              </h4>
            </div>
            ...
            <div className="grid gap-4 text-sm md:grid-cols-2">
              <div>
                ...
                <div className="text-blue-800">
                  {Math.ceil(slaInfo.standardDays * getQuantityMultiplier())}{" "}
                  working days
                  {getQuantityMultiplier() > 1 && (<span className="ml-1 text-blue-600">
              ...
              <div>
                ...
                <div className="text-blue-800">
                  {slaInfo.expressAvailable ? (<>
                      {Math.ceil(slaInfo.expressDays * getQuantityMultiplier())}{" "}
                      working days
                      <span className="ml-1 text-amber-600">
                        (+{slaInfo.rushSurcharge}% surcharge)
                      </span>
                    </>) : ("Not available")}
                </div>
              </div>
        ...
        {validating && (<div className="rounded-lg border border-purple-200 bg-purple-50 p-4">
            <div className="flex items-center gap-2">
              <lucide_react_1.Sparkles className="h-5 w-5 animate-pulse text-purple-600"/>
              <span className="font-medium text-purple-900">
                Ashley AI is analyzing timeline...
              </span>
            </div>
          </div>)}

        {timelineValidation && (<div className={`rounded-lg border p-4 ${timelineValidation.feasible
                ? timelineValidation.isRush
                    ? "border-amber-200 bg-amber-50"
                    : "border-green-200 bg-green-50"
                : "border-red-200 bg-red-50"}`}>
            <div className="flex items-start gap-3">
              {timelineValidation.feasible ? (timelineValidation.isRush ? (<lucide_react_1.Zap className="mt-1 h-5 w-5 text-amber-600"/>) : (<lucide_react_1.CheckCircle className="mt-1 h-5 w-5 text-green-600"/>)) : (<lucide_react_1.AlertTriangle className="mt-1 h-5 w-5 text-red-600"/>)}
              ...
              <div className="flex-1">
                ...
                : "text-red-900"}`}>
                    ...
                    {timelineValidation.isRush &&
                timelineValidation.feasible && (<badge_1.Badge variant="outline" className="ml-2 border-amber-300 text-amber-700">
                          Rush Order
                        </badge_1.Badge>)}
                  </h4>
                ...
                <div className="mb-3 grid gap-4 text-sm md:grid-cols-2">
                  <div>
                    <span className="font-medium">Available Days:</span>
                    <span className="ml-2">
                      {timelineValidation.workingDays} working days
                    </span>
                  </div>
                  <div>
                    <span className="font-medium">Required Days:</span>
                    <span className="ml-2">
                      {timelineValidation.requiredDays} working days
                    </span>
                  </div>
                </div>
                ...
                {timelineValidation.suggestions.length > 0 && (<div className="mb-3">
                    ...
                    <ul className="space-y-1">
                      {timelineValidation.suggestions.map((suggestion, i) => (<li key={i} className="flex items-start gap-1 text-sm">
                ...
                {timelineValidation.warnings.length > 0 && (<div>
                    ...
                    <ul className="space-y-1">
                      {timelineValidation.warnings.map((warning, i) => (<li key={i} className="flex items-start gap-1 text-sm text-red-600">
                          <lucide_react_1.AlertTriangle className="mt-0.5 h-3 w-3 flex-shrink-0"/>
                          {warning}
                        </li>))}
                    </ul>
                  </div>)}
        ...
        {suggestedDates.length > 0 && (<div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {suggestedDates.map((date, index) => {
                ...
                return (<button key={index} onClick={() => selectSuggestedDate(date)} className={`rounded-lg border-2 p-3 text-left transition-colors ${selectedDate &&
                        (0, date_fns_1.format)(selectedDate, "yyyy-MM-dd") ===

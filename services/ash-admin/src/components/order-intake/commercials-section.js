...
const PAYMENT_TERMS = [
    ...
    },
    {
        value: "50_50",
        label: "50/50 Split",
        description: "50% deposit, 50% on delivery",
    },
    {
        value: "prepaid",
        label: "Full Prepayment",
        description: "100% payment upfront",
    },
];
function CommercialsSection({ totalQuantity, printingMethod, garmentType, addOnsCost, colorVariantsCost, rushSurchargePercent, commercials, onCommercialsChange, }) {
    ...
    (0, react_1.useEffect)(() => {
        ...
        commercials.unitPrice,
        commercials.depositPercentage,
    ...
    const getPricingBreak = () => {
        ...
        if (!garmentPricing)
            return null;
        return (garmentPricing.find(break_ => totalQuantity >= break_.minQuantity &&
            totalQuantity <= break_.maxQuantity) || null);
    };
    const calculatePricing = () => {
        ...
        const updatedCommercials = {
            ...
            depositAmount: commercials.taxInclusive
                ? depositAmount
                : depositAmount * 1.12,
            balanceAmount: commercials.taxInclusive
                ? balanceAmount
                : balanceAmount * 1.12,
        };
    ...
    return (<card_1.Card className="border-2">
      <card_1.CardHeader className="border-b-2 bg-gradient-to-r from-green-50 to-emerald-50">
      ...
      <card_1.CardContent className="space-y-6 pt-6">
        ...
        {pricingBreak && (<div className="rounded-lg border border-green-200 bg-green-50 p-4">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <lucide_react_1.TrendingDown className="h-5 w-5 text-green-600"/>
                <h4 className="font-medium text-green-900">
                  Volume Pricing Applied
                </h4>
              </div>
              <badge_1.Badge variant="outline" className="border-green-300 text-green-700">
            ...
            <div className="grid gap-4 text-sm md:grid-cols-2">
              ...
              <div>
                <span className="font-medium text-green-700">Base Price:</span>
                <div className="text-green-800">
                  ₱{pricingBreak.basePrice}/piece
                </div>
              </div>
            </div>

            {nextBreak && (<div className="mt-3 rounded border bg-blue-50 p-3">
                <p className="text-sm text-blue-800">
                  <strong>Next break:</strong> Order {nextBreak.minQuantity}{" "}
                  pieces for ₱{nextBreak.basePrice}/piece ({nextBreak.discount}%
                  discount)
                </p>
              </div>)}
          </div>)}
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="space-y-2">
            <label_1.Label className="text-sm font-semibold">Unit Price (₱) *</label_1.Label>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="space-y-2">
            <label_1.Label className="text-sm font-semibold">Payment Terms</label_1.Label>
          ...
          <div className="space-y-2">
            <label_1.Label className="text-sm font-semibold">Currency</label_1.Label>
        ...
        <div className="rounded-lg border border-purple-200 bg-purple-50 p-4">
          ...
          {priceRecommendation && (<div className="rounded bg-purple-100 p-3 text-sm text-purple-800">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span>
                Subtotal ({totalQuantity} × ₱{commercials.unitPrice})
              </span>
              <span>₱{commercials.subtotal.toLocaleString()}</span>
            </div>

            {addOnsCost > 0 && (<div className="flex items-center justify-between text-blue-600">
                <span>Add-ons Cost</span>
                <span>₱{addOnsCost.toLocaleString()}</span>
              </div>)}

            {colorVariantsCost > 0 && (<div className="flex items-center justify-between text-purple-600">
                <span>Color Variants Cost</span>
                <span>₱{colorVariantsCost.toLocaleString()}</span>
              </div>)}

            {commercials.rushSurcharge > 0 && (<div className="flex items-center justify-between text-amber-600">
                <span>Rush Surcharge ({rushSurchargePercent}%)</span>
                <span>₱{commercials.rushSurcharge.toLocaleString()}</span>
              </div>)}

            {commercials.quantityDiscount > 0 && (<div className="flex items-center justify-between text-green-600">
                <span>Quantity Discount ({pricingBreak?.discount}%)</span>
                <span>-₱{commercials.quantityDiscount.toLocaleString()}</span>
              </div>)}

            {!commercials.taxInclusive && (<div className="flex items-center justify-between text-gray-600">
                <span>VAT (12%)</span>
                <span>
        ...
        <div className="space-y-4">
          <h4 className="flex items-center gap-2 font-medium">
            <lucide_react_1.Percent className="h-4 w-4"/>
            Payment Breakdown
          </h4>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
              <div className="mb-1 font-medium text-blue-900">
                Deposit ({commercials.depositPercentage}%)
              </div>
            ...
            <div className="rounded-lg border border-green-200 bg-green-50 p-4">
              <div className="mb-1 font-medium text-green-900">
                Balance ({100 - commercials.depositPercentage}%)
              </div>
              ...
              <div className="mt-1 text-sm text-green-600">
                Due{" "}
                {PAYMENT_TERMS.find(t => t.value === commercials.paymentTerms)?.label.toLowerCase()}
              </div>
            </div>
          </div>
        </div>
        ...
        {commercials.unitPrice > 0 && (<div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
            <div className="mb-2 flex items-center gap-2">
              <lucide_react_1.AlertCircle className="h-4 w-4 text-gray-700"/>
            ...
            <div className="text-sm">
              ...
              <span className="ml-2 font-medium">
                {Math.round(((commercials.unitPrice - 80) / commercials.unitPrice) * 100)}
                % margin
              </span>
            </div>
          </div>)}

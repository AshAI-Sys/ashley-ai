...
export function VariantsAddonsSection({
  ...
  const selectPresetColor = (variantId: string, color: { name: string; hex: string }) => {
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        onChange={(e) => updateColorVariant(variant.id, 'hexCode', e.target.value)}
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {PRESET_COLORS.map((color) => (
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-1 border rounded-md hover:bg-gray-50 text-sm"
                        title={color.name}
                      >
                        <div
          ...
          {colorVariants.length > 0 && (
            <div className="bg-muted/50 rounded-lg p-3">
              ...
              <div className="space-y-2">
                {colorVariants.filter(v => v.quantity > 0).map((variant) => (
            ...
            {Object.entries(groupedAddOns).map(([type, typeAddOns]) => (
              <div key={type}>
                <h4 className="font-medium capitalize mb-3 flex items-center gap-2">
                  {type === 'print' && <Palette className="w-4 h-4" />}
                  {type === 'garment' && <Tag className="w-4 h-4" />}
                  {type === 'packaging' && <Package className="w-4 h-4" />}

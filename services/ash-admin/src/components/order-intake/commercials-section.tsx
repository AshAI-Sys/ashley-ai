...
export function CommercialsSection({
  ...
  const [priceRecommendation, setPriceRecommendation] = useState<string | null>(
    null
  );
  ...
  const getPricingBreak = (): PricingBreak | null => {
    ...
    const garmentPricing =
      methodPricing[garmentType] || methodPricing["t-shirt"];
    if (!garmentPricing) return null;

    return (
      garmentPricing.find(
        break_ =>
          totalQuantity >= break_.minQuantity &&
          totalQuantity <= break_.maxQuantity
      ) || null
    );
  };
  ...
  const calculatePricing = () => {
    ...
    const totalBeforeDiscount =
      subtotal + addOnsCost + colorVariantsCost + rushSurchargeAmount;
  ...
  const optimizeWithAshley = async () => {
    ...
    try {
      ...
      if (nextBreak) {
        ...
        const savings =
          (commercials.unitPrice - nextBreak.basePrice) * nextBreak.minQuantity;
        suggestions.push(
      ...
      const currentMargin =
        ((commercials.unitPrice - 80) / commercials.unitPrice) * 100; // Assuming ₱80 base cost
      if (currentMargin > 60) {
        suggestions.push(
          "Price is competitive with good margin. Consider slight reduction for bulk orders."
        );
      } else if (currentMargin < 30) {
        suggestions.push(
          "Margin is tight. Consider increasing price or optimizing production costs."
        );
      }

      // Market timing
      suggestions.push(
        "Current market conditions favor standard pricing. No seasonal adjustments needed."
      );
  ...
  return (
    <Card className="border-2">
      <CardHeader className="border-b-2 bg-gradient-to-r from-green-50 to-emerald-50">
      ...
      <CardContent className="space-y-6 pt-6">
        ...
        <div className="rounded-lg border border-purple-200 bg-purple-50 p-4">
          ...
          {priceRecommendation && (
            <div className="rounded bg-purple-100 p-3 text-sm text-purple-800">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-lg border border-green-200 bg-green-50 p-4">
              ...
              <div className="mt-1 text-sm text-green-600">
                Due{" "}
                {PAYMENT_TERMS.find(
                  t => t.value === commercials.paymentTerms
                )?.label.toLowerCase()}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <div className="text-sm">
              ...
              <span className="ml-2 font-medium">
                {Math.round(
                  ((commercials.unitPrice - 80) / commercials.unitPrice) * 100
                )}
                % margin
              </span>
            </div>

...
async function middleware(request) {
    ...
    if (!(await checkRateLimit(request))) {
        ...
        (0, audit_logger_1.logSecurityEvent)('RATE_LIMIT_EXCEEDED', request, {
            path: pathname,
    ...
    if (pathname.startsWith('/admin') || pathname.startsWith('/api/admin')) {
        if (!checkIPWhitelist(request)) {
            // Log IP block event
            (0, audit_logger_1.logSecurityEvent)('IP_BLOCKED', request, {
                path: pathname,
    ...
    if (isStateChanging && isAPI && !skipCSRF) {
        if (!(await verifyCSRFToken(request))) {
            // Log CSRF violation
            (0, audit_logger_1.logSecurityEvent)('CSRF_VIOLATION', request, {
                path: pathname,

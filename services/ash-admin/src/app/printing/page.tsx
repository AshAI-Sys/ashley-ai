...
export default function PrintingPage() {
  ...
  const ____isLoading = runsLoading || machinesLoading || dashboardLoading;
  ...
  const getCompletedQty = (run: PrintRun) => {
    return (run.outputs || []).reduce(
      (sum, output) => sum + output.qty_completed,
      0
    );
  };

  const getRejectedQty = (run: PrintRun) => {
    return (run.rejects || []).reduce(
      (sum, reject) => sum + reject.qty_rejected,
      0
    );
  };
  ...
  const ____PrintRunCardSkeleton = () => (
    <Card>
      <CardHeader>
                    ...
                    >
                      <CardHeader>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div
                              className={`rounded-lg p-2 ${methodColors[run.method]}`}
                            >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Badge
                              variant="outline"
                              className={methodColors[run.method]}
                            >
                              {run.method}
                            </Badge>
                          </div>
                ...
                <Card>
                  <CardContent className="flex h-32 items-center justify-center">
                    <div className="text-center">
                      <Printer className="mx-auto mb-2 h-8 w-8 text-muted-foreground" />
                      <p className="text-muted-foreground">
                ...
                <Card key={machine.id}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        variant={machine.is_active ? "default" : "secondary"}
                      >
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {(dashboard?.method_breakdown || []).map(item => {
                        const MethodIcon =
                          methodIcons[item.method as keyof typeof methodIcons];
                        return (
                          <div
                            key={item.method}
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <div
                                className={`rounded p-1 ${methodColors[item.method as keyof typeof methodColors]}`}
                              >
                    ...
                    {(dashboard?.recent_rejects || []).length > 0 ? (
                      <div className="space-y-2">
                        {(dashboard?.recent_rejects || []).map(reject => (
                          <div key={reject.id} className="rounded border p-2">

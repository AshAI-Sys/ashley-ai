    ...
    return (<div className="container mx-auto py-6">
      ...
      <div className="space-y-6">
        {versions.length > 0 ? (versions
            ...
            const latestCheck = versionChecks.sort((a, b) => new Date(b.created_at).getTime() -
                  ...
                  <card_1.CardHeader>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        {version.version === design.current_version && (<badge_1.Badge className="bg-blue-100 text-blue-800">
                            Current
                          </badge_1.Badge>)}
                  ...
                  <card_1.CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                        {files.mockup_url && (<div className="flex items-center gap-2 rounded bg-gray-50 p-2 text-sm">
                            <lucide_react_1.Image className="h-4 w-4 text-blue-600"/>
                            <span>Mockup</span>
                          </div>)}
                        {files.prod_url && (<div className="flex items-center gap-2 rounded bg-gray-50 p-2 text-sm">
                            <lucide_react_1.FileText className="h-4 w-4 text-green-600"/>
                            <span>Production</span>
                          </div>)}
                        {files.separations && files.separations.length > 0 && (<div className="flex items-center gap-2 rounded bg-gray-50 p-2 text-sm">
                            <lucide_react_1.FileText className="h-4 w-4 text-purple-600"/>
                            <span>{files.separations.length} Separations</span>
                          </div>)}
                        {files.dst_url && (<div className="flex items-center gap-2 rounded bg-gray-50 p-2 text-sm">
                            <lucide_react_1.FileText className="h-4 w-4 text-pink-600"/>
                    ...
                    {palette.length > 0 && (<div>
                        ...
                        <div className="flex gap-1">
                          {palette
                        .slice(0, 8)
                        .map((color, index) => (<div key={index} className="h-6 w-6 rounded border border-gray-300" style={{ backgroundColor: color }} title={color}/>))}
                          {palette.length > 8 && (<div className="flex h-6 w-6 items-center justify-center rounded border border-gray-300 bg-gray-100 text-xs">
                              +{palette.length - 8}
                            </div>)}
                        </div>
                      </div>)}
                    ...
                    {latestCheck && (<div className="border-t pt-4">
                        ...
                        0 && (<span className="text-sm text-amber-600">

...
function NewDesignPage() {
    ...
    const DropZone = ({ type, children, }) => (<div className="rounded-lg border-2 border-dashed border-gray-300 p-6 transition-colors hover:border-blue-400" onDrop={e => handleDrop(e, type)} onDragOver={handleDragOver}>
      {children}
    </div>);
    const FileList = ({ type, title, }) => (<div>
      ...
      <DropZone type={type}>
        <div className="text-center">
          ...
          <input type="file" className="hidden" multiple={type === "separations"} accept={type === "mockups"
            ? "image/*"
            : type === "production"
                ? "image/*,.pdf,.ai"
                : type === "separations"
      ...
      {formData.files[type].length > 0 && (<div className="mt-3 space-y-2">
          {formData.files[type].map((fileUpload, index) => (<div key={index} className="flex items-center justify-between rounded bg-gray-50 p-2">
              <div className="flex items-center gap-2">
                <lucide_react_1.File className="h-4 w-4"/>
                ...
                {fileUpload.error && (<lucide_react_1.AlertCircle className="h-4 w-4 text-red-500"/>)}
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent className="space-y-4">
              <div>
                ...
                {selectedOrder && (<div className="mt-2 rounded bg-blue-50 p-2 text-sm">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent className="space-y-6">
              ...
              {validationResult && (<div className={`rounded-lg p-4 ${validationResult.status === "PASS"
                ? "border border-green-200 bg-green-50"
                : validationResult.status === "WARN"
                    ? "border border-yellow-200 bg-yellow-50"
                    : "border border-red-200 bg-red-50"}`}>
                  <div className="mb-2 flex items-center gap-2">
                    {validationResult.status === "PASS" && (<lucide_react_1.CheckCircle className="h-5 w-5 text-green-600"/>)}
                    {validationResult.status === "WARN" && (<lucide_react_1.AlertCircle className="h-5 w-5 text-yellow-600"/>)}
                    {validationResult.status === "FAIL" && (<lucide_react_1.AlertCircle className="h-5 w-5 text-red-600"/>)}
                    <h4 className="font-medium">Ashley AI Validation Result</h4>
                  </div>
                  {validationResult.issues &&
                validationResult.issues.length > 0 && (<ul className="space-y-1 text-sm">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent className="space-y-4">
              {formData.placements.map((placement, index) => (<div key={placement.id} className="space-y-4 rounded-lg border p-4">
                  <div className="flex items-center justify-between">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                {formData.palette.map((color, index) => (<div key={index} className="flex items-center gap-2 rounded border p-2">
                    <input type="color" value={color} onChange={e => updateColor(index, e.target.value)} className="h-8 w-8 cursor-pointer rounded border border-gray-300"/>
        ...
        <div className="space-y-6">
          ...
          {Object.values(formData.files)
            .flat()
            .some(f => f.uploading) && (<card_1.Card>
              <card_1.CardHeader>
                <card_1.CardTitle>Upload Progress</card_1.CardTitle>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-2">
                  {Object.entries(formData.files).map(([type, files]) => files
                .filter(f => f.uploading)
                .map((file, index) => (<div key={`${type}-${index}`} className="text-sm">

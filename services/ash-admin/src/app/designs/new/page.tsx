...
export default function NewDesignPage() {
  ...
  const handleFileUpload = useCallback(async (file: File, type: keyof DesignForm['files'], index?: number) => {
  ...
  const handleDrop = useCallback((e: React.DragEvent, type: keyof DesignForm['files']) => {
    e.preventDefault()
  ...
  const validateForm = () => {
    ...
    const hasFiles = Object.values(formData.files).some(fileArray => 
      fileArray.length > 0 && fileArray.some(f => f.uploaded)
    )
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`Design ${isDraft ? 'saved as draft' : 'uploaded successfully'}`)
  ...
  const DropZone = ({ type, children }: { type: keyof DesignForm['files'], children: React.ReactNode }) => (
    <div
  ...
  const FileList = ({ type, title }: { type: keyof DesignForm['files'], title: string }) => (
    <div>
          ...
          {formData.files[type].map((fileUpload, index) => (
            <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded">
              <div className="flex items-center gap-2">
                ...
                {fileUpload.uploading && <Clock className="w-4 h-4 text-blue-500 animate-spin" />}
                {fileUpload.uploaded && <CheckCircle className="w-4 h-4 text-green-500" />}
                {fileUpload.error && <AlertCircle className="w-4 h-4 text-red-500" />}
              </div>
              <Button
                variant="ghost"
                size="sm"
    ...
    <div className="container mx-auto py-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Link this design to a Purchase Order and provide basic details</CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Upload design files with drag & drop support</CardDescription>
                </div>
                <Button 
            ...
            <CardContent className="space-y-6">
              ...
              {formData.method === 'EMBROIDERY' && (
                <FileList type="embroidery" title="Embroidery Files (DST/EMB)" />
              )}
              ...
              {validationResult && (
                <div className={`p-4 rounded-lg ${
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    {validationResult.status === 'PASS' && <CheckCircle className="w-5 h-5 text-green-600" />}
                    {validationResult.status === 'WARN' && <AlertCircle className="w-5 h-5 text-yellow-600" />}
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Specify placement areas, sizes, and positions</CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              {formData.placements.map((placement, index) => (
                <div key={placement.id} className="border rounded-lg p-4 space-y-4">
                  <div className="flex justify-between items-center">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updatePlacement(placement.id, 'width_cm', parseFloat(e.target.value) || 0)}
                        min="0"
                        step="0.1"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updatePlacement(placement.id, 'height_cm', parseFloat(e.target.value) || 0)}
                        min="0"
                        step="0.1"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updatePlacement(placement.id, 'offset_x', parseFloat(e.target.value) || 0)}
                        step="0.1"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updatePlacement(placement.id, 'offset_y', parseFloat(e.target.value) || 0)}
                        step="0.1"
                      />
                    </div>
                  </div>
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Define color specifications and Pantone codes</CardDescription>
                </div>
        ...
        <div className="space-y-6">
          ...
          {/* Upload Progress */}
          {Object.values(formData.files).flat().some(f => f.uploading) && (
            <Card>
              <CardHeader>
              ...
              <CardContent>
                <div className="space-y-2">
                  {Object.entries(formData.files).map(([type, files]) => 
                    files.filter(f => f.uploading).map((file, index) => (

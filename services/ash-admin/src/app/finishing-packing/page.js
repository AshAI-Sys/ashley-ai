...
function FinishingPackingPage() {
    ...
    const [finishingRuns, setFinishingRuns] = (0, react_1.useState)([]);
    const [cartons, setCartons] = (0, react_1.useState)([]);
    ...
    return (<dashboard_layout_1.default>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Stats Cards */}
        {stats && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
            <card_1.Card>
        ...
        <div className="border-b border-gray-200 mb-6">
          <nav className="-mb-px flex space-x-8">
            <button onClick={() => setActiveTab('finishing')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'finishing'
            ? 'border-blue-500 text-blue-600'
            : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
              <lucide_react_1.Scissors className="w-4 h-4 inline mr-2"/>
              Finishing Operations
            </button>
            <button onClick={() => setActiveTab('packing')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'packing'
            ? 'border-blue-500 text-blue-600'
            : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
              <lucide_react_1.Box className="w-4 h-4 inline mr-2"/>
        ...
        {/* Content */}
        {activeTab === 'finishing' ? (<FinishingRunsTable runs={filteredFinishingRuns} getStatusBadge={getStatusBadge}/>) : (<PackingCartonsTable cartons={filteredCartons} getStatusBadge={getStatusBadge}/>)}
      </div>
    </dashboard_layout_1.default>);
}
// Separate component for Finishing Runs Table
function FinishingRunsTable({ runs, getStatusBadge }) {
    return (<card_1.Card>
      <card_1.CardHeader>
      ...
      <card_1.CardContent>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {runs.map((run) => (<tr key={run.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3 w-16 bg-gray-200 rounded-full h-2">
                        <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${(run.tasks_completed / run.total_tasks) * 100}%` }}></div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {run.materials_used.map((material, index) => (<div key={index} className="text-xs text-gray-600">
        ...
        {runs.length === 0 && (<div className="text-center py-12">
            <lucide_react_1.Scissors className="mx-auto h-12 w-12 text-gray-400"/>
...
function PackingCartonsTable({ cartons, getStatusBadge }) {
    return (<card_1.Card>
      <card_1.CardHeader>
      ...
      <card_1.CardContent>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {cartons.map((carton) => (<tr key={carton.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm font-medium text-gray-900">
                        {carton.carton_no}
                    ...
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        <div className="w-16 bg-gray-200 rounded-full h-2">
                          <div className={`h-2 rounded-full ${carton.fill_percent >= 90 ? 'bg-green-600' :
                carton.fill_percent >= 70 ? 'bg-yellow-600' : 'bg-red-600'}`} style={{ width: `${carton.fill_percent}%` }}></div>
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center space-x-2">
                      ...
                      {carton.status === 'OPEN' && (<button_1.Button size="sm" variant="outline">
                          <lucide_react_1.PackageCheck className="w-4 h-4"/>
                        </button_1.Button>)}
        ...
        {cartons.length === 0 && (<div className="text-center py-12">
            <lucide_react_1.Box className="mx-auto h-12 w-12 text-gray-400"/>

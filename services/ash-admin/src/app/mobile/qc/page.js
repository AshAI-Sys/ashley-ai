...
exports.default = MobileQCPage;
const react_1 = require("react");
const lucide_react_1 = require("lucide-react");
const navigation_1 = require("next/navigation");
function MobileQCPage() {
    const [step, setStep] = (0, react_1.useState)("search");
    const [bundleCode, setBundleCode] = (0, react_1.useState)("");
    const [bundle, setBundle] = (0, react_1.useState)(null);
    const [inspectionData, setInspectionData] = (0, react_1.useState)({
        sample_size: 0,
        inspected: 0,
        passed: 0,
        failed: 0,
    ...
    const [defectTypes, setDefectTypes] = (0, react_1.useState)([]);
    ...
    const calculateAQLSampleSize = (lotSize) => {
        // Simplified AQL sample size calculation (based on ANSI/ASQ Z1.4)
        if (lotSize <= 50)
            return Math.min(lotSize, 8);
        if (lotSize <= 90)
            return 13;
        if (lotSize <= 150)
            return 20;
    ...
    const progress = inspectionData.sample_size > 0
        ...
        : 0;
    return (<div className="min-h-screen bg-gray-900 pb-20 text-white">
      {/* Header */}
      <div className="border-b border-gray-700 bg-gray-800 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            {step !== "search" && (<button onClick={() => setStep("search")} className="rounded-lg p-2 hover:bg-gray-700">
                <lucide_react_1.ChevronLeft className="h-5 w-5"/>
              </button>)}
            <h1 className="text-xl font-bold">Mobile QC</h1>
          </div>
          <button onClick={() => router.push("/mobile/dashboard")} className="text-sm text-gray-500 hover:text-white">
            Dashboard
          </button>
        </div>
      </div>
      ...
      {/* Search Step */}
      {step === "search" && (<div className="space-y-4 p-4">
          <div className="rounded-lg bg-gray-800 p-6">
            <div className="mb-6 text-center">
              <lucide_react_1.Package className="mx-auto mb-3 h-16 w-16 text-blue-400"/>
            ...
            <div className="space-y-4">
              <div>
                <input type="text" value={bundleCode} onChange={e => setBundleCode(e.target.value)} placeholder="Enter bundle code..." className="w-full rounded-lg border border-gray-600 bg-gray-700 px-4 py-3 text-white placeholder-gray-400 focus:border-blue-500 focus:outline-none" onKeyPress={e => e.key === "Enter" && searchBundle()}/>
              </div>

              <button onClick={searchBundle} disabled={loading || !bundleCode.trim()} className="flex w-full items-center justify-center space-x-2 rounded-lg bg-blue-600 px-6 py-3 font-medium text-white transition-colors hover:bg-blue-700 disabled:bg-gray-600">
              ...
              <button onClick={() => router.push("/mobile/scanner")} className="flex w-full items-center justify-center space-x-2 rounded-lg bg-gray-700 px-6 py-3 font-medium text-white transition-colors hover:bg-gray-600">
      ...
      {/* Inspection Step */}
      {step === "inspect" && bundle && (<div className="space-y-4 p-4">
          {/* Bundle Info */}
          ...
          {/* Inspection Actions */}
          {inspectionData.inspected < inspectionData.sample_size && (<div className="rounded-lg bg-gray-800 p-4">
              <h3 className="mb-4 font-medium">
                Inspect Unit #{inspectionData.inspected + 1}
              </h3>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={recordPass} className="flex flex-col items-center justify-center space-y-2 rounded-lg bg-green-600 py-6 transition-colors hover:bg-green-700">
                  <lucide_react_1.CheckCircle className="h-12 w-12"/>
                  <span className="font-medium">PASS</span>
                </button>
                <button onClick={recordFail} className="flex flex-col items-center justify-center space-y-2 rounded-lg bg-red-600 py-6 transition-colors hover:bg-red-700">
                  <lucide_react_1.XCircle className="h-12 w-12"/>
          ...
          {/* Defects List */}
          {inspectionData.defects.length > 0 && (<div className="rounded-lg bg-gray-800 p-4">
              <h3 className="mb-3 font-medium">
                Recorded Defects ({inspectionData.defects.length})
              </h3>
              <div className="space-y-2">
                {inspectionData.defects.map(defect => (<div key={defect.id} className="flex items-center justify-between rounded-lg bg-gray-700 p-3">
                    <div>
                    ...
                    <span className={`rounded-full px-2 py-1 text-xs font-medium ${defect.severity === "CRITICAL"
          ...
          {/* Complete Inspection */}
          {inspectionData.inspected >= inspectionData.sample_size && (<button onClick={() => setStep("summary")} className="flex w-full items-center justify-center space-x-2 rounded-lg bg-blue-600 px-6 py-4 font-medium text-white transition-colors hover:bg-blue-700">
      ...
      {/* Defects Step */}
      {step === "defects" && (<div className="space-y-4 p-4">
          ...
          <div className="rounded-lg bg-gray-800 p-4">
            ...
            <div className="max-h-96 space-y-2 overflow-y-auto">
              {defectTypes.map(defectType => (<button key={defectType.id} onClick={() => {
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`rounded-full px-2 py-1 text-xs font-medium ${defectType.default_severity === "CRITICAL"
                    ? "bg-red-900 text-red-200"
          ...
          <button onClick={() => setStep("inspect")} className="w-full rounded-lg bg-gray-700 px-6 py-3 font-medium text-white transition-colors hover:bg-gray-600">
            Back to Inspection
          </button>
        </div>)}

      {/* Summary Step */}
      {step === "summary" && (<div className="space-y-4 p-4">
          <div className="rounded-lg bg-gray-800 p-6">
            ...
            <div className="space-y-4">
              ...
              {/* Decision */}
              <div className={`rounded-lg p-6 text-center ${parseFloat(defectRate) <= 2.5
                ...
                {parseFloat(defectRate) <= 2.5 ? (<>
                    <lucide_react_1.CheckCircle className="mx-auto mb-3 h-16 w-16 text-green-400"/>
                    ...
                    <lucide_react_1.XCircle className="mx-auto mb-3 h-16 w-16 text-red-400"/>
              ...
              <div>
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="flex items-center space-x-1 text-sm text-blue-400 hover:text-blue-300">
                ...
                {inspectionData.photos.length > 0 && (<div className="grid grid-cols-3 gap-2">
                    {inspectionData.photos.map((photo, index) => (<img key={index} src={photo} alt={`Defect ${index + 1}`} className="h-24 w-full rounded-lg object-cover"/>))}
                  </div>)}
              </div>
              ...
              <div>
                ...
                <textarea value={inspectionData.notes} onChange={e => setInspectionData(prev => ({
                ...prev,
                notes: e.target.value,
            }))} rows={3} placeholder="Add any additional notes..." className="w-full rounded-lg border border-gray-600 bg-gray-700 px-4 py-3 text-white placeholder-gray-400 focus:border-blue-500 focus:outline-none"/>
              </div>
              ...
              <div>
                ...
                <input type="text" value={inspectionData.inspector_name} onChange={e => setInspectionData(prev => ({
                ...prev,
                inspector_name: e.target.value,
            }))} placeholder="Enter your name..." className="w-full rounded-lg border border-gray-600 bg-gray-700 px-4 py-3 text-white placeholder-gray-400 focus:border-blue-500 focus:outline-none"/>
              </div>
            </div>
          </div>

          {/* Submit */}
          <button onClick={submitInspection} disabled={loading} className="flex w-full items-center justify-center space-x-2 rounded-lg bg-blue-600 px-6 py-4 font-medium text-white transition-colors hover:bg-blue-700 disabled:bg-gray-600">
            <lucide_react_1.Save className="h-5 w-5"/>

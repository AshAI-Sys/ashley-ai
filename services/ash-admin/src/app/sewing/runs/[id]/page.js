...
function SewingRunDetailsPage() {
                    ...
                    ? {
                        ...
                        status: action === "complete"
                            ...
                            : action === "start" || action === "resume"
                                ? "IN_PROGRESS"
                                : "IN_PROGRESS",
                        ended_at: action === "complete"
                            ? new Date().toISOString()
                            : prev.ended_at,
                        actual_minutes: action === "complete"
            ...
            if (response.ok) {
                ...
                setSewingRun(prev => prev
                    ? {
                        ...
                        efficiency_pct: Math.round(((parseInt(goodQty) * 1.5) / (prev.actual_minutes || 25)) *
                            100),
                    }
                    : null);
            }
    ...
    if (loading) {
        ...
        <div className="flex h-64 items-center justify-center">
          <div className="text-center">
            <lucide_react_1.RefreshCw className="mx-auto mb-4 h-8 w-8 animate-spin"/>
    ...
    if (!sewingRun) {
        return (<div className="container mx-auto py-6">
        <card_1.Card>
          <card_1.CardContent className="flex h-64 items-center justify-center">
            <div className="text-center">
              <lucide_react_1.XCircle className="mx-auto mb-2 h-8 w-8 text-muted-foreground"/>
            ...
            <card_1.CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <textarea_1.Textarea id="notes" value={progressNotes} onChange={e => setProgressNotes(e.target.value)} placeholder="Add any notes about progress..." className="h-20 resize-none"/>
                </div>
              </div>

              <button_1.Button onClick={handleUpdateProgress} disabled={!goodQty ||
            parseInt(goodQty) < 0 ||
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="h-3 w-full rounded-full bg-gray-200">
                    <div className="h-3 rounded-full bg-blue-600 transition-all duration-300" style={{ width: `${getCompletionPercentage()}%` }}/>
                  </div>
                </div>
            ...
            <card_1.CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <input_1.Input id="reject_qty" type="number" min="1" max={sewingRun.bundle.qty -
            sewingRun.qty_good -
            sewingRun.qty_reject} value={rejectQty} onChange={e => setRejectQty(e.target.value)}/>
                </div>
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              {qualityRejects.length > 0 ? (<div className="space-y-3">
                  {qualityRejects.map(reject => (<div key={reject.id} className="flex items-center justify-between rounded-lg border p-3">
                      <div>
                      ...
                      <div className="text-right">
                        ...
                        {reject.photo_url && (<p className="mt-1 text-xs text-muted-foreground">
                            Photo attached
                          </p>)}
                      </div>
                    </div>))}
                </div>) : (<p className="py-8 text-center text-muted-foreground">
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {timeEntries.map(entry => (<div key={entry.id} className="flex items-start gap-3">
                    <div className={`rounded-full p-2 ${entry.action === "START" || entry.action === "RESUME"
                ? "bg-green-100"
                : entry.action === "PAUSE"
                    ? "bg-yellow-100"
                    : entry.action === "COMPLETE"
                        ? "bg-blue-100"
                        : "bg-gray-100"}`}>
                      {entry.action === "START" || entry.action === "RESUME" ? (<lucide_react_1.Play className="h-4 w-4 text-green-600"/>) : entry.action === "PAUSE" ? (<lucide_react_1.Pause className="h-4 w-4 text-yellow-600"/>) : entry.action === "COMPLETE" ? (<lucide_react_1.Square className="h-4 w-4 text-blue-600"/>) : (<lucide_react_1.Clock className="h-4 w-4 text-gray-600"/>)}
                    </div>
                ...
                "Unknown Action"}
                      ...
                      {entry.notes && (<p className="mt-1 text-sm text-muted-foreground">
                          {entry.notes}
                        </p>)}
                    </div>
                  </div>))}
              ...
              <card_1.CardContent className="space-y-4">
                <div className="rounded-lg border bg-green-50 p-3">
                  <div className="flex items-start gap-2">
                    <lucide_react_1.CheckCircle className="mt-0.5 h-4 w-4 text-green-600"/>
                    <div>
                ...
                {sewingRun.qty_reject > 0 && (<div className="rounded-lg border bg-yellow-50 p-3">
                    <div className="flex items-start gap-2">
                      <lucide_react_1.AlertTriangle className="mt-0.5 h-4 w-4 text-yellow-600"/>
                      <div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-700">
                        ...
                        completion in{" "}
                        {Math.round((sewingRun.bundle.qty - sewingRun.qty_good) * 1.5)}{" "}
                        minutes.
                      </p>
                    </div>
                  </div>

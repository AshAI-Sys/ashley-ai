...
export default function SewingRunDetailsPage() {
  ...
  const handleRunAction = async (action: 'start' | 'pause' | 'resume' | 'complete') => {
    try {
        ...
        <Card>
          <CardContent className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <Link href="/sewing">
                <Button className="mt-2" variant="outline">Back to Sewing</Button>
              </Link>
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button 
                ...
                disabled={!goodQty || parseInt(goodQty) < 0 || parseInt(goodQty) > sewingRun.bundle.qty}
              >
        ...
        <TabsContent value="quality" className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    min="1"
                    max={sewingRun.bundle.qty - sewingRun.qty_good - sewingRun.qty_reject}
                    value={rejectQty}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>History of rejected pieces for this run</CardDescription>
            </CardHeader>
            <CardContent>
              {qualityRejects.length > 0 ? (
                <div className="space-y-3">
                  {qualityRejects.map((reject) => (
                    <div key={reject.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div>
        ...
        <TabsContent value="timeline" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complete timeline of this sewing run</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {timeEntries.map((entry) => (
                  <div key={entry.id} className="flex items-start gap-3">
                    <div className={`p-2 rounded-full ${
                      entry.action === 'START' || entry.action === 'RESUME' ? 'bg-green-100' :
                      entry.action === 'PAUSE' ? 'bg-yellow-100' :
                      entry.action === 'COMPLETE' ? 'bg-blue-100' : 'bg-gray-100'
                    }`}>
                      {entry.action === 'START' || entry.action === 'RESUME' ? <Play className="w-4 h-4 text-green-600" /> :
                       entry.action === 'PAUSE' ? <Pause className="w-4 h-4 text-yellow-600" /> :
                       entry.action === 'COMPLETE' ? <Square className="w-4 h-4 text-blue-600" /> :

...
export default function NewSewingRunPage() {
  ...
  const [step, setStep] = useState<
    "scan" | "operation" | "operator" | "confirm"
  >("scan");
  ...
  const [selectedOperator, setSelectedOperator] = useState<Employee | null>(
    null
  );
  ...
  const handleQRScan = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/bundles/by-qr?qr_code=${encodeURIComponent(qrInput)}`
      );
  ...
  const fetchAvailableOperations = async (bundle: Bundle) => {
    try {
      ...
      const response = await fetch(
        `/api/sewing/operations?product_type=${garmentType}`
      );
    ...
    <div className="container mx-auto max-w-4xl space-y-6 py-6">
      ...
      <Card>
        <CardContent className="py-4">
          <div className="flex items-center gap-4">
            <div
              className={`flex items-center gap-2 ${step === "scan" ? "text-blue-600" : "text-green-600"}`}
            >
              <div
            ...
            <div
              className={`h-0.5 flex-1 ${step === "operation" || step === "operator" || step === "confirm" ? "bg-green-600" : "bg-gray-300"}`}
            />
            ...
            <div
              className={`h-0.5 flex-1 ${step === "operator" || step === "confirm" ? "bg-green-600" : "bg-gray-300"}`}
            />
            ...
            <div
              className={`h-0.5 flex-1 ${step === "confirm" ? "bg-green-600" : "bg-gray-300"}`}
            />

            <div
              className={`flex items-center gap-2 ${step === "confirm" ? "text-blue-600" : "text-muted-foreground"}`}
            >
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full ${
                  step === "confirm"
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div className="flex items-center justify-center">
                <div className="rounded-lg border-2 border-dashed p-8 text-center">
                  {isScanning ? (
                    <div className="space-y-4">
                      <Scan className="mx-auto h-12 w-12 animate-pulse text-blue-600" />
            ...
            <CardContent>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                  ...
                  <Badge
                    className={`${scannedBundle.status === "CREATED" ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"} mt-1`}
                  >
              ...
              <CardContent>
                <div className="space-y-2">
                  {bundleProgress.map((progress, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-3 rounded-lg bg-green-50 p-2"
                    >
                ...
                {availableOperations.map(operation => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        isAvailable && setSelectedOperation(operation)
                      }
                    >
                      <div className="flex items-center justify-between">
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <p>
                        ...
                        {(
                          selectedOperation.standard_minutes * scannedBundle.qty
                        ).toFixed(1)}{" "}
                        minutes
                      </p>
                      <p>

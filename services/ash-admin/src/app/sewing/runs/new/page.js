...
function NewSewingRunPage() {
    ...
    return (<div className="container mx-auto py-6 max-w-4xl space-y-6">
      {/* Header */}
        ...
        <card_1.CardContent className="py-4">
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 ${step === 'scan' ? 'text-blue-600' : step !== 'scan' ? 'text-green-600' : 'text-muted-foreground'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'scan' ? 'bg-blue-100 border-2 border-blue-600' :
            step !== 'scan' ? 'bg-green-100 border-2 border-green-600' :
                'bg-gray-100 border-2 border-gray-300'}`}>
                {step !== 'scan' ? <lucide_react_1.CheckCircle className="w-4 h-4"/> : <lucide_react_1.QrCode className="w-4 h-4"/>}
              </div>
            ...
            <div className={`flex items-center gap-2 ${step === 'operation' ? 'text-blue-600' :
            step === 'operator' || step === 'confirm' ? 'text-green-600' :
                'text-muted-foreground'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'operation' ? 'bg-blue-100 border-2 border-blue-600' :
            step === 'operator' || step === 'confirm' ? 'bg-green-100 border-2 border-green-600' :
                'bg-gray-100 border-2 border-gray-300'}`}>
                {step === 'operator' || step === 'confirm' ? <lucide_react_1.CheckCircle className="w-4 h-4"/> : <lucide_react_1.Settings className="w-4 h-4"/>}
              </div>
            ...
            <div className={`flex items-center gap-2 ${step === 'operator' ? 'text-blue-600' :
            step === 'confirm' ? 'text-green-600' :
                'text-muted-foreground'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'operator' ? 'bg-blue-100 border-2 border-blue-600' :
            step === 'confirm' ? 'bg-green-100 border-2 border-green-600' :
                'bg-gray-100 border-2 border-gray-300'}`}>
                {step === 'confirm' ? <lucide_react_1.CheckCircle className="w-4 h-4"/> : <lucide_react_1.User className="w-4 h-4"/>}
              </div>
              ...
              <div className="flex items-center justify-center">
                <div className="text-center p-8 border-2 border-dashed rounded-lg">
                  {isScanning ? (<div className="space-y-4">
                      <lucide_react_1.Scan className="w-12 h-12 mx-auto text-blue-600 animate-pulse"/>
                      <p className="text-sm text-muted-foreground">
                        Scanning for QR code...
                      </p>
                    </div>) : (<div className="space-y-4">
                      <lucide_react_1.QrCode className="w-12 h-12 mx-auto text-muted-foreground"/>
                      <p className="text-sm text-muted-foreground">
      ...
      {step === 'operation' && scannedBundle && (<div className="space-y-6">
          ...
          {bundleProgress.length > 0 && (<card_1.Card>
              <card_1.CardHeader>
                ...
                <card_1.CardDescription>Operations completed on this bundle</card_1.CardDescription>
              </card_1.CardHeader>
              <card_1.CardContent>
                <div className="space-y-2">
                  {bundleProgress.map((progress, index) => (<div key={index} className="flex items-center gap-3 p-2 rounded-lg bg-green-50">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                ...
                return (<div key={operation.id} className={`p-4 border rounded-lg cursor-pointer transition-all ${selectedOperation?.id === operation.id
                        ? 'border-blue-600 bg-blue-50'
                        : isAvailable
                            ? 'border-gray-200 hover:border-gray-300'
                            : 'border-gray-100 bg-gray-50 cursor-not-allowed'}`} onClick={() => isAvailable && setSelectedOperation(operation)}>
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            {!isAvailable && (<badge_1.Badge variant="outline" className="text-orange-600 border-orange-600">
                                Blocked
                              </badge_1.Badge>)}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            ...
                            <div className="flex items-center gap-2">
                              <lucide_react_1.DollarSign className="w-4 h-4 text-muted-foreground"/>
                              <span>₱{operation.piece_rate?.toFixed(2) || 'N/A'} per piece</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <lucide_react_1.GitBranch className="w-4 h-4 text-muted-foreground"/>
                              <span>
      ...
      {step === 'operator' && (<card_1.Card>
          ...
          <card_1.CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {operators.map((operator) => (<div key={operator.id} className={`p-4 border rounded-lg cursor-pointer transition-all ${selectedOperator?.id === operator.id
                    ? 'border-blue-600 bg-blue-50'
      ...
      {step === 'confirm' && scannedBundle && selectedOperation && selectedOperator && (<card_1.Card>
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center gap-2">

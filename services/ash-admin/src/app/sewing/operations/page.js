...
const productTypes = [
    {
        value: 'T-Shirt',
        label: 'T-Shirt',
        operations: ['Cut neck', 'Attach sleeves', 'Side seams', 'Hem bottom', 'Neck binding']
    },
    {
        value: 'Hoodie',
        label: 'Hoodie',
        operations: ['Join shoulders', 'Attach collar', 'Set sleeves', 'Side seams', 'Hem bottom', 'Pocket attachment', 'Hood assembly']
    },
    {
        value: 'Polo',
        label: 'Polo Shirt',
        operations: ['Join shoulders', 'Attach collar', 'Set sleeves', 'Side seams', 'Hem bottom', 'Placket creation', 'Button attachment']
    },
    {
        value: 'Jersey',
        label: 'Sports Jersey',
        operations: ['Join shoulders', 'Set sleeves', 'Side seams', 'Hem bottom', 'Number application', 'Name application']
    }
];
    ...
    const handleCreateOperation = async () => {
        try {
            const operationData = {
                ...
                piece_rate: formData.piece_rate ? parseFloat(formData.piece_rate) : null
            };
            ...
            if (response.ok) {
                ...
                const newOperation = {
                    ...
                    name: formData.name,
                    standard_minutes: parseFloat(formData.standard_minutes),
                    piece_rate: formData.piece_rate ? parseFloat(formData.piece_rate) : undefined,
    ...
    return (<div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage operation library with SMV and piece rates</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <card_1.Card>
          <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <card_1.CardTitle className="text-sm font-medium">Total Operations</card_1.CardTitle>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            <div className="text-2xl font-bold">{Object.keys(groupedOperations).length}</div>
          </card_1.CardContent>
        </card_1.Card>
        ...
        <card_1.Card>
          ...
          <card_1.CardContent>
            ...
            ? (operations.filter(op => op.piece_rate).reduce((sum, op) => sum + (op.piece_rate || 0), 0)
          ...
          <div className="flex gap-4 items-center">
            ...
            <select_1.Select value={productTypeFilter} onValueChange={setProductTypeFilter}>
              ...
              <select_1.SelectContent>
                <select_1.SelectItem value="">All product types</select_1.SelectItem>
        ...
        {Object.entries(groupedOperations).map(([productType, typeOperations]) => (<card_1.Card key={productType}>
            <card_1.CardHeader>
              <card_1.CardTitle className="flex items-center gap-2">
                ...
                {productType}
                <badge_1.Badge variant="outline">{typeOperations.length} operations</badge_1.Badge>
              </card_1.CardTitle>
              <card_1.CardDescription>
                Standard manufacturing operations for {productType.toLowerCase()} production
              </card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-3">
                {typeOperations.map((operation) => (<div key={operation.id} className="flex items-center justify-between p-4 border rounded-lg hover:shadow-sm transition-shadow">
                    ...
                    : `${operation.depends_on.length} ${operation.depends_on.length === 1 ? 'dependency' : 'dependencies'}`}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-muted-foreground">
                        <div className="flex items-center gap-2">
                          ...
                          <span>{operation.standard_minutes} minutes SMV</span>
                        </div>
                        <div className="flex items-center gap-2">
      ...
      <dialog_1.Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <dialog_1.DialogContent className="sm:max-w-[600px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input_1.Input id="standard_minutes" type="number" step="0.1" value={formData.standard_minutes} onChange={(e) => setFormData({ ...formData, standard_minutes: e.target.value })} placeholder="1.5"/>
              </div>
            ...
            ...formData,
            depends_on: e.target.value.split(',').map(dep => dep.trim()).filter(dep => dep)
          ...
          <dialog_1.DialogFooter>
            ...
            <button_1.Button onClick={handleCreateOperation} disabled={!formData.product_type || !formData.name || !formData.standard_minutes}>
              Create Operation
            </button_1.Button>
          </dialog_1.DialogFooter>
      ...
      <dialog_1.Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <dialog_1.DialogContent className="sm:max-w-[600px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <label_1.Label htmlFor="edit_standard_minutes">Standard Minutes (SMV)</label_1.Label>
                <input_1.Input id="edit_standard_minutes" type="number" step="0.1" value={formData.standard_minutes} onChange={(e) => setFormData({ ...formData, standard_minutes: e.target.value })} placeholder="1.5"/>
              </div>
          ...
          <dialog_1.DialogFooter>
            ...
            <button_1.Button onClick={handleEditOperation} disabled={!formData.product_type || !formData.name || !formData.standard_minutes}>
              Update Operation
            </button_1.Button>
          </dialog_1.DialogFooter>
        </dialog_1.DialogContent>

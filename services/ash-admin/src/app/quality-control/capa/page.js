...
exports.default = CAPAManagementPage;
...
function CAPAManagementPage() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (data.capa_tasks) {
                setTasks(data.capa_tasks);
                // Calculate stats
                const total = data.capa_tasks.length;
                const open = data.capa_tasks.filter((t) => t.status === "OPEN").length;
                const inProgress = data.capa_tasks.filter((t) => t.status === "IN_PROGRESS").length;
                const now = new Date();
                const overdue = data.capa_tasks.filter((t) => t.due_date && new Date(t.due_date) < now && t.status !== "CLOSED").length;
                const effective = data.capa_tasks.filter((t) => t.effectiveness === "EFFECTIVE").length;
                const totalClosed = data.capa_tasks.filter((t) => t.status === "CLOSED").length;
                setStats({
                    total_tasks: total,
                    open,
                    in_progress: inProgress,
                    overdue,
                    effectiveness_rate: totalClosed > 0 ? (effective / totalClosed) * 100 : 0,
                });
            }
        }
        catch (error) {
    ...
    const filteredTasks = tasks.filter(task => searchTerm === "" ||
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            ...
            <lucide_react_1.Users className="mr-1 h-3 w-3"/>
            In Progress
          </span>);
    ...
    if (loading) {
        ...
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading CAPA tasks...</p>
        </div>
      </div>);
    }
    return (<dashboard_layout_1.default>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 md:flex md:items-center md:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Corrective and Preventive Action tasks for quality improvement
            </p>
          </div>
          <div className="mt-4 flex space-x-3 md:ml-4 md:mt-0">
            <button onClick={() => (window.location.href = "/quality-control")} className="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
        ...
        {/* Stats Cards */}
        {stats && (<div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            <div className="overflow-hidden rounded-lg bg-white shadow">
              <div className="p-5">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <lucide_react_1.FileText className="h-6 w-6 text-gray-500"/>
                  </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      ...
                      <dd className="text-lg font-medium text-gray-900">
                        {stats.effectiveness_rate.toFixed(1)}%
                      </dd>
                    </dl>
                  </div>
                </div>
            ...
            <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:space-x-4 sm:space-y-0">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search CAPA tasks..." className="rounded-md border border-gray-300 py-2 pl-10 pr-4 focus:border-blue-500 focus:ring-blue-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}/>
                </div>

                <select className="rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500" value={selectedStatus} onChange={e => setSelectedStatus(e.target.value)}>
                ...
                <select className="rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500" value={selectedPriority} onChange={e => setSelectedPriority(e.target.value)}>
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {filteredTasks.map(task => (<tr key={task.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      ...
                      {task.order && (<div className="text-sm text-gray-500">
          ...
          {filteredTasks.length === 0 && (<div className="py-12 text-center">
              <lucide_react_1.FileText className="mx-auto h-12 w-12 text-gray-500"/>
              <h3 className="mt-2 text-sm font-medium text-gray-900">
                No CAPA tasks found
              </h3>
              <p className="mt-1 text-sm text-gray-500">
                CAPA tasks will be created automatically when QC inspections
                fail.
              </p>
            </div>)}
        </div>

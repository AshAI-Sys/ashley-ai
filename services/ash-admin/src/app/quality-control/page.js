...
function QualityControlPage() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (data.inspections) {
                ...
                const passed = data.inspections.filter((i) => i.status === 'PASSED').length;
                const failed = data.inspections.filter((i) => i.status === 'FAILED').length;
                const pending = data.inspections.filter((i) => i.status === 'OPEN').length;
                const totalDefects = data.inspections.reduce((sum, i) => sum + i.critical_found + i.major_found + i.minor_found, 0);
                const totalSamples = data.inspections.reduce((sum, i) => sum + i.sample_size, 0);
                setStats({
    ...
    const getStageBadge = (stage) => {
        ...
        return (<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${colors[stage] || 'bg-gray-100 text-gray-800'}`}>
        {stage}
      </span>);
    };
    if (loading) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="md:flex md:items-center md:justify-between mb-8">
          ...
          <div className="mt-4 flex md:mt-0 md:ml-4 space-x-3">
            <button onClick={() => window.location.href = '/quality-control/analytics'} className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
              <lucide_react_1.BarChart3 className="w-4 h-4 mr-2"/>
              Analytics
            </button>
            <button onClick={() => window.location.href = '/quality-control/new'} className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">
              ...
              <div className="p-5">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <lucide_react_1.ClipboardCheck className="h-6 w-6 text-gray-400"/>
                  </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
              <div className="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search inspections..." className="pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                </div>
                
                <select className="border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500" value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)}>
                  <option value="today">Today</option>
                  <option value="week">Past Week</option>
                  <option value="month">Past Month</option>
                </select>

                <select className="border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500" value={filterType} onChange={(e) => setFilterType(e.target.value)}>
                ...
                <select className="border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500" value={filterResult} onChange={(e) => setFilterResult(e.target.value)}>
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
                ...
                {filteredInspections.map((inspection) => (<tr key={inspection.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        ...
                        {inspection._count.capa_tasks > 0 && (<button className="text-orange-600 hover:text-orange-900">
          ...
          {filteredInspections.length === 0 && (<div className="text-center py-12">
              <lucide_react_1.ClipboardCheck className="mx-auto h-12 w-12 text-gray-400"/>

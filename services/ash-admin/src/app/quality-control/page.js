...
function QualityControlPage() {
    const [selectedPeriod, setSelectedPeriod] = (0, react_1.useState)("today");
    const [filterType, setFilterType] = (0, react_1.useState)("all");
    const [filterResult, setFilterResult] = (0, react_1.useState)("all");
    ...
    const { data: inspections = [], isLoading: inspectionsLoading, error: inspectionsError, refetch: refetchInspections, isFetching: inspectionsFetching, } = (0, react_query_1.useQuery)({
    ...
    const _isLoading = inspectionsLoading;
    const isFetching = inspectionsFetching;
    const filteredInspections = inspections.filter(inspection => searchTerm === "" ||
    ...
    const getStatusBadge = (status, result) => {
        switch (status) {
            ...
            <lucide_react_1.XCircle className="mr-1 h-3 w-3"/>
            Failed
          </span>);
            case "CLOSED":
                return result ? (getStatusBadge(result)) : (<span className="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-800">
            Closed
          </span>);
            case "OPEN":
    ...
    const getStageBadge = (stage) => {
        ...
        return (<span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${colors[stage] || "bg-gray-100 text-gray-800"}`}>
        {stage}
      </span>);
    };
    ...
    // Skeleton Loaders
    const StatCardSkeleton = () => (<div className="overflow-hidden rounded-lg bg-white shadow">
      <div className="p-5">
    ...
    const InspectionRowSkeleton = () => (<tr>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 md:flex md:items-center md:justify-between">
          ...
          <div className="mt-4 flex space-x-3 md:ml-4 md:mt-0">
            <button_1.Button variant="outline" onClick={handleRefresh} disabled={isFetching}>
              <lucide_react_1.RefreshCw className={`mr-2 h-4 w-4 ${isFetching ? "animate-spin" : ""}`}/>
              Refresh
            </button_1.Button>
            <button onClick={() => (window.location.href = "/quality-control/analytics")} className="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
              <lucide_react_1.BarChart3 className="mr-2 h-4 w-4"/>
              Analytics
            </button>
            <button onClick={() => (window.location.href = "/quality-control/new")} className="inline-flex items-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-700">
        ...
        {/* Error Alert */}
        {inspectionsError && (<ErrorAlert error={inspectionsError} onRetry={refetchInspections}/>)}

        {/* Stats Cards */}
        {inspectionsLoading ? (<div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {[...Array(4)].map((_, i) => (<StatCardSkeleton key={i}/>))}
          </div>) : stats ? (<div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            <div className="overflow-hidden rounded-lg bg-white shadow">
              <div className="p-5">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <lucide_react_1.ClipboardCheck className="h-6 w-6 text-gray-500"/>
                  </div>
                  <div className="ml-5 w-0 flex-1">
                    <dl>
            ...
            <div className="overflow-hidden rounded-lg bg-white shadow">
              <div className="p-5">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <lucide_react_1.TrendingUp className="h-6 w-6 text-blue-400"/>
                  </div>
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      <dt className="truncate text-sm font-medium text-gray-500">
            ...
            <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:space-x-4 sm:space-y-0">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search inspections..." className="rounded-md border border-gray-300 py-2 pl-10 pr-4 focus:border-blue-500 focus:ring-blue-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}/>
                </div>

                <select className="rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500" value={selectedPeriod} onChange={e => setSelectedPeriod(e.target.value)}>
                  <option value="today">Today</option>
                  <option value="week">Past Week</option>
                  <option value="month">Past Month</option>
                </select>

                <select className="rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500" value={filterType} onChange={e => setFilterType(e.target.value)}>
                ...
                <select className="rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500" value={filterResult} onChange={e => setFilterResult(e.target.value)}>
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow">
          ...
          <div className="overflow-x-auto">
            ...
            : filteredInspections.map(inspection => (<tr key={inspection.id} className="hover:bg-gray-50">
                        <td className="whitespace-nowrap px-6 py-4">
                          <div>
                            <div className="text-sm font-medium text-gray-900">
                              {inspection.order?.order_number || "N/A"}
                            </div>
                            {inspection.bundle && (<div className="text-sm text-gray-500">
                        ...
                        <td className="whitespace-nowrap px-6 py-4 text-sm font-medium">
                          <div className="flex items-center space-x-2">
                            ...
                            {(inspection._count?.capa_tasks ?? 0) > 0 && (<button className="text-orange-600 hover:text-orange-900">
          ...
          {!inspectionsLoading && filteredInspections.length === 0 && (<div className="py-12 text-center">
              <lucide_react_1.ClipboardCheck className="mx-auto h-12 w-12 text-gray-500"/>

  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      const metricsResponse = await fetch(
        `/api/quality-control/analytics/metrics?period=${selectedPeriod}`
      );
      ...
      const spcResponse = await fetch(
        `/api/quality-control/analytics/spc?metric=${selectedMetric}&period=${selectedPeriod}`
      );
      const spcData = await spcResponse.json();
      setSpcData(spcData);

      // Load Pareto data
      const paretoResponse = await fetch(
        `/api/quality-control/analytics/pareto?period=${selectedPeriod}`
      );
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            {trend && (
              <div
                className={`mt-2 flex items-center ${trend === "up" ? "text-green-600" : trend === "down" ? "text-red-600" : "text-gray-600"}`}
              >
                {trend === "up" ? (
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          <Select value={selectedMetric} onValueChange={setSelectedMetric}>
            ...
            <SelectContent>
              ...
              <SelectItem value="first_pass_yield">
                First Pass Yield (%)
              </SelectItem>

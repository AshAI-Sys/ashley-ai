...
function NewInspectionPage() {
    ...
    const addDefectToSample = (sampleIndex, defect) => {
    ...
    const evaluateInspectionResult = (samplesData) => {
        if (!inspectionResult)
            return;
        const totalDefects = samplesData.reduce((sum, sample) => sum + sample.defects.reduce((defectSum, defect) => defectSum + defect.quantity, 0), 0);
    ...
    return (<div className="min-h-screen bg-gray-50">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <select_1.Select value={formData.inspection_type} onValueChange={(value) => setFormData({ ...formData, inspection_type: value })}>
                    ...
                    <select_1.SelectContent>
                      <select_1.SelectItem value="INLINE_PRINTING">Inline Printing</select_1.SelectItem>
                      <select_1.SelectItem value="INLINE_SEWING">Inline Sewing</select_1.SelectItem>
                ...
                <div>
                  ...
                  <input_1.Input id="lot_size" type="number" value={formData.lot_size || ''} onChange={(e) => setFormData({ ...formData, lot_size: parseInt(e.target.value) || 0 })}/>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label_1.Label htmlFor="aql_major">AQL Major</label_1.Label>
                    <input_1.Input id="aql_major" type="number" step="0.1" value={formData.aql_major} onChange={(e) => setFormData({ ...formData, aql_major: parseFloat(e.target.value) })}/>
                  </div>
                  <div>
                    <label_1.Label htmlFor="aql_minor">AQL Minor</label_1.Label>
                    <input_1.Input id="aql_minor" type="number" step="0.1" value={formData.aql_minor} onChange={(e) => setFormData({ ...formData, aql_minor: parseFloat(e.target.value) })}/>
                  </div>
                </div>

                {inspectionResult && (<div className="bg-blue-50 p-4 rounded-lg">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                {samples.length > 0 ? (<div className="space-y-4">
                    {samples.map((sample, index) => (<div key={sample.sample_no} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="font-medium">Sample #{sample.sample_no}</h4>
                          <div className="flex items-center space-x-2">
                            {sample.pass_fail ? (<lucide_react_1.CheckCircle className="w-5 h-5 text-green-500"/>) : (<lucide_react_1.XCircle className="w-5 h-5 text-red-500"/>)}
                        ...
                        {sample.defects.length > 0 && (<div className="space-y-2">
                            {sample.defects.map((defect, defectIndex) => (<div key={defectIndex} className="flex items-center justify-between bg-red-50 p-2 rounded">
                    ...
                    <lucide_react_1.AlertTriangle className="w-12 h-12 mx-auto mb-4 text-gray-400"/>
      ...
      {currentSample && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <card_1.Card className="w-full max-w-md">
            <card_1.CardHeader>
              <card_1.CardTitle>Add Defect - Sample #{currentSample.sample_no}</card_1.CardTitle>
            </card_1.CardHeader>
            <card_1.CardContent>
              ...
              <div className="flex justify-end space-x-2">
                ...
                <button_1.Button>Add Defect</button_1.Button>
              </div>
            </card_1.CardContent>
          </card_1.Card>
        </div>)}

...
function ClientBrandsPage() {
    ...
    const validateForm = () => {
        ...
        if (!formData.code.trim()) {
            ...
            newErrors.code = 'Brand code must contain only uppercase letters, numbers, and underscores';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.success) {
                react_hot_toast_1.toast.success(editingBrand ? 'Brand updated successfully' : 'Brand created successfully');
                resetForm();
                fetchBrands();
            }
            else {
    ...
    return (<div className="container mx-auto py-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Manage brands associated with this client</p>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">Used for order numbering. Only uppercase letters, numbers, and underscores.</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <label_1.Label className="text-base font-semibold">Brand Settings</label_1.Label>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label_1.Label htmlFor="color_primary">Primary Color</label_1.Label>
                    <div className="flex gap-2">
                      <input_1.Input id="color_primary" type="color" value={formData.settings.color_primary} onChange={(e) => handleInputChange('settings.color_primary', e.target.value)} className="w-16 h-10 p-1"/>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input_1.Input id="color_secondary" type="color" value={formData.settings.color_secondary} onChange={(e) => handleInputChange('settings.color_secondary', e.target.value)} className="w-16 h-10 p-1"/>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  <label_1.Label className="text-base font-semibold">Brand Status</label_1.Label>
                  <p className="text-sm text-muted-foreground">Active brands can be used for orders</p>
                </div>
                <button_1.Button type="button" variant="ghost" size="sm" onClick={() => handleInputChange('is_active', !formData.is_active)} className="flex items-center gap-2">
                  {formData.is_active ? (<>
                      <lucide_react_1.ToggleRight className="w-5 h-5 text-green-600"/>
                      <span className="text-green-600 font-medium">Active</span>
                    </>) : (<>
                      <lucide_react_1.ToggleLeft className="w-5 h-5 text-gray-400"/>
      ...
      {loading ? (<div className="flex justify-center py-12">
          ...
          {brands.map((brand) => {
                ...
                return (<card_1.Card key={brand.id} className="hover:shadow-md transition-shadow">
                <card_1.CardContent className="py-4">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        {brand.logo_url && (<img src={brand.logo_url} alt={`${brand.name} logo`} className="w-8 h-8 object-contain rounded" onError={(e) => {
                        ...
                        <badge_1.Badge className={brand.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                          {brand.is_active ? 'Active' : 'Inactive'}
                        </badge_1.Badge>
                      </div>
                      
                      {settings && (<div className="flex items-center gap-4 mb-3">
                          {settings.color_primary && (<div className="flex items-center gap-2">
                              <div className="w-4 h-4 rounded border" style={{ backgroundColor: settings.color_primary }}></div>
                              <span className="text-sm text-muted-foreground">{settings.color_primary}</span>
                            </div>)}
                          {settings.color_secondary && (<div className="flex items-center gap-2">
                              <div className="w-4 h-4 rounded border" style={{ backgroundColor: settings.color_secondary }}></div>
                              <span className="text-sm text-muted-foreground">{settings.color_secondary}</span>
                            </div>)}
                        </div>)}

                      {settings?.notes && (<p className="text-sm text-muted-foreground mb-3">{settings.notes}</p>)}
          ...
          {brands.length === 0 && (<card_1.Card>
              <card_1.CardContent className="py-12 text-center">
                ...
                <p className="text-muted-foreground mb-4">No brands found for this client</p>

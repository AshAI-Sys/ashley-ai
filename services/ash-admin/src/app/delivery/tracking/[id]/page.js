...
function ShipmentTrackingPage({ params }) {
    ...
    const getEventIcon = (eventType) => {
        switch (eventType) {
            case 'CREATED':
                return <lucide_react_1.Package className="w-4 h-4"/>;
            case 'DISPATCHED':
                return <lucide_react_1.Truck className="w-4 h-4"/>;
            case 'IN_TRANSIT':
                return <lucide_react_1.Navigation className="w-4 h-4"/>;
            case 'OUT_FOR_DELIVERY':
    ...
    if (!shipment) {
        return (<div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <lucide_react_1.AlertCircle className="w-12 h-12 mx-auto text-red-500 mb-4"/>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center space-x-3">
            <button_1.Button variant="outline" onClick={refreshTrackingData} disabled={refreshing} className="flex items-center">
              <lucide_react_1.RefreshCw className={`w-4 h-4 mr-2 ${refreshing ? 'animate-spin' : ''}`}/>
              Refresh
            </button_1.Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            minute: '2-digit'
                  ...
                  {shipment.cod_amount && (<div className="bg-green-50 p-3 rounded-lg">
              ...
              {shipment.driver && (<card_1.Card>
                  <card_1.CardHeader>
                    <card_1.CardTitle className="flex items-center">
                      <lucide_react_1.Truck className="w-5 h-5 mr-2"/>
                      Driver Information
                    </card_1.CardTitle>
                  </card_1.CardHeader>
                  <card_1.CardContent className="space-y-4">
                    ...
                    {shipment.vehicle && (<div className="bg-blue-50 p-3 rounded-lg">
              ...
              <card_1.CardHeader>
                <card_1.CardTitle className="flex items-center justify-between">
                  <span className="flex items-center">
                    <lucide_react_1.Navigation className="w-5 h-5 mr-2"/>
                    Tracking Timeline
                  </span>
                  {shipment.current_location && (<div className="text-sm text-gray-600 flex items-center">
              ...
              <card_1.CardContent>
                <div className="space-y-6">
                  {shipment.tracking_events.map((event, index) => (<div key={event.id} className="flex items-start space-x-4">
                      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${index === 0 ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {event.notes && (<div className="text-sm text-gray-500 mt-2 flex items-start">
                            <lucide_react_1.MessageSquare className="w-4 h-4 mr-1 mt-0.5 flex-shrink-0"/>
                            <span>{event.notes}</span>
                          </div>)}

                        {event.photo_url && (<div className="mt-2">
                            <img src={event.photo_url} alt="Tracking event" className="w-24 h-24 object-cover rounded-lg border"/>
                          </div>)}

                        {event.gps_coordinates && (<div className="text-xs text-gray-400 mt-1">
                            GPS: {event.gps_coordinates.lat.toFixed(6)}, {event.gps_coordinates.lng.toFixed(6)}
                          </div>)}
                      </div>

                      {index < shipment.tracking_events.length - 1 && (<div className="absolute left-4 top-8 w-0.5 h-16 bg-gray-300"></div>)}
                    </div>))}

                  {shipment.status !== 'DELIVERED' && shipment.status !== 'FAILED' && (<div className="flex items-center space-x-4 text-gray-400">
                ...
                <div className="mt-8">
                  <div className="h-64 bg-gray-100 rounded-lg flex items-center justify-center">
                    <div className="text-center text-gray-500">
                      <lucide_react_1.MapPin className="w-12 h-12 mx-auto mb-2"/>
                      <p>Live Map View</p>

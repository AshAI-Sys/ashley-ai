    ...
    try {
      ...
      await prisma.reportExecution.create({
        data: {
          ...
          row_count: rowCount,
          status: 'SUCCESS',
          filters_applied: JSON.stringify([...reportFilters, ...(additionalFilters || [])]),
        },
      });
...
async function executeOrdersQuery(workspaceId: string, config: any, filters: any[], sortOrder: any) {
  ...
  const orders = await prisma.order.findMany({
    ...
    orderBy: sortOrder ? { [sortOrder.column]: sortOrder.direction } : { created_at: 'desc' },
...
async function executeProductionQuery(workspaceId: string, config: any, filters: any[], sortOrder: any) {
  const where: any = { workspace_id: workspaceId };
...
async function executeFinanceQuery(workspaceId: string, config: any, filters: any[], sortOrder: any) {
  const where: any = { workspace_id: workspaceId };
...
async function executeHRQuery(workspaceId: string, config: any, filters: any[], sortOrder: any) {
  const where: any = { workspace_id: workspaceId };
...
async function executeInventoryQuery(workspaceId: string, config: any, filters: any[], sortOrder: any) {
  const where: any = { workspace_id: workspaceId };
...
async function executeQualityQuery(workspaceId: string, config: any, filters: any[], sortOrder: any) {
  const where: any = { workspace_id: workspaceId };

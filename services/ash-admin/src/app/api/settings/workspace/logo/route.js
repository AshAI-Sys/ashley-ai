...
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, authUser) => {
    try {
        ...
        return server_1.NextResponse.json({ success: true, logo_url });
    }
    catch (error) {
        console.error("Error uploading logo:", error);
        return server_1.NextResponse.json({ error: "Failed to upload logo" }, { status: 500 });
    }
    export const DELETE = (0, auth_middleware_1.requireAuth)(async (request, authUser) => {
        try {
            ...
            return server_1.NextResponse.json({ error: "Failed to remove logo" }, { status: 500 });
        }
    });
});

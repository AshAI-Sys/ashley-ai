...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!operation || !amount) {
      return NextResponse.json({ error: 'operation and amount are required' }, { status: 400 })
    }
    ...
    switch (operation.toLowerCase()) {
      ...
      case 'withholding':
        if (!atc_code) {
          return NextResponse.json({ error: 'atc_code is required for withholding calculation' }, { status: 400 })
        }
      ...
      case 'validate_tin':
        ...
        if (!tin) {
          return NextResponse.json({ error: 'tin is required for validation' }, { status: 400 })
        }
        result = {
          tin,

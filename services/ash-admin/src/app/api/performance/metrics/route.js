...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        const queryMetrics = (0, query_cache_1.getQueryMetrics)();
        // Get Redis status
        const redisAvailable = await (0, redis_1.checkRedisAvailable)();
        const redisInfo = redisAvailable ? await (0, redis_1.getRedisInfo)() : null;
    ...
    return server_1.NextResponse.json({
        ...
        data: {
            ...
            queries: {
                ...
                avgCachedDuration: Math.round(queryMetrics.avgCachedDuration * 10) / 10,
                avgUncachedDuration: Math.round(queryMetrics.avgUncachedDuration * 10) / 10,
            ...
            // Recommendations
            recommendations: getRecommendations(queryMetrics, redisAvailable, redisCacheHitRate),
        },
    });
});
try { }
    ...
    return server_1.NextResponse.json({ success: false, error: "Failed to fetch performance metrics" }, { status: 500 });
}
/**
 ...
 */
function getSpeedGrade(avgDuration) {
    if (avgDuration <= 50)
        return "A+";
    if (avgDuration <= 100)
        return "A";
 ...
 */
function getRecommendations(queryMetrics, redisAvailable, redisCacheHitRate) {
    const recommendations = [];
    // Query cache recommendations
    if (queryMetrics.cacheHitRate < 70) {
        recommendations.push("Low cache hit rate detected. Consider increasing cache TTL for frequently accessed data.");
    }
}
if (queryMetrics.avgUncachedDuration > 500) {
    recommendations.push("Slow uncached queries detected. Review database indexes and query optimization.");
}
if (queryMetrics.totalQueries < 100) {
    recommendations.push("Limited data - continue monitoring for more accurate metrics.");
}
// Redis recommendations
if (!redisAvailable) {
    recommendations.push("Redis is not available. Deploy Redis for production to improve performance.");
}
else if (redisCacheHitRate < 80) {
    recommendations.push("Redis cache hit rate is below optimal. Review cache invalidation strategy.");
}
// Performance recommendations
if (queryMetrics.avgDuration > 200) {
    recommendations.push("Average query duration is high. Consider implementing more aggressive caching.");
}

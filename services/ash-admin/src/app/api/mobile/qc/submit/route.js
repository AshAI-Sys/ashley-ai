...
exports.POST = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        const { bundle_id, order_id, sample_size, inspected, passed, failed, defects, photos, notes, inspector_name, } = body;
        if (!bundle_id || !order_id) {
            return server_1.NextResponse.json({ success: false, error: "Bundle ID and Order ID are required" }, { status: 400 });
        }
        ...
        const inspection = await prisma.qCInspection.create({
            ...
            if(defects) { }
        } && defects.length > 0), { await, Promise, all };
        (defects.map((defect) => prisma.qCDefect.create({
            data: {
                workspace_id: workspaceId,

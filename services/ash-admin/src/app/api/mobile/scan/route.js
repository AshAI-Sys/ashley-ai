...
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        if (code.includes("BDL-") || code.includes("BUNDLE-")) {
            const bundle = await db_1.prisma.bundle.findFirst({
                where: {
                    ...
                    const order = await db_1.prisma.order.findFirst({
                        ...
                        if(order) {
                            ...
                            if (code.includes("FU-") || code.includes("SKU-")) {
                                const finishedUnit = await db_1.prisma.finishedUnit.findFirst({
                                    where: {
                                        OR: [{ sku: code }, { id: code }],
                                    },
                                    include: {
                                    ...
                                    if(finishedUnit) {
                                        ...
                                        if (code.includes("CTN-") || code.includes("CARTON-")) {
                                            const carton = await db_1.prisma.carton.findFirst({
                                                where: {
                                                    OR: [{ qr_code: code }, { id: code }],
                                                },
                                                include: {
                                                ...
                                                if(carton) {
                                                    ...
                                                    const results = await Promise.all([
                                                        db_1.prisma.bundle.findUnique({ where: { id: code } }),
                                                        db_1.prisma.order.findUnique({ where: { id: code } }),
                                                        db_1.prisma.finishedUnit.findUnique({ where: { id: code } }),
                                                        db_1.prisma.carton.findUnique({ where: { id: code } }),
                                                    ]);

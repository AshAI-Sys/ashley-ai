...
async function GET(_request) {
    try {
        ...
        const [readyForPickup, inTransit, deliveredToday, failedDeliveries, totalShipmentsThisWeek, deliveredThisWeek, averageDeliveryTimes] = await Promise.all([
            // Ready for pickup
            db_1.prisma.shipment.count({
        ...
        const actualOnTimeRate = averageDeliveryTimes.length > 0 ?
            (onTimeDeliveries / averageDeliveryTimes.length) * 100 : 0;
        ...
        return server_1.NextResponse.json({ error: 'Failed to calculate delivery statistics' }, { status: 500 });
    }
}
        ...
        shipments.forEach(shipment => {
            if (shipment.consignee_info) {
                try {
                    ...
                    let city = 'Other';
                    if (address.includes('Quezon City'))
                        city = 'Quezon City';
                    else if (address.includes('Manila'))
                        city = 'Manila';
                    else if (address.includes('Makati'))
                        city = 'Makati';
                    else if (address.includes('BGC') || address.includes('Taguig'))
                        city = 'BGC/Taguig';
                    else if (address.includes('Pasig'))
                        city = 'Pasig';
                    else if (address.includes('Mandaluyong'))
                        city = 'Mandaluyong';
    ...
    try {
        ...
        for (const method of methods) {
            const [total, delivered, avgTime] = await Promise.all([
                db_1.prisma.shipment.count({ where: { method } }),
                db_1.prisma.shipment.count({ where: { method, status: 'DELIVERED' } }),
                db_1.prisma.shipment.findMany({
                    where: {
                        method,
                        status: 'DELIVERED',

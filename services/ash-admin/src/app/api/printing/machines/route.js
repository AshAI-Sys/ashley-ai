...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        const machines = await db_1.prisma.machine.findMany({
            where,
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to fetch machines" }, { status: 500 });
    }
});
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        if (!name || !workcenter) {
            return server_1.NextResponse.json({ success: false, error: "Machine name and workcenter are required" }, { status: 400 });
        }
        // Validate workcenter
        const validWorkcenters = ["PRINTING", "HEAT_PRESS", "EMB", "DRYER"];
        if (!validWorkcenters.includes(workcenter)) {
            return server_1.NextResponse.json({ success: false, error: "Invalid workcenter" }, { status: 400 });
        }
        // Create machine
        const machine = await db_1.prisma.machine.create({
            data: {
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to create machine" }, { status: 500 });
    }
});
async function calculateMachineUtilization(machineId) {
    try {
        ...
        const totalRuns = await db_1.prisma.printRun.count({
            where: {
        ...
        const completedRuns = await db_1.prisma.printRun.count({
            where: {
                machine_id: machineId,
                status: "DONE",
                ended_at: {
...
async function checkMaintenanceDue(machineId) {
    try {
        // Check if there are any pending maintenance work orders
        const pendingMaintenance = await db_1.prisma.workOrder.findFirst({
            where: {
                asset: {

...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        const [runs, totalCount] = await Promise.all([
            ...
            db_1.prisma.printRun.count({ where }),
        ]);
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to fetch print runs" }, { status: 500 });
    }
});
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        const body = await request.json();
        const { order_id, routing_step_id, machine_id, method, target_qty, priority = "NORMAL", notes, } = body;
        ...
        const workcenter = workcenters[method] || "PRINTING";
        // Create print run
        const printRun = await db_1.prisma.printRun.create({
            data: {
                order_id,
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to create print run" }, { status: 500 });
    }
});
function getMethodSpecificData(run) {
    switch (run.method) {
        case "SILKSCREEN":
            return {

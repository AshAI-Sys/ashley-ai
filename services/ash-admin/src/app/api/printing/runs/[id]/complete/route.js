...
async function POST(request, { params }) {
    try {
        ...
        const { qty_completed, qty_rejected = 0, reject_reasons = [], material_consumption = [], quality_notes, ashley_feedback } = body;
        // Check if run exists and can be completed
        const run = await db_1.prisma.printRun.findUnique({
            where: { id: runId },
            include: {
                outputs: true,
                rejects: true
        ...
        if (run.status !== 'IN_PROGRESS' && run.status !== 'PAUSED') {
            return server_1.NextResponse.json({ success: false, error: 'Only in-progress or paused runs can be completed' }, { status: 400 });
        }
...
async function updateMethodSpecificCompletion(tx, runId, method, data) {
    switch (method) {
        case 'SILKSCREEN':
        ...
        case 'EMBROIDERY':
            ...
            if (data.actual_stitch_count || data.thread_breaks || data.runtime_minutes) {
                await tx.embroideryRun.updateMany({
                    where: { run_id: runId },
                    data: {
                        ...(data.actual_stitch_count && { stitch_count: data.actual_stitch_count }),
                        ...(data.thread_breaks && { thread_breaks: data.thread_breaks }),
                        ...(data.runtime_minutes && { runtime_minutes: data.runtime_minutes })
                    }
                });
            }
            break;
...
async function createFinalAnalysis(tx, runId, run, completionData) {
    try {
        ...
        const qualityRate = totalProduced > 0 ? (totalCompleted / totalProduced) * 100 : 100;
        ...
        const analysisData = {
            ...
            recommendations: JSON.stringify(generateRecommendations(run.method, completionData)),
            ...
            result: JSON.stringify({
                ...
                quality_rating: qualityRate >= 95 ? 'EXCELLENT' : qualityRate >= 90 ? 'GOOD' : 'NEEDS_IMPROVEMENT',
                efficiency_rating: efficiencyScore >= 90 ? 'HIGH' : efficiencyScore >= 80 ? 'MEDIUM' : 'LOW',
...
function calculateEfficiencyScore(run, completionData) {
    // Simple efficiency calculation based on time and output
    if (!run.started_at || !completionData.qty_completed)
        return 50;
    const runtimeMinutes = Math.round((new Date().getTime() - new Date(run.started_at).getTime()) / 60000);
    ...
    const expectedMinutes = (expectedMinutesPerPiece[run.method] || 2) * completionData.qty_completed;
    const efficiency = Math.min(100, Math.round((expectedMinutes / runtimeMinutes) * 100));
    return Math.max(0, efficiency);
}
function identifyIssues(completionData) {
    const issues = [];
    if (completionData.qty_rejected > 0) {
        const rejectRate = (completionData.qty_rejected / (completionData.qty_completed + completionData.qty_rejected)) * 100;
        if (rejectRate > 10)

...
async function POST(request, { params }) {
    try {
        ...
        if (run.status !== 'CREATED' && run.status !== 'PAUSED') {
            return server_1.NextResponse.json({ success: false, error: 'Print run cannot be started from current status' }, { status: 400 });
        }
        ...
        if (run.machine_id) {
            ...
            if (busyMachine) {
                return server_1.NextResponse.json({ success: false, error: 'Machine is currently busy with another run' }, { status: 400 });
            }
        }
        // Start the run
    ...
    try {
        switch (method) {
            case 'SILKSCREEN':
                ...
                await db_1.prisma.silkscreenPrep.create({
                    data: {
                        run_id: runId,
                ...
                await db_1.prisma.silkscreenSpec.create({
                    data: {
                        run_id: runId,
                        ink_type: 'PLASTISOL',
                        coats: 1,
                ...
                break;
            case 'SUBLIMATION':
                await db_1.prisma.sublimationPrint.create({
                    data: {
                        run_id: runId,
                ...
                break;
            case 'DTF':
                await db_1.prisma.dtfPrint.create({
                    data: {
                        run_id: runId,
                ...
                await db_1.prisma.dtfPowderCure.create({
                    data: {
                        run_id: runId,
            ...
            case 'EMBROIDERY':
                if (designVersion) {
                    await db_1.prisma.embroideryRun.create({
                        data: {
                            run_id: runId,
...
async function createAshleyAnalysis(runId, run) {
    try {
        const analysisData = {
            entity: 'PRINT_RUN',
            entity_id: runId,
...
function calculateEstimatedRuntime(run) {
    ...
    const minutesPerPiece = baseMinutes[run.method] || 2;
    const targetQty = 100; // Default, should come from run data
    return minutesPerPiece * targetQty;
}
function estimateMaterialConsumption(run) {

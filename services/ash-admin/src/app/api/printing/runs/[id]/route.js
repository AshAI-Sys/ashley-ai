    ...
    try {
        ...
        const transformedRun = {
            ...
            runtime_minutes: run.started_at && run.ended_at ?
                Math.round((new Date(run.ended_at).getTime() - new Date(run.started_at).getTime()) / 60000) :
                null,
        ...
        case 'EMBROIDERY':
            return {
                ...
                runs: run.embroidery_runs.map((embRun) => ({
                    ...embRun,
                    design_file: embRun.design?.files ? JSON.parse(embRun.design.files).dst_url : null
                }))
            };
        default:
            return { type: 'unknown' };

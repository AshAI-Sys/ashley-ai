...
async function PATCH(request, { params }) {
    try {
        ...
        if (material_consumption && Array.isArray(material_consumption)) {
            await Promise.all(material_consumption.map((material) => prisma.printRunMaterial.create({
                data: {
                    run_id: runId,
        ...
        if (quality_data) {
            ...
            if (qty_reject > 0 && reject_reasons) {
                await Promise.all(reject_reasons.map((reject) => prisma.printReject.create({
                    data: {
                        run_id: runId,
                        bundle_id: bundle_id || null,

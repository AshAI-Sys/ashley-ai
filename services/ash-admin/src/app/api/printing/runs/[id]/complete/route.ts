...
export async function POST(
  ...
  try {
    ...
    if (run.status !== 'IN_PROGRESS' && run.status !== 'PAUSED') {
      return NextResponse.json(
        { success: false, error: 'Only in-progress or paused runs can be completed' },
        { status: 400 }
      )
    }
...
async function updateMethodSpecificCompletion(tx: any, runId: string, method: string, data: any) {
  switch (method) {
    case 'SILKSCREEN':
...
async function createFinalAnalysis(tx: any, runId: string, run: any, completionData: any) {
  try {
...
function calculateEfficiencyScore(run: any, completionData: any): number {
  ...
  const expectedMinutes = (expectedMinutesPerPiece[run.method as keyof typeof expectedMinutesPerPiece] || 2) * completionData.qty_completed
...
function generateRecommendations(method: string, completionData: any): string[] {

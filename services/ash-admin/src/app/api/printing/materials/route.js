...
async function GET(request) {
    try {
        ...
        const materials = await db_1.prisma.materialInventory.findMany({
            where,
            orderBy: [
                { material_type: 'asc' },
                { material_name: 'asc' }
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to fetch materials' }, { status: 500 });
    }
}
async function POST(request) {
    try {
        ...
        if (!run_id || !materials || !Array.isArray(materials)) {
            return server_1.NextResponse.json({ success: false, error: 'Run ID and materials array are required' }, { status: 400 });
        }
        ...
        return server_1.NextResponse.json({ success: false, error: error instanceof Error ? error.message : 'Failed to process material consumption' }, { status: 500 });
    }
}
// Get material types for filtering
async function OPTIONS(request) {
    try {
        const materialTypes = await db_1.prisma.materialInventory.findMany({
            select: {
                material_type: true
            },
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to fetch material types' }, { status: 500 });
    }
}

  ...
  try {
    ...
    if (!print_method || !order_id || !quantity) {
      return NextResponse.json(
        { success: false, error: 'Print method, order ID, and quantity are required' },
        { status: 400 }
      )
    }
...
function generateRecommendations(method: string, methodOpt: any, baseFactors: any) {
...
function calculateEstimatedTime(method: string, quantity: number, factors: any) {
  const baseTimes = {
...
function calculateCostPrediction(method: string, quantity: number, materials: any[], factors: any) {
  const baseCosts = {
    SILKSCREEN: 2.50,
    SUBLIMATION: 3.75,
    DTF: 4.25,

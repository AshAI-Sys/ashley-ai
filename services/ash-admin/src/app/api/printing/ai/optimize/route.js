...
async function POST(request) {
    try {
        ...
        const { print_method, order_id, quantity, materials, machine_id, quality_requirements, rush_order = false } = body;
        if (!print_method || !order_id || !quantity) {
            return server_1.NextResponse.json({ success: false, error: 'Print method, order ID, and quantity are required' }, { status: 400 });
        }
        // Get machine data for optimization
        const machine = machine_id ? await db_1.prisma.printMachine.findUnique({
            where: { id: machine_id },
            include: {
                PrintRun: {
                    where: {
        ...
        const historicalRuns = await db_1.prisma.printRun.findMany({
            where: {
                print_method,
                status: 'COMPLETED',
                quantity: {
        ...
        // Store AI analysis
        const aiAnalysis = await db_1.prisma.printRunAIAnalysis.create({
            data: {
                print_method,
                order_id,
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to generate AI optimization' }, { status: 500 });
    }
}
function generateOptimization(method, params) {
    const { quantity, materials, machine, historicalRuns, quality_requirements, rush_order } = params;
    ...
    return {
        recommendations: generateRecommendations(method, methodOptimization, baseFactors),
        confidence_score: calculateConfidenceScore(baseFactors, historicalRuns.length),
        estimated_completion_time: calculateEstimatedTime(method, quantity, baseFactors),
        cost_prediction: calculateCostPrediction(method, quantity, materials, baseFactors),
        quality_prediction: calculateQualityPrediction(baseFactors, quality_requirements),
        material_efficiency: baseFactors.material_optimization,
        factors: { ...baseFactors, ...methodOptimization }
    };
}
function calculateQuantityEfficiency(quantity, method) {
    const optimalRanges = {
    ...
    if (quantity >= range.min && quantity <= range.optimal) {
        ...
        return 0.85 + (1 - ((quantity - range.optimal) / (range.max - range.optimal))) * 0.1;
    }
    else {
        return 0.75;
    }
}
function calculateMaterialOptimization(materials, method) {
...
function calculateMachineEfficiency(machine) {
...
function calculateHistoricalPerformance(runs) {
    if (runs.length === 0)
        return 0.8;
    const qualityScores = runs.map(run => {
        if (!run.QualityControl || run.QualityControl.length === 0)
            return 0.85;
...
function optimizeSilkscreen(params, factors) {
    return {
...
function optimizeSublimation(params, factors) {
    return {
        heat_press_temp: 400,
...
function optimizeDTF(params, factors) {
    return {
...
function optimizeEmbroidery(params, factors) {
    return {
        stitch_density: 4.5,
...
function generateRecommendations(method, methodOpt, baseFactors) {
        ...
        case 'SUBLIMATION':
            if (methodOpt.batch_size_optimization < 10) {
                recommendations.push({
                    type: 'BATCHING',
                    priority: 'LOW',
                    message: 'Small batch detected - consider combining with similar orders',
                    impact: 'efficiency'
                });
            }
            break;
...
function calculateConfidenceScore(factors, historicalCount) {
    ...
    const factorConfidence = Object.values(factors).reduce((sum, val) => sum + (typeof val === 'number' ? val : 0), 0) / Object.keys(factors).length;
    return Math.round((dataConfidence * factorConfidence) * 100) / 100;
}
function calculateEstimatedTime(method, quantity, factors) {
    const baseTimes = {
    ...
    const efficiencyFactor = (factors.machine_efficiency + factors.quantity_efficiency) / 2;
    return Math.round(baseTime / efficiencyFactor);
}
function calculateCostPrediction(method, quantity, materials, factors) {
    const baseCosts = {
        SILKSCREEN: 2.50,
        SUBLIMATION: 3.75,
        DTF: 4.25,
    ...
    const materialEfficiencyDiscount = (factors.material_optimization - 0.8) * 0.15;
    const quantityDiscount = factors.quantity_efficiency > 0.9 ? 0.1 : 0;
    return Math.round(baseCost * (1 - materialEfficiencyDiscount - quantityDiscount) * 100) / 100;
}
function calculateQualityPrediction(factors, requirements) {
    const baseQuality = factors.historical_performance * 0.4 + factors.machine_efficiency * 0.6;
    const requirementsFactor = requirements?.high_quality ? 0.95 : 0.88;
    return Math.min(0.99, Math.round(baseQuality * requirementsFactor * 100) / 100);
}

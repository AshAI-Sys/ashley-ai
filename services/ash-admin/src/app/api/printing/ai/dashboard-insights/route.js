...
function calculateRunPerformanceScore(run) {
    ...
    if (run.started_at) {
        const elapsedHours = (new Date().getTime() - new Date(run.started_at).getTime()) /
            (1000 * 60 * 60);
...
function generateGlobalRecommendations(activeRuns) {
    ...
    activeRuns.forEach(run => {
        ...
        if (run.materials) {
            const totalPlanned = run.materials.reduce((sum, mat) => sum + (mat.qty || 0), 0);
            const totalUsed = run.materials.reduce((sum, mat) => sum + (mat.actual_qty || mat.qty || 0), 0);

...
async function GET(request) {
    try {
        ...
        const activeRuns = await db_1.prisma.printRun.findMany({
            where: {
        ...
        const recentRuns = await db_1.prisma.printRun.findMany({
            where: {
                status: 'COMPLETED',
                completed_at: {
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to generate dashboard insights' }, { status: 500 });
    }
}
function generateDashboardInsights(activeRuns, recentRuns) {
...
function calculateOverallPerformance(activeRuns, recentRuns) {
...
function calculateRunPerformanceScore(run) {
    ...
    // AI analysis factor
    if (run.PrintRunAIAnalysis && run.PrintRunAIAnalysis.length > 0) {
        const aiAnalysis = run.PrintRunAIAnalysis[0];
        if (aiAnalysis.quality_prediction) {
            score = score * 0.8 + aiAnalysis.quality_prediction * 0.2;
        }
    }
    // Time efficiency factor
    if (run.started_at) {
        const elapsedHours = (new Date().getTime() - new Date(run.started_at).getTime()) / (1000 * 60 * 60);
        const estimatedHours = run.estimated_completion_time ? run.estimated_completion_time / 60 : elapsedHours * 1.2;
        const timeEfficiency = estimatedHours > 0 ? Math.min(1, estimatedHours / elapsedHours) : 0.8;
        score = score * 0.9 + timeEfficiency * 0.1;
    }
    return Math.max(0, Math.min(1, score));
}
function analyzeActiveRuns(activeRuns) {
    ...
    activeRuns.forEach(run => {
        ...
        if (run.PrintRunAIAnalysis && run.PrintRunAIAnalysis.length > 0) {
            const aiAnalysis = run.PrintRunAIAnalysis[0];
            if (aiAnalysis.material_efficiency < 0.85 || aiAnalysis.quality_prediction < 0.9) {
...
function generateGlobalRecommendations(activeRuns) {
    ...
    activeRuns.forEach(run => {
        ...
        if (run.PrintRunMaterial) {
            const totalPlanned = run.PrintRunMaterial.reduce((sum, mat) => sum + (mat.qty || 0), 0);
            const totalUsed = run.PrintRunMaterial.reduce((sum, mat) => sum + (mat.actual_qty || mat.qty || 0), 0);
...
function calculatePerformanceTrends(recentRuns) {
    ...
    return {
        efficiency_trend: getTrend(recentMetrics.efficiency, earlierMetrics.efficiency),
...
function calculatePeriodMetrics(runs) {
    if (runs.length === 0) {
    ...
    runs.forEach(run => {
        ...
        if (run.PrintRunAIAnalysis && run.PrintRunAIAnalysis.length > 0) {
            totalEfficiency += run.PrintRunAIAnalysis[0].material_efficiency || 0.8;
        }
        else {
            totalEfficiency += 0.8;
        }
...
function calculateMethodPerformance(activeRuns, recentRuns) {
    const methodStats = {};
    const allRuns = [...activeRuns, ...recentRuns];
    allRuns.forEach(run => {
    ...
    Object.entries(methodStats).forEach(([method, stats]) => {
        ...
        const topIssue = Object.entries(stats.issues).sort(([, a], [, b]) => b - a)[0];
        result[method] = {
            score: Math.round(avgScore * 100) / 100,

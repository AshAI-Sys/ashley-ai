...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, user) => {
    try {
        ...
        if (type)
            where.trend_type = type;
        if (scope)
            where.geographic_scope = scope;
        const trends = await db_1.prisma.marketTrend.findMany({
            where,
            orderBy: [
                { impact_score: 'desc' },
        ...
        const trendStats = {
            total: trends.length,
            by_category: trends.reduce((acc, trend) => {
                acc[trend.trend_category] = (acc[trend.trend_category] || 0) + 1;
                return acc;
            }, {}),
            by_type: trends.reduce((acc, trend) => {
                acc[trend.trend_type] = (acc[trend.trend_type] || 0) + 1;
                return acc;
            }, {}),
            avg_impact: trends.reduce((sum, trend) => sum + trend.impact_score, 0) / trends.length,
            avg_confidence: trends.reduce((sum, trend) => sum + trend.confidence_score, 0) / trends.length,
        };
...
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, user) => {
    try {
        ...
        if (generateTrends) {
            ...
            const trend = await db_1.prisma.marketTrend.create({
                data: {
                    workspace_id: workspaceId,
...
async function generateMarketTrends(workspaceId) {
    ...
    for (const trend of trends) {
        try {
            const savedTrend = await db_1.prisma.marketTrend.create({
                data: trend
            });
...
function getSeasonalTrends(season, workspaceId) {
    const trends = [];
    const now = new Date();
    const seasonalData = {
        SPRING: {
    ...
    trends.push({
        ...
        data_sources: JSON.stringify(['Historical Sales Data', 'Weather Patterns', 'Fashion Industry Reports']),
        keywords: JSON.stringify([season.toLowerCase(), 'seasonal', 'weather-appropriate']),
...
function getColorTrends(workspaceId) {
    const trends = [];
    ...
    const colorTrendData = [
        {
            ...
            confidence: 0.85,
            impact: 0.78,
            reasoning: 'Natural, grounding colors reflecting sustainability consciousness'
        },
        {
            name: 'Digital Blues',
    ...
    colorTrendData.forEach((colorTrend, index) => {
        trends.push({
            ...
            data_sources: JSON.stringify(['Pantone Reports', 'Fashion Week Analysis', 'Social Media Trends']),
            keywords: JSON.stringify(['color', 'palette', colorTrend.name.toLowerCase().replace(' ', '_')]),
...
function getStyleTrends(workspaceId) {
    const trends = [];
    ...
    styleTrendData.forEach(styleTrend => {
        trends.push({
            ...
            data_sources: JSON.stringify(['Fashion Industry Reports', 'Consumer Behavior Studies', 'Social Media Analytics']),
            keywords: JSON.stringify([styleTrend.name.toLowerCase().replace(' ', '_'), 'style', 'fashion']),
...
function getSustainabilityTrends(workspaceId) {
    return [{
            workspace_id: workspaceId,
            ...
            data_sources: JSON.stringify(['Sustainability Reports', 'Consumer Surveys', 'Environmental Studies']),
            keywords: JSON.stringify(['sustainability', 'circular', 'eco-friendly', 'ethical']),
            style_attributes: JSON.stringify({
...
function getTechnologyTrends(workspaceId) {
    return [{
            workspace_id: workspaceId,
            ...
            data_sources: JSON.stringify(['Technology Reports', 'Innovation Labs', 'Patent Filings']),
            keywords: JSON.stringify(['smart_textiles', 'wearable_tech', 'performance_enhancement']),
            style_attributes: JSON.stringify({
                technology: ['moisture_management', 'temperature_regulation', 'biometric_monitoring'],

...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, user) => {
    try {
        ...
        const forecasts = await db_1.prisma.demandForecast.findMany({
            where,
            include: {
                client: true,
                brand: true,
        ...
        async function generateDemandForecast(params) {
            ...
            const historicalOrders = await db_1.prisma.order.findMany({
                where: {
            ...
            const totalHistoricalQuantity = historicalOrders.reduce((sum, order) => sum +
                order.line_items.reduce((itemSum, item) => itemSum + (item.quantity || 0), 0), 0);
            ...
            const forecast = await db_1.prisma.demandForecast.create({
                data: {
                    workspace_id: params.workspaceId,

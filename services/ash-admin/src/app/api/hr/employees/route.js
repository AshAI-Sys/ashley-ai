...
exports.GET = (0, auth_middleware_1.requireAuth)((0, error_handling_1.withErrorHandling)(async (request, user) => {
    const { searchParams } = new URL(request.url);
    const status = searchParams.get("status");
    const position = searchParams.get("position");
    const department = searchParams.get("department");
    const search = searchParams.get("search");
    const where = { workspace_id: user.workspaceId };
...
;
exports.POST = (0, auth_middleware_1.requireAnyPermission)(["hr:create"])((0, error_handling_1.withErrorHandling)(async (request, user) => {
    const data = await request.json();
    const { first_name, last_name, employee_number, email, password, role = "employee", position, department, hire_date, salary_type, base_salary, piece_rate, contact_info = {}, } = data;
...
;
exports.PUT = (0, auth_middleware_1.requireAuth)((0, error_handling_1.withErrorHandling)(async (request, user) => {
    const data = await request.json();

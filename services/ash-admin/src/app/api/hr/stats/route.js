...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        const cacheKey = query_cache_1.CacheKeys.hrStats(workspaceId);
        // Use cached query with 2 minute cache (stats update frequently)
        const stats = await (0, query_cache_1.cachedQueryWithMetrics)(cacheKey, async () => await calculateHRStats(workspaceId, today, startOfToday, endOfToday, thisMonth), query_cache_1.CACHE_DURATION.STATS // 1 minute cache for stats
        );
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to calculate HR statistics" }, { status: 500 });
    }
});
async function calculateHRStats(workspaceId, today, startOfToday, endOfToday, thisMonth) {
    ...
    const [totalEmployees, activeEmployees, todayAttendance, pendingOvertimeRequests, pendingLeaveRequests, currentMonthPayroll, todayAbsences, lastPayrollRun,] = await Promise.all([
        ...
        db_1.prisma.attendanceLog.count({
            where: {
                workspace_id: workspaceId,
                date: {
                    gte: startOfToday,
        ...
        db_1.prisma.payrollEarning.aggregate({
            where: {
                workspace_id: workspaceId,
                payroll_period: {
                    period_start: {
        ...
        // Last payroll period
        db_1.prisma.payrollPeriod.findFirst({
            where: { workspace_id: workspaceId },
    ...
    const allEmployees = await db_1.prisma.employee.findMany({
        where: {
            workspace_id: workspaceId,
            is_active: true,
        },
    ...
    const allEmployeesForTenure = await db_1.prisma.employee.findMany({
        where: {
            workspace_id: workspaceId,
            is_active: true,
        },

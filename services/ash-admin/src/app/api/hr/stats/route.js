...
async function GET(request) {
    try {
        ...
        const departmentStats = await db_1.prisma.employee.groupBy({
            by: ['department'],
            where: {
                workspace_id: 'default',
                is_active: true
        ...
        const positionStats = await db_1.prisma.employee.groupBy({
            by: ['position'],
            where: {
                workspace_id: 'default',
                is_active: true
        ...
        const salaryTypeStats = await db_1.prisma.employee.groupBy({
            by: ['salary_type'],
            where: {
                workspace_id: 'default',
                is_active: true
        ...
        const employeesWithTenure = await db_1.prisma.employee.findMany({
            where: {
                workspace_id: 'default',
                is_active: true,
                hire_date: { not: null }
        ...
        const [sewingProductivity, printingProductivity] = await Promise.all([
            db_1.prisma.sewingRunOperator.aggregate({
                where: {
                    sewing_run: {
                        start_time: {
                            gte: thisMonth
            ...
            db_1.prisma.printRunOperator.aggregate({
                where: {
                    print_run: {
                        start_time: {
                            gte: thisMonth

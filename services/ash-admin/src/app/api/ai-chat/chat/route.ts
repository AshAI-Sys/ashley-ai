...
async function getBusinessContext(workspaceId: string, message: string): Promise<string> {
  try {
    ...
    if (message.toLowerCase().includes('employee') || message.toLowerCase().includes('staff') || message.toLowerCase().includes('payroll')) {

...
const anthropic = process.env.ASH_ANTHROPIC_API_KEY
    ? new sdk_1.default({ apiKey: process.env.ASH_ANTHROPIC_API_KEY })
    : null;
const openai = process.env.ASH_OPENAI_API_KEY
    ? new openai_1.default({ apiKey: process.env.ASH_OPENAI_API_KEY })
    : null;
...
async function getBusinessContext(workspaceId, message) {
    try {
        const contextParts = [];
        ...
        if (orders.length > 0) {
            ...
            orders.forEach((order, idx) => {
                contextParts.push(`${idx + 1}. Order #${order.order_number} - ${order.client?.name || "N/A"} (${order.brand?.name || "N/A"}) - Status: ${order.status} - ₱${order.total_amount?.toLocaleString() || "0"}`);
            });
        }
        // Get clients summary
        ...
        if (clients.length > 0) {
            ...
            clients.forEach((client, idx) => {
                contextParts.push(`${idx + 1}. ${client.name} - ${client._count.brands} brands, ${client._count.orders} orders - Contact: ${client.email || "N/A"}`);
            });
        }
        ...
        if (message.toLowerCase().includes("employee") ||
            ...
            if (employees.length > 0) {
                ...
                employees.forEach((emp, idx) => {
                    contextParts.push(`${idx + 1}. ${emp.first_name} ${emp.last_name} - ${emp.position || "N/A"} - ${emp.salary_type} (${emp.is_active ? "Active" : "Inactive"})`);
                });
            }
        }
...
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        if (!message || !workspace_id) {
            return server_1.NextResponse.json({ error: "message and workspace_id are required" }, { status: 400 });
        }
        ...
        try {
            ...
            return server_1.NextResponse.json({ error: "Failed to setup workspace: " + error.message }, { status: 500 });
        }

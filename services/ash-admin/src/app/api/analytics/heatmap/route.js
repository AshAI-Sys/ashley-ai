...
exports.GET = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        const stats = {
            avgEfficiency: heatmapData.reduce((sum, d) => sum + d.efficiency, 0) /
                heatmapData.length || 0,
            avgDefectRate: heatmapData.reduce((sum, d) => sum + d.defect_rate, 0) /
                heatmapData.length || 0,
        ...
        // Calculate averages
        const heatmapGrid = Object.values(hourlyData).map((item) => ({
            station_type: item.station_type,
            hour: item.hour,
            avg_efficiency: item.efficiency.reduce((a, b) => a + b, 0) /
                item.efficiency.length,
            avg_defect_rate: item.defect_rate.reduce((a, b) => a + b, 0) /
...
exports.POST = (0, auth_middleware_1.requireAuth)(async (req, user) => {
    try {
        ...
        const { date, hour, shift, station_type, station_id, efficiency, output_units, target_units, defect_rate, downtime_mins, operators_count, } = body;

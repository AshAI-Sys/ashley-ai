...
export async function GET(req: NextRequest) {
  try {
    ...
    const stats = {
      avgEfficiency: heatmapData.reduce((sum, d) => sum + d.efficiency, 0) / heatmapData.length || 0,
      avgDefectRate: heatmapData.reduce((sum, d) => sum + d.defect_rate, 0) / heatmapData.length || 0,
    ...
    const heatmapGrid = Object.values(hourlyData).map((item: any) => ({
      station_type: item.station_type,
      hour: item.hour,
      avg_efficiency: item.efficiency.reduce((a: number, b: number) => a + b, 0) / item.efficiency.length,
      avg_defect_rate: item.defect_rate.reduce((a: number, b: number) => a + b, 0) / item.defect_rate.length,

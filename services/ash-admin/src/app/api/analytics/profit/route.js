...
exports.GET = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        const stats = {
            ...
            avgGrossMargin: analyses.reduce((sum, a) => sum + a.gross_margin, 0) /
                analyses.length || 0,
            avgNetMargin: analyses.reduce((sum, a) => sum + a.net_margin, 0) / analyses.length ||
                0,
        ...
        Object.values(byClient).forEach((client) => {
            client.avg_margin =
                client.margins.reduce((a, b) => a + b, 0) /
                    client.margins.length;
            delete client.margins;
        });
        const clientComparison = Object.values(byClient).sort((a, b) => b.total_profit - a.total_profit);
...
exports.POST = (0, auth_middleware_1.requireAuth)(async (req, user) => {
    try {
        ...
        const { order_id, client_id, total_revenue, base_price, rush_fee = 0, design_fee = 0, shipping_fee = 0, material_cost, labor_cost, overhead_cost, shipping_cost = 0, wastage_cost = 0, production_days, lead_time_days, notes, } = body;
        if (!order_id ||
            !client_id ||
            !total_revenue ||
            !material_cost ||
...
exports.PUT = (0, auth_middleware_1.requireAuth)(async (req, user) => {
    try {
        const workspaceId = req.headers.get("x-workspace-id") || "default-workspace";
        const body = await req.json();
        const { order_id } = body;
        ...
        const productionDays = order.delivery_date
            ? Math.ceil((new Date(order.delivery_date).getTime() -

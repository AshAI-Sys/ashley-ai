  ...
  try {
    ...
    if (user.password_reset_sent_at) {
      ...
      if (user.password_reset_sent_at > twoMinutesAgo) {
        const waitSeconds = Math.ceil((user.password_reset_sent_at.getTime() - twoMinutesAgo.getTime()) / 1000)
        return NextResponse.json({
          success: false,
    ...
    // Log event
    await logAuthEvent('PASSWORD_RESET_REQUESTED', user.workspace_id, user.id, request, {
      email: user.email,
    })

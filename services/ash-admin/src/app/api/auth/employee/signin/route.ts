...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!employee.is_active) {
      return createErrorResponse('Employee account is inactive', 401)
    }
    ...
    const token = jwt.sign(
      ...
      JWT_SECRET,
      { expiresIn: '8h' } // 8 hour work shift
    )
...
function getRolePermissions(role: string, position: string) {
  ...
  const positionPermissions: { [key: string]: any } = {
    'Cutting Operator': {

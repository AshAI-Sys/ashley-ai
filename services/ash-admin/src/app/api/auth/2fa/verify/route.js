...
async function POST(request) {
    try {
        ...
        if (!user2FAData || !user2FAData.is_2fa_enabled) {
            return server_1.NextResponse.json({ success: false, error: 'Two-factor authentication is not enabled for this user' }, { status: 400 });
        }
        ...
        if (!isValidTotp && !isValidBackup) {
            ...
            return server_1.NextResponse.json({ success: false, error: 'Invalid verification code' }, { status: 400 });
        }
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to verify two-factor authentication' }, { status: 500 });
    }
}
// Demo function to get user's 2FA data
async function getUserTwoFactorData(userId) {

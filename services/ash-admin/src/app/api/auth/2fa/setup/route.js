...
const Setup2FASchema = zod_1.z.object({
    action: zod_1.z.enum(['generate', 'verify', 'disable']),
    token: zod_1.z.string().optional() // For verification
});
...
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, user) => {
    try {
        ...
        switch (action) {
            ...
            case 'verify':
                if (!token) {
                    return server_1.NextResponse.json({ success: false, error: 'Token is required for verification' }, { status: 400 });
                }
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to process 2FA request' }, { status: 500 });
    }
});
// Generate 2FA setup (secret and QR code)
async function generate2FASetup(user) {
    try {
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to generate 2FA setup' }, { status: 500 });
    }
}
// Verify 2FA setup token
async function verify2FASetup(user, token) {
    try {
        ...
        if (!isValid) {
            return server_1.NextResponse.json({ success: false, error: 'Invalid verification code. Please try again.' }, { status: 400 });
        }
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to verify 2FA setup' }, { status: 500 });
    }
}
// Disable 2FA
async function disable2FA(user) {
    try {
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to disable 2FA' }, { status: 500 });
    }
}
// Generate backup codes
...
// Log audit event
async function logAuditEvent(userId, action, description, metadata) {
    try {

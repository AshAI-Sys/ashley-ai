...
const OnboardingStepSchema = z.object({
  step: z.enum(['personal_info', 'role_assignment', 'department_setup', 'training_schedule', 'equipment_assignment', 'complete']),
  data: z.object({
    // Personal Info Step
    ...
    role_assignment: z.object({
      role: z.enum(['admin', 'manager', 'designer', 'cutting_operator', 'printing_operator', 'sewing_operator', 'qc_inspector', 'finishing_operator', 'warehouse_staff', 'finance_staff', 'hr_staff', 'maintenance_tech']).optional(),
...
const CreateOnboardingSchema = z.object({
  ...
  template_type: z.enum(['basic', 'production', 'office', 'management']).default('basic'),
  priority: z.enum(['low', 'normal', 'high', 'urgent']).default('normal'),
  expected_completion_date: z.string().optional()
})

// GET - List onboarding processes
export const GET = requireAnyPermission(['admin:read', 'hr:read'])(async (request: NextRequest, user: any) => {
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          ...
          overdue: demoOnboardingProcesses.filter(p =>
            p.status !== 'completed' &&
...
export const POST = requireAnyPermission(['admin:create', 'hr:create'])(async (request: NextRequest, user: any) => {
  try {
...
function generateOnboardingChecklist(template_type: string) {
  ...
  return checklists[template_type as keyof typeof checklists] || checklists.basic
}

...
exports.GET = (0, auth_middleware_1.requireAnyPermission)(["admin:read"])(async (request, user) => {
    try {
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to fetch reports" }, { status: 500 });
    }
});
...
exports.POST = (0, auth_middleware_1.requireAnyPermission)(["admin:create"])(async (request, user) => {
    try {
        ...
        return server_1.NextResponse.json({ success: false, error: "Failed to generate report" }, { status: 500 });
    }
});
// Generate specific report
async function generateReport(reportType, searchParams, user) {
...
async function generateReportData(params, user) {
...
async function generateUserActivityReport(department, dateFrom, dateTo) {
    return {
        ...
        top_users: [
            {
                ...
                logins: 47,
                last_login: "2024-09-28T14:30:00Z",
            },
            {
                name: "Maria Santos",
                email: "maria.santos@company.com",
                logins: 42,
                last_login: "2024-09-28T16:15:00Z",
            },
            {
                name: "Carlos Reyes",
                email: "carlos.reyes@company.com",
                logins: 38,
                last_login: "2024-09-28T15:45:00Z",
            },
        ],
        activity_trends: {
...
async function generateDepartmentSummaryReport(department) {
    return {
        report_id: `rpt-${Date.now()}`,
...
async function generateRoleDistributionReport(department) {
    return {
        report_id: `rpt-${Date.now()}`,
...
async function generateSecurityEventsReport(dateFrom, dateTo) {
    return {
        report_id: `rpt-${Date.now()}`,
...
async function generateOnboardingProgressReport(department, dateFrom, dateTo) {
    return {
        report_id: `rpt-${Date.now()}`,
...
// System Usage Report
async function generateSystemUsageReport(dateFrom, dateTo) {
    return {
        report_id: `rpt-${Date.now()}`,
...
async function generatePerformanceMetricsReport(department, dateFrom, dateTo) {
    return {
        report_id: `rpt-${Date.now()}`,
...
async function generateComplianceAuditReport(department, dateFrom, dateTo) {
    return {
        report_id: `rpt-${Date.now()}`,

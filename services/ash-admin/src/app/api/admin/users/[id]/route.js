...
exports.PUT = (0, auth_middleware_1.requireAnyPermission)(['admin:update'])(async (request, user) => {
    try {
        const id = request.url.split('/').pop();
        ...
        if (validatedData.password) {
            updateData.password_hash = await bcryptjs_1.default.hash(validatedData.password, 12);
            delete updateData.password;
        }
        // Track changes for audit
        const changes = {};
        Object.keys(validatedData).forEach(key => {
            if (validatedData[key] !== existingUser[key]) {
                changes[key] = {
...
exports.DELETE = (0, auth_middleware_1.requireAnyPermission)(['admin:delete'])(async (request, user) => {
    try {
        const id = request.url.split('/').pop();
        // Check if user exists
        const existingUser = await db_1.prisma.user.findFirst({

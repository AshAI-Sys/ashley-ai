...
export const PUT = requireAnyPermission(['admin:update'])(async (request: NextRequest, user: any) => {
  try {
    const id = request.url.split('/').pop()
    ...
    Object.keys(validatedData).forEach(key => {
      if (validatedData[key as keyof typeof validatedData] !== existingUser[key as keyof typeof existingUser]) {
        changes[key] = {
...
export const DELETE = requireAnyPermission(['admin:delete'])(async (request: NextRequest, user: any) => {
  try {
    const id = request.url.split('/').pop()

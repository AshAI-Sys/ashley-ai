...
const CreateAuditLogSchema = zod_1.z.object({
    ...
    target_user_id: zod_1.z.string().optional(),
    target_user_email: zod_1.z.string().optional(),
...
exports.GET = (0, auth_middleware_1.requireAnyPermission)(['admin:read'])(async (request, user) => {
    try {
        ...
        let filteredLogs = demoAuditLogs.filter(log => {
            ...
            if (search) {
                matches = matches && (log.description.toLowerCase().includes(search.toLowerCase()) ||
        ...
        const summary = {
            ...
            action_counts: {
                user_actions: filteredLogs.filter(log => log.action.startsWith('USER_')).length,
                security_events: filteredLogs.filter(log => ['SECURITY_ALERT', 'USER_LOGIN', 'USER_LOGOUT'].includes(log.action)).length,
                admin_actions: filteredLogs.filter(log => ['BULK_USER_UPDATE', 'SYSTEM_SETTING_CHANGED'].includes(log.action)).length
            }
        };
...
exports.POST = (0, auth_middleware_1.requireAnyPermission)(['admin:create'])(async (request, user) => {
    try {
        ...
        const ip_address = validatedData.ip_address ||
            request.headers.get('x-forwarded-for') ||
            request.headers.get('x-real-ip') ||
            'unknown';
        const user_agent = validatedData.user_agent ||
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to create audit log' }, { status: 500 });
    }
});
// Helper function to create audit log
async function createAuditLog(performer_user_id, action, description, metadata, target_user_id, target_user_email, severity = 'medium') {
    try {
        const auditLog = {
            action,
            description,

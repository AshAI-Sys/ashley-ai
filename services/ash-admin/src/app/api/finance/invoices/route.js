...
exports.GET = (0, auth_middleware_1.requireAnyPermission)(['finance:read'])((0, error_handling_1.withErrorHandling)(async (request, user) => {
...
exports.POST = (0, auth_middleware_1.requireAnyPermission)(['finance:write'])((0, error_handling_1.withErrorHandling)(async (request, user) => {
    const data = await request.json();
    const { brand_id, client_id, order_id, lines, discount = 0, tax_mode = 'VAT_INCLUSIVE', due_date } = data;
    // Validate required fields
    const validationError = (0, error_handling_1.validateRequiredFields)(data, ['brand_id', 'client_id', 'lines']);
    if (validationError) {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const lineError = (0, error_handling_1.validateRequiredFields)(line, ['description', 'qty', 'unit_price']);
        if (lineError) {
    ...
    // Verify brand exists
    const brand = await db_1.prisma.brand.findUnique({ where: { id: brand_id } });
    if (!brand) {
        throw new error_handling_1.NotFoundError('Brand');
    }

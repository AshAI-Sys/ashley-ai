...
export async function GET(request: NextRequest) {
  try {
    ...
    const processedRuns = runs.map(run => {
      ...
      const completedTasks = run.status === 'COMPLETED' ? totalTasks :
                           run.status === 'IN_PROGRESS' ? Math.floor(totalTasks * 0.6) : 0
    ...
    return NextResponse.json({ error: 'Failed to fetch finishing runs' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create finishing run' }, { status: 500 })
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to update finishing run' }, { status: 500 })
  }
}

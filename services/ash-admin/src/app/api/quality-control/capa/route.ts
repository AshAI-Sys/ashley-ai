...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch CAPA tasks' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const lastCapa = await prisma.cAPATask.findFirst({
      where: {
        capa_number: {
    ...
    return NextResponse.json({ error: 'Failed to create CAPA task' }, { status: 500 })
  }
}
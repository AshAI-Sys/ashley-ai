...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        if$3(inspectionType && inspectionType !== "all");
        {
...
const inspections = await db_1.prisma.qCInspection.findMany({
    where,
    include: {
        order: {
    ...
    return: server_1.NextResponse.json({
        ...
        pagination: {
            page,
            limit,
            total: await db_1.prisma.qCInspection.count({ where }),
        },
    })
});
try { }
    ...
    return server_1.NextResponse.json({ error: "Failed to fetch inspections" }, { status: 500 });
}
;
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, user) => {
    try {
        ...
        const { sample_size, acceptance, rejection } = calculateAQLSampling(data.lot_size, data.aql || 2.5, data.level || "GENERAL_II");
        const inspection = await db_1.prisma.qCInspection.create({
            data: {
        ...
        return server_1.NextResponse.json({ error: "Failed to create inspection" }, { status: 500 });
    }
});
// AQL Sampling calculation based on ANSI/ASQ Z1.4
function calculateAQLSampling(lotSize, aql, level) {

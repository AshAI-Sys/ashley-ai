...
const totalDefects = inspections.reduce((sum, i) => sum + i.critical_found + i.major_found + i.minor_found, 0);
...
switch (period) {
    ...
    case "quarter":
        ...
        break;
        const prevInspections = await db_1.prisma.qCInspection.findMany({
            where: {
                inspection_date: {
                    gte: prevStartDate,
                    lte: prevEndDate,
        ...
        const prevTotalDefects = prevInspections.reduce((sum, i) => sum + i.critical_found + i.major_found + i.minor_found, 0);
        const prevDefectRate = prevTotalSamples > 0 ? (prevTotalDefects / prevTotalSamples) * 100 : 0;
        let trend = "stable";
        if (defectRate > prevDefectRate * 1.05)
            trend = "up";
        else if (defectRate < prevDefectRate * 0.95)
            trend = "down";
...
try { }
    ...
    return server_1.NextResponse.json({ error: "Failed to calculate metrics" }, { status: 500 });
}

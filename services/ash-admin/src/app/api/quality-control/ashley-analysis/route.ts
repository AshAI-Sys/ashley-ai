...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to perform Ashley analysis' }, { status: 500 })
  }
}
...
async function performProcessControlAnalysis(inspection: any) {
  ...
  if (historicalInspections.length >= 10) {
    const defectRates = historicalInspections.map(insp =>
      (insp.critical_found + insp.major_found + insp.minor_found) / insp.sample_size
    )
...
function generateDefectTrendRecommendations(trendDirection: string, defectTrends: any) {
...
function generateRiskMitigationRecommendations(riskLevel: string, riskFactors: any[]) {
  const recommendations = []
...
function generateSPCRecommendations(controlStatus: string, currentRate: number, controlLimits: any) {
  const recommendations = []
...
function generateOverallRecommendation(trend: any, rootCause: any, risk: any, spc: any) {

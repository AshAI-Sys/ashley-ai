...
async function performDefectTrendAnalysis(inspection) {
    ...
    return {
        ...
        recommended_capa_priority: rootCausePredictions.some(pred => pred.confidence_score > 80)
            ? "HIGH"
            : "MEDIUM",
    };
}
async function performQualityRiskAssessment(inspection) {
    ...
    const defectRate = ((inspection.critical_found +
        inspection.major_found +
        inspection.minor_found) /
        inspection.sample_size) *
        100;
...
async function performProcessControlAnalysis(inspection) {
    ...
    if (historicalInspections.length >= 10) {
        const defectRates = historicalInspections.map(insp => (insp.critical_found + insp.major_found + insp.minor_found) /
            insp.sample_size);
        const avgDefectRate = defectRates.reduce((sum, rate) => sum + rate, 0) / defectRates.length;
        const avgSampleSize = historicalInspections.reduce((sum, insp) => sum + insp.sample_size, 0) /

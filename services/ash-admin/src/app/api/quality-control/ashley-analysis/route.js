...
async function performDefectTrendAnalysis(inspection) {
    ...
    // Calculate trends
    const currentDefectRate = (inspection.critical_found + inspection.major_found + inspection.minor_found) / inspection.sample_size * 100;
    const avgDefectRate = timeSeriesData.reduce((sum, data) => sum + data.defect_rate, 0) / timeSeriesData.length || 0;
    const trendDirection = currentDefectRate > avgDefectRate * 1.2 ? 'INCREASING' :
        currentDefectRate < avgDefectRate * 0.8 ? 'DECREASING' : 'STABLE';
    return {
...
async function performRootCausePrediction(inspection) {
    ...
    inspection.defects.forEach(defect => {
        ...
        defectPatterns[category].severity_distribution[defect.severity] += defect.quantity;
        if (defect.location) {
    ...
    return {
        ...
        overall_confidence: rootCausePredictions.reduce((sum, pred) => sum + pred.confidence_score, 0) / rootCausePredictions.length || 0,
        recommended_capa_priority: rootCausePredictions.some(pred => pred.confidence_score > 80) ? 'HIGH' : 'MEDIUM'
    };
}
async function performQualityRiskAssessment(inspection) {
    const riskFactors = [];
    let overallRiskScore = 0;
    // Factor 1: Defect rate vs historical average
    const defectRate = (inspection.critical_found + inspection.major_found + inspection.minor_found) / inspection.sample_size * 100;
    if (defectRate > 5) {
        riskFactors.push({
    ...
    if (inspectionType === 'FINAL' && (inspection.major_found > 0 || inspection.critical_found > 0)) {
        riskFactors.push({
            ...
            impact: 'HIGH',
            score: 35,
            description: 'Defects found at final inspection indicate upstream process issues'
        });
        overallRiskScore += 35;
    }
    ...
    return {
        ...
        recommendations: generateRiskMitigationRecommendations(riskLevel, riskFactors)
    };
}
async function performProcessControlAnalysis(inspection) {
    ...
    const historicalInspections = await db_1.prisma.qCInspection.findMany({
        where: {
            workspace_id: inspection.workspace_id,
            inspection_type: inspection.inspection_type,
            inspection_date: {
    ...
    if (historicalInspections.length >= 10) {
        const defectRates = historicalInspections.map(insp => (insp.critical_found + insp.major_found + insp.minor_found) / insp.sample_size);
        const avgDefectRate = defectRates.reduce((sum, rate) => sum + rate, 0) / defectRates.length;
        const avgSampleSize = historicalInspections.reduce((sum, insp) => sum + insp.sample_size, 0) / historicalInspections.length;
    ...
    const currentDefectRate = (inspection.critical_found + inspection.major_found + inspection.minor_found) / inspection.sample_size;
...
async function performComprehensiveAnalysis(inspection) {
    ...
    return {
        ...
        overall_recommendation: generateOverallRecommendation(trendAnalysis, rootCauseAnalysis, riskAssessment, spcAnalysis)
    };
}
function generateDefectTrendRecommendations(trendDirection, defectTrends) {
    const recommendations = [];
    if (trendDirection === 'INCREASING') {
        recommendations.push('Immediate investigation required - defect rate trending upward');
        recommendations.push('Review recent process changes or material batches');
        recommendations.push('Consider increasing inspection frequency temporarily');
    }
    return recommendations;
}
function generateRiskMitigationRecommendations(riskLevel, riskFactors) {
    const recommendations = [];
    if (riskLevel === 'HIGH') {
        recommendations.push('IMMEDIATE ACTION: Hold shipment pending investigation');
...
function generateSPCRecommendations(controlStatus, currentRate, controlLimits) {
    const recommendations = [];
    if (controlStatus === 'OUT_OF_CONTROL_HIGH') {
        recommendations.push('Process out of control - investigate special causes');
        recommendations.push('Do not ship until process is brought back into control');
    }
    return recommendations;
}
function generateOverallRecommendation(trend, rootCause, risk, spc) {
    if (risk.risk_level === 'HIGH' || spc.control_status === 'OUT_OF_CONTROL_HIGH') {
        return {
            priority: 'IMMEDIATE',
    ...
    if (risk.risk_level === 'MEDIUM' || trend.trend_direction === 'INCREASING') {
        return {
            priority: 'HIGH',
            action: 'INVESTIGATE',
            summary: 'Quality issues detected - investigation and corrective action needed'
        };
    }
    return {
        priority: 'NORMAL',
...
function calculateProcessCapability(historicalInspections) {
    if (historicalInspections.length < 30) {
        return { cp: null, cpk: null, note: 'Insufficient data for capability analysis' };
    }

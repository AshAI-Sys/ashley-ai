...
export async function GET(request: NextRequest) {
  try {
    ...
    if (start_date) {
      filteredLays = filteredLays.filter(lay => 
        new Date(lay.created_at) >= new Date(start_date)
      )
    }
    if (end_date) {
      filteredLays = filteredLays.filter(lay => 
        new Date(lay.created_at) <= new Date(end_date)
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newCutLay = {
      ...
      // Process outputs
      outputs: body.outputs.map((output: any) => ({
...
function calculateAshleyAnalysis(layData: any) {
  const totalPieces = layData.outputs.reduce((sum: number, output: any) => sum + parseInt(output.qty), 0)
...
function calculateEstimatedBundles(outputs: any[]): number {

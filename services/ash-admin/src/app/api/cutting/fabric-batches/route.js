...
const CreateFabricBatchSchema = zod_1.z.object({
    ...
    fabricType: zod_1.z.string().min(1, 'Fabric type is required'),
    color: zod_1.z.string().min(1, 'Color is required'),
    gsm: zod_1.z.number().positive('GSM must be positive'),
    widthCm: zod_1.z.number().positive('Width must be positive'),
    qtyOnHand: zod_1.z.number().positive('Quantity on hand must be positive'),
    uom: zod_1.z.enum(['KG', 'M', 'YD']),
    estimatedYield: zod_1.z.number().positive('Estimated yield must be positive').optional(),
    receivedAt: zod_1.z.string().transform((str) => new Date(str)),
    supplierInfo: zod_1.z.string().optional(),
});
...
async function POST(request) {
    try {
        ...
        const existingBatch = await db_1.prisma.fabricBatch.findFirst({
            where: { lotNo: validatedData.lotNo }
        });
        if (existingBatch) {
            return server_1.NextResponse.json({ success: false, error: 'Fabric batch with this lot number already exists' }, { status: 400 });
        }
        const fabricBatch = await db_1.prisma.fabricBatch.create({
            data: {
                ...validatedData,
                estimatedYield: validatedData.estimatedYield || calculateEstimatedYield(validatedData.uom, validatedData.gsm, validatedData.widthCm),
            },
            include: {
                brand: {
                    select: {
...
async function DELETE(request) {
    try {
        ...
        if (existingBatch._count.fabricIssues > 0) {
            return server_1.NextResponse.json({ success: false, error: 'Cannot delete fabric batch with existing fabric issues' }, { status: 400 });
        }
...
function calculateEstimatedYield(uom, gsm, widthCm) {

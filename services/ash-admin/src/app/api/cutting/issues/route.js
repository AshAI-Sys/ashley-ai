...
async function GET(request) {
    try {
        ...
        if (start_date) {
            filteredIssues = filteredIssues.filter(issue => new Date(issue.created_at) >= new Date(start_date));
        }
        if (end_date) {
            filteredIssues = filteredIssues.filter(issue => new Date(issue.created_at) <= new Date(end_date));
        }
...
function calculateEstimatedPieces(quantity, uom) {

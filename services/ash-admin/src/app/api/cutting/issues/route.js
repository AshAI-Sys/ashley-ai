...
const CreateFabricIssueSchema = zod_1.z.object({
    ...
    batch_id: zod_1.z.string().min(1, "Fabric batch ID is required"),
    qty_issued: zod_1.z.number().positive("Quantity issued must be positive"),
    uom: zod_1.z.string().min(1, "Unit of measure is required"),
    issued_by: zod_1.z.string().min(1, "Issued by is required"),
});
...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        const [fabricIssues, total] = await Promise.all([
            ...
            db_1.prisma.cutIssue.count({ where }),
        ]);
...
exports.PUT = (0, auth_middleware_1.requireAuth)(async (request, user) => {
    try {
        ...
        const existingIssue = await db_1.prisma.cutIssue.findUnique({
            where: { id },
        });
        if (!existingIssue) {
            return server_1.NextResponse.json({ success: false, error: "Fabric issue not found" }, { status: 404 });
        }
        const fabricIssue = await db_1.prisma.cutIssue.update({
            where: { id },
            data: validatedData,
            include: {
                order: {

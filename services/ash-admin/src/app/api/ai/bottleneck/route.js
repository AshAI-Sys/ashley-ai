...
exports.GET = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        const analysis = await bottleneck_detection_1.bottleneckDetectionAI.analyzeProductionSystem(metrics);
        return server_1.NextResponse.json({
            success: true,
            analysis,
        ...
        return server_1.NextResponse.json({ error: "Failed to detect bottlenecks", details: error.message }, { status: 500 });
    }
});
...
exports.POST = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        if (missingFields.length > 0) {
            return server_1.NextResponse.json({ error: `Missing required fields: ${missingFields.join(", ")}` }, { status: 400 });
        }
        metrics.timestamp = new Date();
        // Detect bottleneck at this station
        const detection = await bottleneck_detection_1.bottleneckDetectionAI.detectStationBottleneck(metrics);
        return server_1.NextResponse.json({
            success: true,
            detection,
            analyzed_at: new Date(),
        ...
        return server_1.NextResponse.json({ error: "Failed to detect station bottleneck", details: error.message }, { status: 500 });
    }
});

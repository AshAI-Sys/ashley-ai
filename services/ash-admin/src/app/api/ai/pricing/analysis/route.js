...
exports.GET = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        const pricingData = orders.map(order => {
            ...
            const sewingCost = order.sewing_runs.reduce((sum, run) => sum + (run.piece_rate_pay || 0), 0);
        ...
        const validData = pricingData.filter(d => d.quoted_price > 0 && d.actual_cost > 0);
        ...
        // Perform analysis
        const analysis = await dynamic_pricing_1.dynamicPricingAI.analyzeHistoricalPricing(validData);
        return server_1.NextResponse.json({
            success: true,
            analysis,
        ...
        return server_1.NextResponse.json({ error: "Failed to analyze pricing", details: error.message }, { status: 500 });
    }
});
...
exports.POST = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        if (currentMargin < targetMarginValue &&
            ...
            recommendations.insights.push(`💰 Opportunity to increase prices by ${priceIncrease.toFixed(1)}% to reach ${targetMarginValue}% margin`);
            recommendations.insights.push(`✅ High acceptance rate (${acceptanceRate.toFixed(0)}%) suggests room for price optimization`);
            recommendations.suggested_price = Math.round(suggestedPrice * 100) / 100;
        }
        else if (currentMargin > targetMarginValue &&
            acceptanceRate < minAcceptanceValue) {
            ...
            recommendations.insights.push(`📉 Consider reducing prices by ${priceDecrease.toFixed(1)}% to improve acceptance rate`);
            recommendations.insights.push(`⚠️ Low acceptance rate (${acceptanceRate.toFixed(0)}%) indicates pricing may be too high`);
            recommendations.suggested_price = Math.round(suggestedPrice * 100) / 100;
        }
        else if (currentMargin >= targetMarginValue &&
            acceptanceRate >= minAcceptanceValue) {
            // Optimal pricing
            recommendations.insights.push("✅ Current pricing is optimal - meeting both margin and acceptance targets");
            recommendations.insights.push(`💰 Margin: ${currentMargin.toFixed(1)}% (target: ${targetMarginValue}%)`);
            recommendations.insights.push(`📊 Acceptance: ${acceptanceRate.toFixed(0)}% (target: ${minAcceptanceValue}%)`);
            recommendations.suggested_price = Math.round(avgPrice * 100) / 100;
        }
        else {
            // Below both targets
            recommendations.insights.push("⚠️ Pricing challenges detected - low margin and low acceptance");
            recommendations.insights.push("💡 Focus on cost reduction or value differentiation");
            recommendations.suggested_price = Math.round(avgPrice * 100) / 100;
        }
        ...
        return server_1.NextResponse.json({ error: "Failed to optimize pricing", details: error.message }, { status: 500 });
    }
});

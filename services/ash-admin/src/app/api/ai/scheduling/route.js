...
exports.POST = (0, auth_middleware_1.requireAuth)(async (req, user) => {
    try {
        ...
        const schedule = await smart_scheduling_1.smartSchedulingAI.optimizeSchedule(jobs, resources, startDate);
        return server_1.NextResponse.json({
            success: true,
            schedule,
        ...
        return server_1.NextResponse.json({ error: "Failed to generate schedule", details: error.message }, { status: 500 });
    }
});
...
exports.GET = (0, auth_middleware_1.requireAuth)(async (req, _user) => {
    try {
        ...
        const schedulePreview = orders.map(order => {
            ...
            const daysUntilDeadline = (deadline.getTime() - Date.now()) / (1000 * 60 * 60 * 24);
            const urgency = daysUntilDeadline < 3
                ? "URGENT"
                : daysUntilDeadline < 7
                    ? "HIGH"
                    : "NORMAL";
        ...
        return server_1.NextResponse.json({ error: "Failed to generate preview", details: error.message }, { status: 500 });
    }
});

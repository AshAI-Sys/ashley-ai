...
async function GET(request) {
    try {
        ...
        const schedules = await db_1.prisma.maintenanceSchedule.findMany({
            where,
            include: {
                asset: {
                    select: {
        ...
        const processedSchedules = schedules.map(schedule => {
            ...
            const completedWorkOrders = schedule.work_orders.filter(wo => wo.status === 'completed');
            ...
            return {
                ...
                last_completed: schedule.last_completed?.toISOString() || lastMaintenance?.toISOString() || null,
                estimated_duration: schedule.estimated_duration,
                priority: schedule.priority,
                is_active: schedule.is_active,
                instructions: schedule.instructions,
                required_parts: schedule.required_parts ? JSON.parse(schedule.required_parts) : null,
                required_skills: schedule.required_skills ? JSON.parse(schedule.required_skills) : null,
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to fetch maintenance schedules' }, { status: 500 });
    }
}
async function POST(request) {
    try {
        const body = await request.json();
        const { asset_id, schedule_name, description, maintenance_type, frequency_type, frequency_value, next_due_date, estimated_duration, priority, instructions, required_parts, required_skills, safety_notes } = body;
        if (!asset_id || !schedule_name || !maintenance_type || !frequency_type || !frequency_value) {
        ...
        if (!next_due_date) {
            ...
            switch (frequency_type) {
                ...
                case 'QUARTERLY':
                    ...
                    break;
                case 'YEARLY':
                    calculatedNextDueDate.setFullYear(now.getFullYear() + frequency_value);
                    break;
                default:
                    calculatedNextDueDate.setDate(now.getDate() + frequency_value);
            }
        }
        const schedule = await db_1.prisma.maintenanceSchedule.create({
            data: {
                ...
                estimated_duration: estimated_duration ? parseFloat(estimated_duration) : null,
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to create maintenance schedule' }, { status: 500 });
    }
}
async function PUT(request) {
    try {
        ...
        if (!id) {
            return server_1.NextResponse.json({ success: false, error: 'Schedule ID is required' }, { status: 400 });
        }
        const data = {};
        if (updateData.schedule_name)
            data.schedule_name = updateData.schedule_name;
        ...
        if (updateData.estimated_duration !== undefined) {
            data.estimated_duration = updateData.estimated_duration ? parseFloat(updateData.estimated_duration) : null;
        }
        ...
        if (updateData.required_parts !== undefined) {
            data.required_parts = updateData.required_parts ? JSON.stringify(updateData.required_parts) : null;
        }
        if (updateData.required_skills !== undefined) {
            data.required_skills = updateData.required_skills ? JSON.stringify(updateData.required_skills) : null;
        }
        if (updateData.safety_notes !== undefined)
            data.safety_notes = updateData.safety_notes;
        const schedule = await db_1.prisma.maintenanceSchedule.update({
            where: { id },
            data,
            include: {
                asset: {
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to update maintenance schedule' }, { status: 500 });
    }
}
async function DELETE(request) {
    try {
        ...
        const linkedWorkOrders = await db_1.prisma.workOrder.count({
            where: {
        ...
        if (linkedWorkOrders > 0) {
            return server_1.NextResponse.json({ success: false, error: 'Cannot delete schedule with active work orders' }, { status: 409 });
        }
        await db_1.prisma.maintenanceSchedule.delete({
            where: { id }
        });
        return server_1.NextResponse.json({
            success: true,
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to delete maintenance schedule' }, { status: 500 });
    }
}

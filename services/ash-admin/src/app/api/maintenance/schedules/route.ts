...
export async function GET(request: NextRequest) {
  try {
    ...
    if (maintenance_type && maintenance_type !== 'all') where.maintenance_type = maintenance_type
    if (frequency_type && frequency_type !== 'all') where.frequency_type = frequency_type
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!asset_id || !schedule_name || !maintenance_type || !frequency_type || !frequency_value) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: asset_id, schedule_name, maintenance_type, frequency_type, frequency_value' },
        { status: 400 }
      )
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.maintenance_type) data.maintenance_type = updateData.maintenance_type
    if (updateData.frequency_type) data.frequency_type = updateData.frequency_type
    if (updateData.frequency_value) data.frequency_value = parseInt(updateData.frequency_value)
    if (updateData.next_due_date) data.next_due_date = new Date(updateData.next_due_date)
    if (updateData.last_completed) data.last_completed = new Date(updateData.last_completed)
    ...
    if (updateData.safety_notes !== undefined) data.safety_notes = updateData.safety_notes
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (linkedWorkOrders > 0) {
      return NextResponse.json(
        { success: false, error: 'Cannot delete schedule with active work orders' },
        { status: 409 }
      )
    }

  ...
  try {
    ...
    const processedWorkOrders = workOrders.map(workOrder => {
      return {
        ...
        parts_used: workOrder.parts_used ? JSON.parse(workOrder.parts_used) : null,
        ...
        days_overdue: workOrder.scheduled_date
          ? Math.max(0, Math.floor((new Date().getTime() - new Date(workOrder.scheduled_date).getTime()) / (1000 * 3600 * 24)))
          : 0,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!asset_id || !title || !type || !priority) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: asset_id, title, type, priority' },
        { status: 400 }
      )
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.assigned_to !== undefined) data.assigned_to = updateData.assigned_to
    if (updateData.scheduled_date) data.scheduled_date = new Date(updateData.scheduled_date)
    if (updateData.cost !== undefined) data.cost = updateData.cost ? parseFloat(updateData.cost) : null
    if (updateData.labor_hours !== undefined) data.labor_hours = updateData.labor_hours ? parseFloat(updateData.labor_hours) : null
    if (updateData.parts_used !== undefined) data.parts_used = updateData.parts_used ? JSON.stringify(updateData.parts_used) : null
    if (updateData.notes !== undefined) data.notes = updateData.notes
    if (updateData.completion_notes !== undefined) data.completion_notes = updateData.completion_notes

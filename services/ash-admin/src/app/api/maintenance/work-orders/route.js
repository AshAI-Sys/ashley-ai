...
async function POST(request) {
    try {
        ...
        if (!asset_id || !title || !type || !priority) {
            return server_1.NextResponse.json({ success: false, error: 'Missing required fields: asset_id, title, type, priority' }, { status: 400 });
        }
...
async function PUT(request) {
    try {
        ...
        if (updateData.labor_hours !== undefined)
            data.labor_hours = updateData.labor_hours ? parseFloat(updateData.labor_hours) : null;
        if (updateData.parts_used !== undefined)
            data.parts_used = updateData.parts_used ? JSON.stringify(updateData.parts_used) : null;

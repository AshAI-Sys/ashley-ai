...
exports.GET = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        const [totalAssets, activeAssets, totalWorkOrders, openWorkOrders, overdueSchedules, totalSchedules, completedThisMonth, pendingWorkOrders, workOrdersByType, workOrdersByPriority, assetsByType, upcomingMaintenance, recentWorkOrders, maintenanceCosts,] = await Promise.all([
            // Total assets count
            ...
            db_1.prisma.maintenanceSchedule.count({
                where: {
                    workspace_id,
                    is_active: true,
            ...
            // Assets by type
            db_1.prisma.asset.groupBy({
                by: ["type"],
                where: { workspace_id, deleted_at: null },
                _count: { type: true },
            }),
            // Upcoming maintenance (next 30 days)
            db_1.prisma.maintenanceSchedule.findMany({
                where: {
                    workspace_id,
                    is_active: true,
                    next_due_date: {
            ...
            // Recent work orders
            db_1.prisma.workOrder.findMany({
                where: { workspace_id },
                include: {
                    asset: {
                        select: {
            ...
            db_1.prisma.workOrder.aggregate({
                where: {
                    workspace_id,
                    completed_at: {
                        gte: startOfMonth,
        ...
        const assetUtilization = totalAssets > 0 ? Math.round((activeAssets / totalAssets) * 100) : 0;
        // Process work order distributions
        const workOrderTypeDistribution = workOrdersByType.map(item => ({
            type: item.type,
            count: item._count.type,
            percentage: totalWorkOrders > 0
        ...
        const response = {
            ...
            data: {
                ...
                upcoming_maintenance: upcomingMaintenance.map(schedule => ({
                    ...
                    days_until_due: Math.ceil((schedule.next_due_date.getTime() - today.getTime()) /
                        (1000 * 3600 * 24)),
                })),
                ...
                alerts: {
                    ...
                    high_priority_open: workOrdersByPriority.find(p => p.priority === "CRITICAL")?._count
                        .priority || 0,

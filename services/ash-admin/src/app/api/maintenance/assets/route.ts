...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !asset_number || !type) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: name, asset_number, type' },
        { status: 400 }
      )
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.purchase_cost) data.purchase_cost = parseFloat(updateData.purchase_cost)
    if (updateData.status) data.status = updateData.status
    if (updateData.metadata) data.metadata = JSON.stringify(updateData.metadata)
    if (updateData.purchase_date) data.purchase_date = new Date(updateData.purchase_date)

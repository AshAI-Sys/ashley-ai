...
exports.POST = (0, auth_middleware_1.requireAuth)(async (request, _user) => {
    try {
        ...
        if (!approval_ids ||
            ...
            return server_1.NextResponse.json({ success: false, message: "No approvals selected" }, { status: 400 });
        }
        if (!["send_reminder", "extend_expiry", "cancel_approval"].includes(action)) {
            return server_1.NextResponse.json({ success: false, message: "Invalid action" }, { status: 400 });
        }
        // Get approvals with related data
        const approvals = await db_1.prisma.designApproval.findMany({
            where: {
                id: { in: approval_ids },
        ...
        if (approvals.length === 0) {
            return server_1.NextResponse.json({ success: false, message: "No valid approvals found" }, { status: 404 });
        }
        const results = {
            processed: 0,
            errors: [],
        ...
        switch (action) {
            case "send_reminder":
                for (const approval of approvals) {
                    try {
                        ...
                        const emailContent = message
                            ...
                            .replace("{{expiry_date}}", approval.expires_at?.toLocaleDateString() || "Not set");
                        const reminderEmail = {
              ...
              `,
                        ...
                        console.error(`Error sending reminder for approval ${approval.id}:`, error);
                        results.errors.push(`Failed to send reminder for ${approval.design_asset.name}`);
                    }
                }
                break;
            case "extend_expiry":
                if (!extension_days || extension_days < 1) {
                    return server_1.NextResponse.json({ success: false, message: "Invalid extension period" }, { status: 400 });
                }
                for (const approval of approvals) {
                    try {
                        const newExpiryDate = new Date();
                        newExpiryDate.setDate(newExpiryDate.getDate() + parseInt(extension_days));
                        ...
                        results.errors.push(`Failed to extend expiry for ${approval.design_asset.name}`);
                    }
                }
                break;
            case "cancel_approval":
                for (const approval of approvals) {
                    try {
                        ...
                        results.errors.push(`Failed to cancel approval for ${approval.design_asset.name}`);
                    }
                }
                break;
        }

...
exports.GET = (0, error_handling_1.withErrorHandling)(async (request, { params }) => {
    ...
    if (!employee) {
        throw new error_handling_1.NotFoundError("Employee not found");
    }
    ...
    if (employee.department === "Sewing") {
        ...
        const todayRuns = sewingRuns.filter(run => run.created_at >= today && run.created_at < tomorrow);
        todayPieces = todayRuns.reduce((sum, run) => sum + (run.qty_good || 0), 0);
        const weekRuns = sewingRuns.filter(run => run.created_at >= weekStart && run.created_at < weekEnd);
        ...
        const qcInspections = await db_1.prisma.qCInspection.findMany({
            where: { inspector_id: employeeId },
        });
        totalPieces = qcInspections.length;
        const todayInspections = qcInspections.filter(check => check.created_at >= today && check.created_at < tomorrow);
        todayPieces = todayInspections.length;
        const weekInspections = qcInspections.filter(check => check.created_at >= weekStart && check.created_at < weekEnd);
        ...
        const cutLays = await db_1.prisma.cutLay.findMany({
            where: {
        ...
        const todayLays = cutLays.filter(lay => lay.created_at >= today && lay.created_at < tomorrow);
        todayPieces = todayLays.reduce((sum, lay) => sum + lay.bundles.length, 0);
        const weekLays = cutLays.filter(lay => lay.created_at >= weekStart && lay.created_at < weekEnd);
        weekPieces = weekLays.reduce((sum, lay) => sum + lay.bundles.length, 0);
        tasksCompleted = cutLays.filter(lay => lay.bundles.every(b => b.status === "DONE")).length;
    }
    else if (employee.department === "Printing") {
        // Count print runs (no printer tracking in schema, use created_by)
        const printRuns = await db_1.prisma.printRun.findMany({
            where: {
                created_by: employeeId,
            },
            include: {
        ...
        totalPieces = printRuns.reduce((sum, run) => sum + run.outputs.reduce((s, o) => s + o.qty_good, 0), 0);
        const todayRuns = printRuns.filter(run => run.created_at >= today && run.created_at < tomorrow);
        todayPieces = todayRuns.reduce((sum, run) => sum + run.outputs.reduce((s, o) => s + o.qty_good, 0), 0);
        const weekRuns = printRuns.filter(run => run.created_at >= weekStart && run.created_at < weekEnd);
        weekPieces = weekRuns.reduce((sum, run) => sum + run.outputs.reduce((s, o) => s + o.qty_good, 0), 0);
    ...
    const efficiencyRate = targetPiecesPerDay > 0
    ...
    if (employee.department === "Sewing" || employee.department === "Cutting") {
        const recentInspections = await db_1.prisma.qCInspection.findMany({
            where: {
                created_at: {
                    gte: weekStart,
                },

...
export const GET = withErrorHandling(async (
  ...
  if (employee.department === 'Sewing') {
    ...
    const todayInspections = qcInspections.filter(check =>
      check.created_at >= today && check.created_at < tomorrow
    )
    todayPieces = todayInspections.length

    const weekInspections = qcInspections.filter(check =>
      check.created_at >= weekStart && check.created_at < weekEnd
    )

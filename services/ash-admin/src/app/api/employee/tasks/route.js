...
exports.GET = (0, error_handling_1.withErrorHandling)(async (request) => {
    ...
    if (!department) {
        return server_1.NextResponse.json({ success: false, error: "Department is required" }, { status: 400 });
    }
    let tasks = [];
    ...
    switch (department) {
        ...
        case "Finishing":
            ...
            const finishingRuns = await db_1.prisma.finishingRun.findMany({
                where: {
                    status: {
    ...
    return (0, error_handling_1.createSuccessResponse)(tasks);
});

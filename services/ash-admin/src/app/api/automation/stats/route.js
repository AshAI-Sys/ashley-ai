...
async function GET(request) {
    try {
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to fetch automation statistics' }, { status: 500 });
    }
}
// Helper functions
...
async function getRulesStats(workspaceId, timeFilter) {
    const [total, active, recentExecutions] = await Promise.all([
        db_1.prisma.automationRule.count({ where: { workspace_id: workspaceId } }),
        db_1.prisma.automationRule.count({
            where: { workspace_id: workspaceId, is_active: true }
        }),
        db_1.prisma.automationExecution.count({
            where: {
...
async function getNotificationsStats(workspaceId, timeFilter) {
...
async function getAlertsStats(workspaceId, timeFilter) {
    const [total, unresolved, bySeverity] = await Promise.all([
        db_1.prisma.alert.count({
            where: { workspace_id: workspaceId, created_at: { gte: timeFilter } }
        }),
        db_1.prisma.alert.count({
            where: {
...
async function getIntegrationsStats(workspaceId, timeFilter) {
    const [total, connected, syncLogs] = await Promise.all([
        db_1.prisma.integration.count({ where: { workspace_id: workspaceId } }),
        db_1.prisma.integration.count({
            where: { workspace_id: workspaceId, is_connected: true }
        }),
        db_1.prisma.integrationSyncLog.groupBy({
            by: ['status'],
...
async function getExecutionStats(workspaceId, timeFilter) {
...
async function getExecutionChartData(workspaceId, timeFilter) {
    // Get hourly execution counts for the time range
    const executions = await db_1.prisma.automationExecution.findMany({
        where: {
            workspace_id: workspaceId,
            started_at: { gte: timeFilter }
        },
...
async function getRecentActivity(workspaceId, limit) {
    const [recentExecutions, recentAlerts, recentNotifications] = await Promise.all([
        db_1.prisma.automationExecution.findMany({

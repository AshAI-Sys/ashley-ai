...
async function GET(request) {
    try {
        ...
        if (isConnected !== null)
            where.is_connected = isConnected === 'true';
        const integrations = await db_1.prisma.integration.findMany({
            where,
            include: {
                user: {
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to fetch integrations' }, { status: 500 });
    }
}
...
async function POST(request) {
    try {
        ...
        const { workspace_id = 'workspace_1', name, type, provider, config, sync_frequency, is_active = true, created_by = 'user_1' } = body;
        // Validate required fields
        if (!name || !type || !provider || !config) {
            return server_1.NextResponse.json({ success: false, error: 'Missing required fields: name, type, provider, config' }, { status: 400 });
        }
        ...
        if (!validationResult.valid) {
            return server_1.NextResponse.json({ success: false, error: validationResult.error }, { status: 400 });
        }
        // Test connection if possible
        const connectionTest = await testConnection(type, provider, config);
        const integration = await db_1.prisma.integration.create({
            data: {
                workspace_id,
                name,
                type,
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to create integration' }, { status: 500 });
    }
}
...
async function PUT(request) {
    try {
        ...
        if (!id) {
            return server_1.NextResponse.json({ success: false, error: 'Integration ID is required' }, { status: 400 });
        }
        // Convert config to JSON string if provided
        if (updateData.config) {
            updateData.config = JSON.stringify(updateData.config);
        }
        const integration = await db_1.prisma.integration.update({
            where: { id },
            data: updateData,
            include: {
                user: {
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to update integration' }, { status: 500 });
    }
}
...
async function DELETE(request) {
    try {
        ...
        await db_1.prisma.integration.delete({
            where: { id }
        });
        return server_1.NextResponse.json({
            success: true,
        ...
        return server_1.NextResponse.json({ success: false, error: 'Failed to delete integration' }, { status: 500 });
    }
}
// Helper functions
...
function validateIntegrationConfig(type, provider, config) {
...
async function testConnection(type, provider, config) {
    try {

...
async function POST(request) {
    try {
        ...
        if (!alert_type || !title || !description) {
            return server_1.NextResponse.json({ success: false, error: 'Missing required fields: alert_type, title, description' }, { status: 400 });
        }
    ...
    try {
        ...
        switch (action) {
            ...
            case 'escalate':
                ...
                break;
            default:
                return server_1.NextResponse.json({ success: false, error: 'Invalid action. Use: acknowledge, resolve, or escalate' }, { status: 400 });
        }
...
async function getAlertSummary(workspaceId) {
    ...
    summary.forEach(group => {
        ...
        result.by_severity[group.severity] += group._count.id;
    });
    return result;
}

...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isAcknowledged !== null) where.is_acknowledged = isAcknowledged === 'true';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!alert_type || !title || !description) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: alert_type, title, description' },
        { status: 400 }
      );
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      default:
        return NextResponse.json(
          { success: false, error: 'Invalid action. Use: acknowledge, resolve, or escalate' },
          { status: 400 }
        );
    }
...
async function getAlertSummary(workspaceId: string) {
  ...
  summary.forEach(group => {
    ...
    result.by_severity[group.severity as keyof typeof result.by_severity] += group._count.id;
  });

  return result;
}

function getEscalationDelay(severity: string, escalationLevel: number = 1): number {
  const baseDelays = {
    LOW: 240,      // 4 hours
    MEDIUM: 120,   // 2 hours
    HIGH: 60,      // 1 hour
  ...
  const baseDelay = baseDelays[severity as keyof typeof baseDelays] || baseDelays.MEDIUM;

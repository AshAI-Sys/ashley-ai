          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="flex items-center text-2xl font-bold text-gray-900">
                <lucide_react_1.Shield className="mr-3 h-8 w-8 text-blue-600"/>
                Security Settings
              </h1>
            ...
            <button onClick={() => router.push("/dashboard")} className="rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200">
              Back to Dashboard
            </button>
          </div>
        </header>
        ...
        <main className="mx-auto max-w-4xl px-6 py-8">
          ...
          <div className="mb-6 rounded-lg border bg-white p-6 shadow-sm">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h2 className="flex items-center text-lg font-semibold text-gray-900">
                  <lucide_react_1.Smartphone className="mr-2 h-5 w-5 text-green-600"/>
            ...
            {!is2FAEnabled ? (<div className="space-y-4">
                ...
                <button onClick={initiateTwoFactorSetup} disabled={loading} className="flex items-center rounded-md bg-green-600 px-6 py-3 font-medium text-white transition-colors hover:bg-green-700 disabled:opacity-50">
                  {loading ? (<lucide_react_1.RefreshCw className="mr-2 h-4 w-4 animate-spin"/>) : (<lucide_react_1.Lock className="mr-2 h-4 w-4"/>)}
                ...
                <div className="flex space-x-3">
                  <button onClick={() => setShowBackupCodes(!showBackupCodes)} className="flex items-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700">
                    <lucide_react_1.Key className="mr-2 h-4 w-4"/>
                    {showBackupCodes ? "Hide" : "Show"} Backup Codes
                  </button>

                  <button onClick={disableTwoFactor} disabled={loading} className="flex items-center rounded-md bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700 disabled:opacity-50">
                    {loading ? (<lucide_react_1.RefreshCw className="mr-2 h-4 w-4 animate-spin"/>) : (<lucide_react_1.Unlock className="mr-2 h-4 w-4"/>)}
                    Disable 2FA
                  </button>
                </div>

                {showBackupCodes && (<div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <div className="mb-3 flex items-center justify-between">
                      <h4 className="font-medium text-gray-900">
                        Backup Codes
                      </h4>
                      <button onClick={downloadBackupCodes} className="flex items-center text-sm text-blue-600 hover:text-blue-800">
                    ...
                    "JKL22222",
                    "MNO33333",
                ].map((code, index) => (<div key={index} className="flex items-center justify-between rounded border bg-white p-2">
                          <span className="font-mono text-sm">{code}</span>
                          <button onClick={() => copyToClipboard(code)} className="text-gray-500 hover:text-gray-600">
          ...
          <div className="rounded-lg border bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 py-3">
                <div className="flex items-center">
                  <div className={`mr-3 h-3 w-3 rounded-full ${user?.is_active ? "bg-green-400" : "bg-red-400"}`}/>
                  <div>
                ...
                <span className={`rounded-full px-2 py-1 text-xs font-semibold ${user?.is_active
            ...
            : "bg-red-100 text-red-800"}`}>
              ...
              <div className="flex items-center justify-between border-b border-gray-100 py-3">
                <div className="flex items-center">
                  <div className={`mr-3 h-3 w-3 rounded-full ${is2FAEnabled ? "bg-green-400" : "bg-yellow-400"}`}/>
                  <div>
                ...
                <span className={`rounded-full px-2 py-1 text-xs font-semibold ${is2FAEnabled
            ? "bg-green-100 text-green-800"
      ...
      <div className="max-h-screen w-full max-w-md overflow-y-auto rounded-lg bg-white p-6">
        ...
        {step === 1 && twoFactorData && (<div className="space-y-4">
            <div className="text-center">
              <lucide_react_1.QrCode className="mx-auto mb-4 h-12 w-12 text-blue-600"/>
            ...
            <div className="mb-4 flex justify-center">
              <img src={twoFactorData.qrCode} alt="2FA QR Code" className="rounded border"/>
            </div>
            ...
            <button onClick={() => setVerificationToken("")} className="w-full rounded-md bg-blue-600 py-2 text-white transition-colors hover:bg-blue-700">
              Next: Verify Setup
            </button>
          </div>)}
        ...
        {step === 2 && (<div className="space-y-4">
            <div className="text-center">
              <lucide_react_1.Smartphone className="mx-auto mb-4 h-12 w-12 text-green-600"/>
            ...
            <input type="text" maxLength={6} value={verificationToken} onChange={e => setVerificationToken(e.target.value.replace(/\D/g, ""))} placeholder="000000" className="w-full rounded-md border border-gray-300 px-4 py-3 text-center font-mono text-lg focus:ring-2 focus:ring-blue-500"/>

            <div className="flex space-x-3">
              <button onClick={onCancel} className="flex-1 rounded-md bg-gray-100 py-2 text-gray-700 transition-colors hover:bg-gray-200">
                Cancel
              </button>
              <button onClick={onVerify} disabled={verificationToken.length !== 6 || loading} className="flex flex-1 items-center justify-center rounded-md bg-blue-600 py-2 text-white transition-colors hover:bg-blue-700 disabled:opacity-50">
                {loading ? (<lucide_react_1.RefreshCw className="h-4 w-4 animate-spin"/>) : ("Verify & Enable")}
              </button>
            </div>
          </div>)}
        ...
        {step === 3 && (<div className="space-y-4">
            <div className="text-center">
              <lucide_react_1.CheckCircle className="mx-auto mb-4 h-12 w-12 text-green-600"/>
              <h4 className="mb-2 font-medium text-gray-900">
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {backupCodes.map((code, index) => (<div key={index} className="flex items-center justify-between rounded border bg-white p-2">
                    <span className="font-mono text-sm">{code}</span>
                    <button onClick={() => copyToClipboard(code)} className="text-gray-500 hover:text-gray-600">
                      <lucide_react_1.Copy className="h-4 w-4"/>
                    </button>
            ...
            <button onClick={onComplete} className="w-full rounded-md bg-green-600 py-2 text-white transition-colors hover:bg-green-700">
              Complete Setup
            </button>
          </div>)}
      </div>

...
function CreateBundlesPage({ params }) {
    ...
    return (<div className="container mx-auto py-6 max-w-6xl">
        ...
        <card_1.CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <badge_1.Badge className="mt-1">{lay.order.brand.code}</badge_1.Badge> {lay.order.order_number}
            </div>
            <div>
      ...
      {step === 'config' && (<card_1.Card>
          <card_1.CardHeader>
            ...
            <card_1.CardDescription>Set up bundle sizes for each size category</card_1.CardDescription>
          </card_1.CardHeader>
          <card_1.CardContent className="space-y-6">
            {bundleConfig.map((config, index) => (<div key={config.size_code} className="border rounded-lg p-4">
      ...
      {step === 'preview' && (<div className="space-y-6">
          <card_1.Card>
            <card_1.CardHeader>
              ...
              <card_1.CardDescription>Review generated QR codes before creation</card_1.CardDescription>
            </card_1.CardHeader>
            <card_1.CardContent>
              <div className="space-y-6">
                {bundleConfig.map((config, index) => (<div key={config.size_code}>
                    <div className="flex items-center gap-3 mb-3">
                      <badge_1.Badge className="text-lg px-3 py-1">{config.size_code}</badge_1.Badge>
                      <span className="font-medium">{config.bundles_count} bundles</span>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {config.qr_codes.map((qr, qrIndex) => (<div key={qrIndex} className="border rounded-lg p-3 bg-gray-50">
          ...
          <card_1.Card>
            ...
            <card_1.CardContent>
              <div className="space-y-4">
                {bundleConfig.map((config) => (<div key={config.size_code} className="border rounded-lg p-4">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Bundle IDs: {config.qr_codes.slice(0, 3).map((qr) => qr.qr_code.split('-').slice(-2).join('-')).join(', ')}
      ...
      {errors.submit && (<div className="flex items-start gap-2 mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">

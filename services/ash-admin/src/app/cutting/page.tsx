...
import { Plus, Scissors, Package, BarChart3, QrCode, Eye, Edit, Trash2, RefreshCw } from 'lucide-react'
...
export default function CuttingPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!data.success) throw new Error(data.error || 'Failed to fetch cut lays')
      return data.data
    },
    staleTime: 30000,
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <p className="text-sm font-medium text-gray-600">Total Lays</p>
                      <p className="text-2xl font-bold text-gray-900">{totalLays}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-2xl font-bold text-gray-900">
                        {cutLays.length > 0
                          ? ((cutLays.reduce((sum, lay) => sum + (lay.netUsed / lay.grossUsed), 0) / cutLays.length) * 100).toFixed(1)
                          : '0'}%
                      </p>
                    </div>
                  </div>

...
export default function CreateLayPage() {
  ...
  const handleOutputChange = (index: number, field: keyof SizeOutput, value: string | number) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.lay_length_m) newErrors.lay_length_m = 'Lay length is required'
    if (!formData.plies) newErrors.plies = 'Number of plies is required'
    if (!formData.gross_used) newErrors.gross_used = 'Gross fabric used is required'
    
    const totalCut = outputs.reduce((sum, output) => sum + output.qty, 0)
    if (totalCut === 0) newErrors.outputs = 'At least one size output is required'
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Select the production order for this lay</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Configure marker and lay parameters</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="marker_width_cm">Marker Width (cm, optional)</Label>
                <Input
                  id="marker_width_cm"
                  type="number"
                  value={formData.marker_width_cm}
                  onChange={(e) => setFormData({ ...formData, marker_width_cm: e.target.value })}
                  placeholder="160"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, lay_length_m: e.target.value })}
                  placeholder="25.5"
                  required
                />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Record fabric consumption and waste</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="gross_used">Gross Used ({formData.uom}) *</Label>
                <Input
                  id="gross_used"
                  type="number"
                  step="0.01"
                  value={formData.gross_used}
                  onChange={(e) => setFormData({ ...formData, gross_used: e.target.value })}
                  placeholder="18.2"
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, offcuts: e.target.value })}
                  placeholder="0.8"
                />
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Record actual pieces cut for each size</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
              ...
              <div key={index} className="flex gap-4 items-end">
                <div className="flex-1 space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleOutputChange(index, 'size_code', e.target.value)}
                    placeholder="M"
                    required
                  />
                  {errors[`output_${index}_size`] && (
                    <p className="text-red-600 text-sm">{errors[`output_${index}_size`]}</p>
                  )}
                </div>
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    value={output.qty}
                    onChange={(e) => handleOutputChange(index, 'qty', parseInt(e.target.value) || 0)}
                    placeholder="48"
                  />
                </div>
            ...
            <Button type="button" variant="outline" onClick={addOutput} className="w-full">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className={`text-center p-3 rounded-lg ${
                ...
                <p className={`text-2xl font-bold ${
                  calculations.efficiency >= 78 ? 'text-green-600' : 'text-orange-600'
                }`}>

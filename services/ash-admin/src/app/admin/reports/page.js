...
function ReportsPage() {
    ...
    const getCategoryIcon = (category) => {
        switch (category) {
            ...
            case "Compliance":
                return <lucide_react_1.CheckCircle className="h-5 w-5"/>;
            default:
    ...
    return (<PermissionGate_1.default roles={["admin"]} fallback={<div className="flex min-h-screen items-center justify-center">
          <div className="text-center">
            <lucide_react_1.BarChart3 className="mx-auto mb-4 h-16 w-16 text-gray-500"/>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className="flex items-center text-2xl font-bold text-gray-900">
                  <lucide_react_1.BarChart3 className="mr-3 h-8 w-8 text-blue-600"/>
                  Reports & Analytics
                </h1>
              ...
              <button onClick={fetchReports} className="flex items-center rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200">
          ...
          <div className="px-6 py-6">
            <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
              <div className="rounded-lg border-l-4 border-blue-500 bg-white p-4 shadow-sm">
                <div className="flex items-center">
                  <lucide_react_1.FileText className="mr-3 h-8 w-8 text-blue-600"/>
                  <div>
              ...
              <div className="rounded-lg border-l-4 border-green-500 bg-white p-4 shadow-sm">
                <div className="flex items-center">
                  <lucide_react_1.Clock className="mr-3 h-8 w-8 text-green-600"/>
                  <div>
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
              ...
              <div className="rounded-lg border-l-4 border-purple-500 bg-white p-4 shadow-sm">
                <div className="flex items-center">
                  <lucide_react_1.PieChart className="mr-3 h-8 w-8 text-purple-600"/>
                  <div>
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      {categories.length}
                    </p>
              ...
              <div className="rounded-lg border-l-4 border-orange-500 bg-white p-4 shadow-sm">
                <div className="flex items-center">
                  <lucide_react_1.TrendingUp className="mr-3 h-8 w-8 text-orange-600"/>
                  <div>
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      {departments.length}
                    </p>
            ...
            <div className="mb-6 rounded-lg bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center">
                  <lucide_react_1.Filter className="mr-2 h-4 w-4 text-gray-500"/>
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-600">
                    Filters:
                  </span>
                </div>

                <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
                ...
                <select value={selectedDepartment} onChange={e => setSelectedDepartment(e.target.value)} className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div className="rounded-lg bg-white shadow-sm">
                  ...
                  {loading ? (<div className="p-8 text-center">
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        {filteredReports.map(report => (<div key={report.id} className="rounded-lg border border-gray-200 p-4 transition-shadow hover:shadow-md">
                            <div className="mb-3 flex items-start justify-between">
                              ...
                              <span className={`rounded-full px-2 py-1 text-xs font-medium ${getCategoryColor(report.category)}`}>
                                {report.category}
                              </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex flex-wrap gap-1">
                                {report.departments.slice(0, 2).map(dept => (<span key={dept} className="rounded bg-gray-100 px-2 py-1 text-xs text-gray-600">
                                    {dept}
                                  </span>))}
                                {report.departments.length > 2 && (<span className="rounded bg-gray-100 px-2 py-1 text-xs text-gray-600">
                                    +{report.departments.length - 2}
                                  </span>)}
                              </div>

                              <button onClick={() => generateReport(report.id)} disabled={generatingReport === report.id} className="flex items-center rounded bg-blue-600 px-3 py-1 text-xs font-medium text-white transition-colors hover:bg-blue-700 disabled:opacity-50">
                                {generatingReport === report.id ? (<lucide_react_1.RefreshCw className="mr-1 h-3 w-3 animate-spin"/>) : (<lucide_react_1.Download className="mr-1 h-3 w-3"/>)}
                                Generate
                              </button>
                            </div>
                          </div>))}
              ...
              <div>
                <div className="rounded-lg bg-white shadow-sm">
                  ...
                  <div className="p-6">
                    {recentReports.length === 0 ? (<div className="py-8 text-center">
                        <lucide_react_1.FileText className="mx-auto mb-4 h-12 w-12 text-gray-600"/>
                        <p className="text-gray-500 dark:text-gray-500">
                          No recent reports
                        </p>
                      </div>) : (<div className="space-y-4">
                        {recentReports.map(report => (<div key={report.id} className="rounded-lg border border-gray-200 p-3">
                            <div className="mb-2 flex items-center justify-between">
                              <h4 className="text-sm font-medium text-gray-900 dark:text-white">
                                {report.name}
                              </h4>
                              <span className={`rounded-full px-2 py-1 text-xs font-medium ${report.status === "completed"
                    ...
                    : "bg-yellow-100 text-yellow-800"}`}>
                            ...
                            <div className="mt-3 flex justify-end">
                              <button className="flex items-center text-xs text-blue-600 hover:text-blue-800" title="View report">
                                <lucide_react_1.Eye className="mr-1 h-3 w-3"/>
                                View
                              </button>
                            </div>
                          </div>))}
      ...
      <div className="max-h-screen w-full max-w-4xl overflow-y-auto rounded-lg bg-white p-6">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <button onClick={() => downloadReport("json")} className="flex items-center rounded bg-blue-600 px-3 py-1 text-sm text-white hover:bg-blue-700">
              <lucide_react_1.Download className="mr-1 h-4 w-4"/>
              JSON
            </button>
            <button onClick={() => downloadReport("csv")} className="flex items-center rounded bg-green-600 px-3 py-1 text-sm text-white hover:bg-green-700">
              <lucide_react_1.Download className="mr-1 h-4 w-4"/>
              CSV
            </button>
            <button onClick={onClose} className="text-gray-500 hover:text-gray-600 dark:text-gray-500">
              ✕
            </button>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {data.summary && (<div>
                ...
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                  {Object.entries(data.summary).map(([key, value]) => (<div key={key} className="rounded-lg border border-gray-200 bg-white p-3">
            ...
            {data.department_breakdown && (<div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full rounded-lg border border-gray-200">
                    ...
                    <tbody>
                      {data.department_breakdown.map((dept, index) => (<tr key={index} className="border-t border-gray-200 dark:border-gray-700">

            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-900 flex items-center">
                  <lucide_react_1.Shield className="w-8 h-8 mr-3 text-purple-600"/>
                  Audit Logs
                </h1>
                <p className="text-sm text-gray-600">System security and user activity monitoring</p>
              </div>
              <div className="flex space-x-3">
                <button onClick={fetchAuditLogs} className="bg-gray-100 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-200 transition-colors flex items-center">
                  <lucide_react_1.RefreshCw className="w-4 h-4 mr-2"/>
                  Refresh
                </button>
                <button onClick={exportAuditLogs} className="bg-purple-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-purple-700 transition-colors flex items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-white rounded-lg p-4 shadow-sm border-l-4 border-blue-500">
                <div className="flex items-center justify-between">
                  ...
                  <lucide_react_1.Activity className="w-8 h-8 text-blue-600"/>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm border-l-4 border-green-500">
                <div className="flex items-center justify-between">
                  ...
                  <lucide_react_1.Clock className="w-8 h-8 text-green-600"/>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm border-l-4 border-yellow-500">
                <div className="flex items-center justify-between">
                  ...
                  <lucide_react_1.Calendar className="w-8 h-8 text-yellow-600"/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                ...
                <div className="md:col-span-2">
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Search audit logs..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-300 rounded-md text-sm w-full focus:ring-2 focus:ring-purple-500 focus:border-purple-500"/>
                  </div>
                </div>

                {/* Action Filter */}
                <select value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-purple-500">
                  <option value="all">All Actions</option>
                  {actions.map(action => (<option key={action} value={action}>{action.replace(/_/g, ' ')}</option>))}
                </select>

                {/* Severity Filter */}
                <select value={severityFilter} onChange={(e) => setSeverityFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-purple-500">
                ...
                {/* Date From */}
                <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-purple-500"/>

                {/* Date To */}
                <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-purple-500"/>
              </div>
            </div>
                ...
                return (<tr key={log.id} className="hover:bg-gray-50">
                        ...
                        : 'System'}
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <button onClick={() => setSelectedLog(log)} className="text-purple-600 hover:text-purple-900" title="View details">
                  ...
                  {totalPages > 1 && (<div className="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex space-x-2">
                          <button onClick={() => setPage(page - 1)} disabled={page === 1} className="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 flex items-center">
                            <lucide_react_1.ChevronLeft className="w-4 h-4 mr-1"/>
                            Previous
                          </button>
                          <button onClick={() => setPage(page + 1)} disabled={page === totalPages} className="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 flex items-center">
                            Next
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${log.severity === 'low' ? 'bg-gray-100 text-gray-800' :
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center">
              <lucide_react_1.Monitor className="w-4 h-4 mr-1"/>
              User Agent
            </label>

...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const apiEndpoint =
        loginType === "admin" ? "/api/auth/login" : "/api/auth/employee-login";
  ...
  return (
    <div
      className="light flex min-h-screen items-center justify-center p-4 font-sans"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
          <div className="corporate-card mx-4 w-full max-w-sm p-8 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 animate-pulse items-center justify-center rounded-full border-4 border-corporate-blue bg-blue-50">
              <svg
                className="h-8 w-8 animate-spin text-corporate-blue"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
      ...
      <div className="corporate-card w-full max-w-md p-10">
        ...
        <div className="mb-6 flex gap-2">
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-2 rounded-lg p-3 text-sm font-medium transition-all ${
              loginType === "admin"
        ...
        <form onSubmit={handleSubmit} className="mb-6">
          ...
          <div className="mb-6 flex items-center justify-between">
            <label
              className="flex cursor-pointer select-none items-center text-sm font-semibold transition-colors"
              style={{ color: "#000000" }}
            >
              <input
                type="checkbox"
                ...
                className="mr-2 h-4 w-4 cursor-pointer rounded accent-corporate-blue"
              />
              Remember my account
            </label>
            <Link
              href="/forgot-password"
              className="text-sm font-semibold text-corporate-blue transition-colors hover:text-blue-700"
            >
              Forgot Password?
            </Link>
          </div>
        ...
        {loginType === "admin" && (
          <div className="mt-6">
            <Link
              href="/register"
              className="block w-full rounded-lg border-2 border-corporate-blue bg-white py-3 text-center text-sm font-semibold text-corporate-blue transition-all hover:bg-blue-50"
            >
              Create New Account
            </Link>
          </div>

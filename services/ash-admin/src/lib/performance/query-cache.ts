...
export const CacheKeys = {
  ...
  attendanceToday: (workspaceId: string, date: string) =>
    `attendance:${workspaceId}:${date}`,

  // Finance
  financeStats: (workspaceId: string) => `finance:stats:${workspaceId}`,
  invoicesList: (page: number, limit: number) =>
    `invoices:list:${page}:${limit}`,
  ...
  printingRuns: (page: number, limit: number) =>
    `printing:runs:${page}:${limit}`,

  // QC
  qcStats: (workspaceId: string) => `qc:stats:${workspaceId}`,
  inspections: (page: number, limit: number) =>
    `qc:inspections:${page}:${limit}`,

  // Maintenance
  maintenanceStats: (workspaceId: string) => `maintenance:stats:${workspaceId}`,
  workOrders: (page: number, limit: number) =>
    `maintenance:workorders:${page}:${limit}`,
...
export function getQueryMetrics() {
  ...
  const avgDuration =
    queryMetrics.reduce((sum, m) => sum + m.duration, 0) / totalQueries || 0;
  ...
  return {
    ...
    avgUncachedDuration,
    speedup:
      avgUncachedDuration > 0 ? avgUncachedDuration / avgCachedDuration : 1,
  };
}

...
exports.getRefreshTokenTTL = getRefreshTokenTTL;
 ...
 */
async function generateTokenPair(userData, deviceInfo) {
    // Generate access token (short-lived)
    const accessToken = (0, jwt_1.generateToken)(userData);
    // Generate refresh token (long-lived, cryptographically secure)
    const refreshToken = crypto_1.default.randomBytes(REFRESH_TOKEN_LENGTH).toString("hex");
    ...
    await redis.setex(`refresh_token:${refreshToken}`, REFRESH_TOKEN_EXPIRY, JSON.stringify(refreshTokenData));
    // Also track all refresh tokens for this user
    await redis.sadd(`user_refresh_tokens:${userData.userId}`, refreshToken);
    await redis.expire(`user_refresh_tokens:${userData.userId}`, REFRESH_TOKEN_EXPIRY);
    return {
        accessToken,
        refreshToken,
 ...
 */
async function refreshAccessToken(refreshToken, currentDeviceInfo) {
    try {
...
async function revokeRefreshToken(refreshToken) {
    try {
        ...
        if (data) {
            ...
            await redis.srem(`user_refresh_tokens:${refreshTokenData.userId}`, refreshToken);
        }
...
async function revokeAllUserTokens(userId) {
    try {
        ...
        await pipeline.exec();
        console.log(`Revoked ${refreshTokens.length} refresh tokens for user ${userId}`);
    }
    catch (error) {
 ...
 */
async function getRefreshTokenTTL(refreshToken) {
    try {

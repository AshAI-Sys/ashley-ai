...
exports.MAX_FILENAME_LENGTH = exports.MAX_FILE_SIZE = exports.ALLOWED_FILE_TYPES = void 0;
...
exports.ALLOWED_FILE_TYPES.all = [
    ...exports.ALLOWED_FILE_TYPES.images,
    ...exports.ALLOWED_FILE_TYPES.documents,
    ...exports.ALLOWED_FILE_TYPES.spreadsheets,
];
 ...
 */
async function validateFile(file, allowedTypes = exports.ALLOWED_FILE_TYPES.all, maxSize = exports.MAX_FILE_SIZE) {
    ...
    if (!validateMagicBytes(bytes, file.type)) {
        return {
            valid: false,
            error: "File signature does not match declared MIME type (possible file type spoofing)",
        };
    }
    // 7. Sanitize filename
...
function sanitizeFilename(filename) {
    ...
    if (sanitized.length > exports.MAX_FILENAME_LENGTH) {
        const ext = getFileExtension(sanitized);
        const nameWithoutExt = sanitized.substring(0, sanitized.lastIndexOf("."));
        const maxNameLength = exports.MAX_FILENAME_LENGTH - (ext ? ext.length + 1 : 0);
        sanitized =
 ...
 */
function performSecurityChecks(filename, bytes) {
 ...
 */
async function validateFiles(files, allowedTypes = exports.ALLOWED_FILE_TYPES.all, maxSize = exports.MAX_FILE_SIZE) {
    const results = await Promise.all(files.map(file => validateFile(file, allowedTypes, maxSize)));

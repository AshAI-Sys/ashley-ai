 ...
 */
export async function getUserActiveSessions(userId: string): Promise<RefreshTokenData[]> {
  try {
    const refreshTokens = await redis.smembers(`user_refresh_tokens:${userId}`)
 ...
 */
export async function getRefreshTokenTTL(refreshToken: string): Promise<number> {
  try {

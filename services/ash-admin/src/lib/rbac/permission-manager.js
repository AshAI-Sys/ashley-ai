...
exports.permissionManager = exports.PermissionManager = void 0;
class PermissionManager {
    ...
    async hasPermission(user_id, resource, action) {
    ...
    getRolePermissions(role) {
        const roleConfig = this.SYSTEM_ROLES[role];
        if (!roleConfig) {
            return [];
        }
    ...
    matchesPermissionPattern(permissionName, pattern) {
        if (pattern === "*")
            return true;
    ...
    async assignPermissions(user_id, permissionNames) {
        const { prisma } = await Promise.resolve().then(() => __importStar(require("@/lib/database")));
        const allPerms = this.getAllPermissions();
    ...
    // Create custom role
    async createRole(workspace_id, name, description, permissionNames) {
        const allPerms = this.getAllPermissions();

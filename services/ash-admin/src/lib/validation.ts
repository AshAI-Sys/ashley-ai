...
export function validateEnum(value: string, allowedValues: string[], fieldName: string): string | null {
...
export function validateAndSanitizeMarketTrendData(data: any): { isValid: boolean; errors: string[]; sanitizedData?: any } {
  const errors: string[] = []

  // Required fields
  const requiredFields = ['trend_name', 'trend_category', 'trend_type', 'confidence_score', 'impact_score']
  ...
  if (data.trend_category && validateEnum(data.trend_category, validCategories, 'trend_category')) {
    errors.push(validateEnum(data.trend_category, validCategories, 'trend_category')!)
  }

  const validTypes = ['PEAK', 'EMERGING', 'DECLINING']
  if (data.trend_type && validateEnum(data.trend_type, validTypes, 'trend_type')) {
    errors.push(validateEnum(data.trend_type, validTypes, 'trend_type')!)
  }

  // Validate score ranges
  if (data.confidence_score && (data.confidence_score < 0 || data.confidence_score > 1)) {
  ...
  const allowedFields = [
    'trend_name', 'trend_category', 'trend_type', 'product_categories',
    'geographic_scope', 'confidence_score', 'impact_score', 'adoption_rate',
    'peak_period_start', 'peak_period_end', 'data_sources', 'keywords',
    'color_palette', 'style_attributes', 'target_demographics', 'opportunity_score'
  ]

...
class JNTProvider {
    ...
    async request(endpoint, body) {
        ...
        const digest = crypto_1.default.createHash("md5").update(dataJson).digest("base64");
        const signature = this.generateSignature(`${this.apiKey}${dataJson}${timestamp}`);
        const headers = {
    ...
    async getQuote(shipment) {
        try {
            ...
            const weightRate = shipment.package_details.weight > 1
     ...
     */
    async bookShipment(shipment, referenceNumber) {
        try {
            const requestBody = {
     ...
     * Cancel shipment
     */
    async cancelShipment(bookingId, reason) {
        try {
            const requestBody = {
...
exports.JNTProvider = JNTProvider;

...
exports.isCSRFExcluded = isCSRFExcluded;
...
function validatePassword(password) {
    const errors = [];
    const { MIN_LENGTH, MAX_LENGTH, REQUIRE_UPPERCASE, REQUIRE_LOWERCASE, REQUIRE_NUMBER, REQUIRE_SPECIAL, SPECIAL_CHARS, } = exports.SECURITY_CONFIG.PASSWORD;
...
function generateCSPHeader() {
    const directives = Object.entries(exports.SECURITY_CONFIG.CSP.directives).map(([key, values]) => {
...
function isIPWhitelisted(ip) {
    if (exports.SECURITY_CONFIG.IP_WHITELIST.length === 0)
        return true;
    return exports.SECURITY_CONFIG.IP_WHITELIST.includes(ip);
}
/**
 * Check if origin is allowed for CORS
 */
function isOriginAllowed(origin) {
    return (exports.SECURITY_CONFIG.CORS.allowedOrigins.includes(origin) ||
        exports.SECURITY_CONFIG.CORS.allowedOrigins.includes("*"));
}
/**
 ...
 */
function isCSRFExcluded(pathname) {
    return exports.SECURITY_CONFIG.CSRF.EXCLUDED_PATHS.some(path => pathname.includes(path));
}

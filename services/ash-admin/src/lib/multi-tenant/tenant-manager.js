...
class TenantManager {
    ...
    async createTenant(config) {
        try {
            ...
            const existing = await database_1.prisma.workspace.findUnique({
    ...
    async getTenantConfig(workspace_id) {
        try {
    ...
    async updateTenantConfig(workspace_id, updates) {
        try {
            ...
            const updatedSettings = {
                ...
                max_users: updates.max_users !== undefined
                    ? updates.max_users
                    : currentConfig.max_users,
                max_orders_per_month: updates.max_orders_per_month !== undefined
                ...
                storage_quota_gb: updates.storage_quota_gb !== undefined
    ...
    // Check tenant limits
    async checkLimits(workspace_id) {
    ...
    async isFeatureEnabled(workspace_id, feature) {
        const config = await this.getTenantConfig(workspace_id);
        if (!config)
            return false;
    ...
    async validateTenantAccess(workspace_id, user_id) {
        try {
    ...
    async getTenantStats(workspace_id) {
        const workspace = await database_1.prisma.workspace.findUnique({
            where: { id: workspace_id },
            include: {
                users: true,
        ...
        const daysActive = Math.floor((Date.now() - workspace.created_at.getTime()) / (1000 * 60 * 60 * 24));
        return {
    ...
    async suspendTenant(workspace_id, reason) {
        try {
            await database_1.prisma.workspace.update({
                where: { id: workspace_id },
    ...
    async activateTenant(workspace_id) {
        try {
            await database_1.prisma.workspace.update({
                where: { id: workspace_id },
                data: { is_active: true },
    ...
    async deleteTenant(workspace_id, confirmation) {
        const workspace = await database_1.prisma.workspace.findUnique({
            where: { id: workspace_id },
        });
        if (!workspace) {
    ...
    async createDefaultAdmin(workspace_id, workspaceName) {
        // Create default admin user
    ...
    async initializeDefaults(workspace_id) {
        ...
        for (const defect of defaultDefectCodes) {
            await database_1.prisma.defectCode.create({
                data: {
                    ...defect,
                    workspace_id,
                },

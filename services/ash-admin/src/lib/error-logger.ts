 ...
 * Log API error
 */
export function logAPIError(error: Error, endpoint: string, method: string, statusCode?: number) {
  logError(error, {
 ...
 */
export function logValidationError(message: string, field?: string, value?: any) {
  logError(message, {
 ...
 */
export function createUserErrorContext(userId: string, email?: string, role?: string) {
  Sentry.setUser({
    id: userId,
    email,
    role,
 ...
 */
export function withAPIErrorLogging<T extends (...args: any[]) => Promise<Response>>(
  handler: T,
  endpoint: string
): T {
  return (async (...args: Parameters<T>) => {

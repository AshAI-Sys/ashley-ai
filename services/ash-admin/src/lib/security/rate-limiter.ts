 ...
 */
async function checkRateLimitRedis(key: string, limit: number, window: number): Promise<RateLimitResult> {
  if (!redis) {
 ...
 */
function checkRateLimitMemory(key: string, limit: number, window: number): RateLimitResult {
  const now = Date.now()
 ...
 */
export function getIdentifier(request: Request, type: 'ip' | 'user' | 'api-key' = 'ip'): string {
  if (type === 'ip') {
 ...
 */
export async function getRateLimitStatus(identifier: string, tier: keyof typeof RateLimitTiers = 'STANDARD') {
  const config = RateLimitTiers[tier]

...
export async function validateFileUpload(
  ...
  if (fileSize > maxSize) {
    errors.push(
      `File size (${formatBytes(fileSize)}) exceeds maximum allowed (${formatBytes(maxSize)})`
    );
  }
  ...
  if (config.allowedMimeTypes && config.allowedMimeTypes.length > 0) {
    if (!config.allowedMimeTypes.includes(mimeType)) {
      errors.push(
        `File type '${mimeType}' is not allowed. Allowed types: ${config.allowedMimeTypes.join(", ")}`
      );
    }
  }
  ...
  if (dangerousPatterns.length > 0) {
    errors.push(
      `File contains potentially dangerous content: ${dangerousPatterns.join(", ")}`
    );
  }

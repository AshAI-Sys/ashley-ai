...
export function validatePassword(password: string): {
  ...
  const errors: string[] = []
  const { MIN_LENGTH, MAX_LENGTH, REQUIRE_UPPERCASE, REQUIRE_LOWERCASE, REQUIRE_NUMBER, REQUIRE_SPECIAL, SPECIAL_CHARS } = SECURITY_CONFIG.PASSWORD
  ...
  if (REQUIRE_SPECIAL && !new RegExp(`[${SPECIAL_CHARS.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&')}]`).test(password)) {
...
export function generateCSPHeader(): string {
  const directives = Object.entries(SECURITY_CONFIG.CSP.directives).map(([key, values]) => {
...
export function isCSRFExcluded(pathname: string): boolean {
  return SECURITY_CONFIG.CSRF.EXCLUDED_PATHS.some(path => pathname.includes(path))
}

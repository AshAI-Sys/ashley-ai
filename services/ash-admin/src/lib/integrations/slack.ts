...
export class SlackService {
  ...
  async requestOrderApproval(orderNumber: string, clientName: string, amount: number, approvalUrl: string): Promise<boolean> {
    return this.send({
      ...
      actions: [
        { text: 'Review Order', url: approvalUrl },
      ],
    });
  }

  // Production milestone notification
  async notifyProductionMilestone(orderNumber: string, milestone: string): Promise<boolean> {
  ...
  async alertCriticalIssue(title: string, description: string, affectedOrders: string[]): Promise<boolean> {
    return this.send({
      title: `🚨 CRITICAL: ${title}`,
      message: description,
      color: 'danger',
      fields: [
        { title: 'Affected Orders', value: affectedOrders.join(', '), short: false },
  ...
  async notifyQualityIssue(bundleNumber: string, defectRate: number, details: string): Promise<boolean> {
    return this.send({
  ...
  async sendDailySummary(stats: {
    ...
    return this.send({
      ...
      color: 'good',
      fields: [
        { title: 'Orders Completed', value: stats.ordersCompleted.toString(), short: true },
        { title: 'Units Produced', value: stats.unitsProduced.toLocaleString(), short: true },
  ...
  // Team notification
  async notifyTeam(title: string, message: string, channel?: string): Promise<boolean> {
    return this.send({
      title,
      message,
      color: '#3b82f6',
  ...
  // Core send function
  private async send(payload: NotificationPayload, channel?: string): Promise<boolean> {
    try {
      const slackMessage = {
...
export class TeamsService {
  ...
  async alertCriticalIssue(title: string, description: string): Promise<boolean> {
    return this.send({
      title: `🚨 CRITICAL: ${title}`,
      message: description,
      color: 'danger',

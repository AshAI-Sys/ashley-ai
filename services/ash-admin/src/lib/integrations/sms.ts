...
export class SMSService {
  ...
  async sendProductionDelayAlert(managerPhone: string, orderNumber: string, delayHours: number): Promise<boolean> {
  ...
  // Quality issue alert
  async sendQualityIssueAlert(supervisorPhone: string, bundleNumber: string, defectRate: number): Promise<boolean> {
  ...
  async sendStockShortageAlert(purchasingPhone: string, material: string, currentStock: number): Promise<boolean> {
  ...
  async sendMachineBreakdownAlert(maintenancePhone: string, machineName: string, location: string): Promise<boolean> {
    const message = `🔧 MAINTENANCE ALERT: ${machineName} at ${location} has broken down. Immediate attention required.`;
    return this.sendSMS({ to: maintenancePhone, message, priority: 'critical' });
  }

  // Delivery issue alert
  async sendDeliveryIssueAlert(logisticsPhone: string, shipmentNumber: string, issue: string): Promise<boolean> {
  ...
  async sendPaymentReceivedNotification(accountingPhone: string, invoiceNumber: string, amount: number): Promise<boolean> {
  ...
  // Custom alert
  async sendCustomAlert(phone: string, message: string, priority: 'low' | 'medium' | 'high' | 'critical' = 'medium'): Promise<boolean> {

 ...
 */
export async function checkSessionTimeout(sessionId: string): Promise<SessionTimeoutStatus | null> {
  try {
    ...
    const inactiveDuration = Math.floor((now.getTime() - lastActivity.getTime()) / 1000)
 ...
 */
export async function getUserActiveSessions(userId: string): Promise<SessionActivity[]> {
  try {

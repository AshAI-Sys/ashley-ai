...
function handleDatabaseError(error) {
    if (error && typeof error === 'object' && 'code' in error) {
        ...
        switch (prismaError.code) {
            ...
            case 'P2025':
                return new NotFoundError('Record', { originalError: prismaError.message });
            case 'P2003':
                return new ValidationError('Foreign key constraint failed', undefined, { originalError: prismaError.message });
            case 'P2014':
    ...
    if (error && typeof error === 'object' && 'constructor' in error) {
        ...
        if (errorConstructorName.includes('PrismaClient')) {
            ...
            if (errorConstructorName.includes('UnknownRequestError')) {
                return new DatabaseError('Unknown database error occurred', { originalError: prismaError.message });
            }
            if (errorConstructorName.includes('RustPanicError')) {
                return new DatabaseError('Database engine error', { originalError: prismaError.message });
            }
...
function validateUUID(value, fieldName) {
    ...
    if (!uuidRegex.test(value)) {
        return new ValidationError(`${fieldName} must be a valid UUID`, fieldName, { receivedValue: value });
    }
    return null;
}
...
function validateDate(value, fieldName) {
    ...
    if (isNaN(date.getTime())) {
        return new ValidationError(`${fieldName} must be a valid date`, fieldName, { receivedValue: value });
    }
    return null;
}
// Utility functions

...
export class PredictiveMaintenanceAI {
  ...
  async predictAllAssets(assets: MaintenanceData[]): Promise<PredictionResult[]> {
  ...
  private calculateHistoryRisk(history: MaintenanceData['failure_history']): number {
  ...
  private estimateFailureDate(probability: number, currentHours: number): Date | null {
  ...
  private calculateConfidence(data: MaintenanceData): number {
    ...
    // Check data recency
    const daysSinceLastMaintenance =
      (Date.now() - new Date(data.last_maintenance).getTime()) / (1000 * 60 * 60 * 24);
    ...
    const sorted = predictions
      ...
      .sort((a, b) => {
        if (a.risk_level === b.risk_level) {
          return (a.predicted_failure_date?.getTime() || 0) - (b.predicted_failure_date?.getTime() || 0);
        }
      ...
      if (currentDayHours + hoursPerMaintenance > dailyCapacity) {
        ...
        if (currentDayAssets.length > 0) {
          schedule.push({
            ...
            priority: sorted.find(p => currentDayAssets.includes(p.asset_id))?.risk_level || 'MEDIUM',

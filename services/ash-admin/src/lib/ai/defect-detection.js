...
exports.defectDetectionAI = exports.DefectDetectionAI = void 0;
...
class DefectDetectionAI {
    constructor() {
        this.modelVersion = "v1.0.0-simulated";
    }
    // Main defect detection function
    async detectDefects(image, garmentType) {
        ...
        const avgConfidence = defectsFound > 0
        ...
        const majorDefects = detectedDefects.filter(d => d.severity === "MAJOR").length;
    ...
    generateDefect(severity, garmentType) {
        // Get defect types matching severity
        const matchingDefects = Object.entries(DEFECT_PATTERNS).filter(([_, pattern]) => pattern.severity === severity);
        if (matchingDefects.length === 0)
            return null;
        const [defectType, pattern] = matchingDefects[Math.floor(Math.random() * matchingDefects.length)];
        // Generate confidence (higher for more severe defects)
        const baseConfidence = severity === "CRITICAL" ? 90 : severity === "MAJOR" ? 80 : 70;
        const confidence = baseConfidence + Math.random() * 10;
        // Generate description
        const description = this.generateDefectDescription(defectType, pattern, garmentType);
    ...
    generateDefectDescription(defectType, pattern, garmentType) {
        const location = pattern.typical_locations[Math.floor(Math.random() * pattern.typical_locations.length)];
        const descriptions = {
    ...
    generateRecommendation(defectType, severity) {
        const recommendations = {
            ...
            BROKEN_STITCH: "REJECT - Re-sew completely. Inspect thread quality and machine settings.",
            ...
            PRINT_MISALIGNMENT: "REJECT - Reprint required. Calibrate printing equipment.",
            INCOMPLETE_PRINT: "REJECT - Reprint. Check ink levels and print pressure.",
        ...
        return (recommendations[defectType] ||
            "Consult quality supervisor for resolution.");
    }
    // Calculate overall quality score
    calculateQualityScore(defects) {
        let score = 100;
    ...
    async analyzeDefectPatterns(inspections) {
        ...
        inspections.forEach(inspection => {
            inspection.defects.forEach(defect => {
                ...
                const severityScore = defect.severity === "CRITICAL"
                    ? 3
        ...
        const inspectionsWithDefects = inspections.filter(i => i.defects.length > 0).length;
    ...
    // Identify root causes
    identifyRootCauses(commonDefects, patternType) {
        const causes = [];
    ...
    // Generate prevention tips
    generatePreventionTips(commonDefects) {
        const tips = [
        ...
        commonDefects.forEach(defect => {
            ...
            if (defect.frequency > 25) {
                tips.push(`Focus on reducing ${defect.type.toLowerCase()} defects through targeted training`);
            }
        });
    ...
    async compareQuality(data) {
        const results = data.map(entity => {
            ...
            const avgQualityScore = entity.inspections.reduce((sum, i) => sum + i.quality_score, 0) /
                totalInspections;
            const totalDefects = entity.inspections.reduce((sum, i) => sum + i.defects_found, 0);
            const criticalDefects = entity.inspections.reduce((sum, i) => sum +
                i.detected_defects.filter(d => d.severity === "CRITICAL").length, 0);
...
exports.DefectDetectionAI = DefectDetectionAI;
// Export singleton
exports.defectDetectionAI = new DefectDetectionAI();

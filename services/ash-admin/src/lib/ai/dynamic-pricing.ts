...
export class DynamicPricingAI {
  ...
  private readonly OVERHEAD_PERCENTAGE = 0.2; // 20% overhead
  private readonly TARGET_MARGIN = 0.3; // 30% target margin
  ...
  async calculatePricing(
    ...
    const clientFactor = this.getClientRelationshipFactor(
  ...
  private getQuantityAdjustment(quantity: number): number {
    ...
    if (quantity >= 5000) return 0.9; // 10% discount
  ...
  private getUrgencyFactor(deadlineDays: number): number {
    if (deadlineDays <= 3) return 1.3; // 30% rush fee
  ...
  private generateRecommendations(
    ...
    if (market.competitor_avg_price && market.competitor_avg_price > 0) {
      const diff =
        ((price - market.competitor_avg_price) / market.competitor_avg_price) *
        100;
      if (diff > 15) {
        recommendations.push(
  ...
  async analyzeHistoricalPricing(
    orders: Array<{
      client_id: string;
      product_type: string;
      quantity: number;
    ...
    const avgAcceptedPrice =
      acceptedOrders.reduce((sum, o) => sum + o.quoted_price, 0) /
      acceptedOrders.length;
    const avgRejectedPrice =
      orders
        .filter(o => !o.accepted)
    ...
    const priceElasticity =
      ((avgRejectedPrice - avgAcceptedPrice) / avgAcceptedPrice) * 100;

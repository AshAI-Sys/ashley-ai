...
exports.dynamicPricingAI = exports.DynamicPricingAI = void 0;
class DynamicPricingAI {
    constructor() {
        // Base pricing constants (can be configured per workspace)
        this.BASE_LABOR_RATE = 500; // PHP per hour
        this.OVERHEAD_PERCENTAGE = 0.2; // 20% overhead
        this.TARGET_MARGIN = 0.3; // 30% target margin
        this.RUSH_FEE_MULTIPLIER = 1.15; // 15% rush fee
    }
    // Calculate recommended pricing
    async calculatePricing(factors, marketConditions) {
        ...
        const pricingFactors = this.analyzePricingFactors(factors, marketConditions, {
            complexityMultiplier,
            quantityAdjustment,
            clientFactor,
            demandFactor,
        ...
        const recommendations = this.generateRecommendations(factors, marketConditions, optimalPrice, marginPercentage);
        // Assess risk level
        const riskLevel = this.assessPricingRisk(marginPercentage, factors.client_history, marketConditions);
        return {
    ...
    getQuantityAdjustment(quantity) {
        ...
        if (quantity < 100)
            return 1.05; // 5% small order fee
        return 1.0; // No adjustment
    }
    // Get client relationship factor
    getClientRelationshipFactor(history) {
        if (!history)
            return 1.0;
        let factor = 1.0;
        ...
        if (history.payment_reliability < 70) {
            factor *= 1.1; // 10% risk premium for unreliable payers
        }
    ...
    getMarketDemandFactor(market) {
        let factor = 1.0;
        // Demand level
        switch (market.demand_level) {
            case "VERY_HIGH":
                factor *= 1.2; // Premium pricing when demand is high
                break;
            case "HIGH":
                factor *= 1.1;
                break;
        ...
        if (market.capacity_utilization > 90) {
            ...
            factor *= 0.9; // Discount to fill capacity
        }
    ...
    getUrgencyFactor(deadlineDays) {
        if (deadlineDays <= 3)
            return 1.3; // 30% rush fee
        if (deadlineDays <= 7)
            return 1.15; // 15% rush fee
        if (deadlineDays <= 14)
            return 1.05; // 5% rush fee
        if (deadlineDays >= 60)
            return 0.95; // 5% discount for flexible timeline
        return 1.0; // Standard timeline
    }
    // Calculate confidence score
    calculateConfidence(factors, market) {
        let confidence = 100;
        // Reduce confidence if missing data
        if (!factors.client_history)
            confidence -= 15;
        if (!market.competitor_avg_price)
            confidence -= 10;
        if (factors.labor_hours_estimate === 0)
            confidence -= 20;
        if (factors.material_cost === 0)
            confidence -= 20;
        // Reduce confidence for very complex or unusual orders
        if (factors.complexity === "VERY_COMPLEX")
            confidence -= 10;
        if (factors.quantity > 50000 || factors.quantity < 10)
            confidence -= 5;
    ...
    analyzePricingFactors(factors, market, multipliers) {
        const analysis = [];
        // Complexity
    ...
    generateRecommendations(factors, market, price, margin) {
        ...
        if (market.competitor_avg_price && market.competitor_avg_price > 0) {
            const diff = ((price - market.competitor_avg_price) / market.competitor_avg_price) *
                100;
            if (diff > 15) {
    ...
    // Assess pricing risk
    assessPricingRisk(margin, clientHistory, market) {
        let riskScore = 0;
        ...
        // Market conditions
        if (market) {
            if (market.capacity_utilization < 50)
                riskScore += 1; // Desperate for orders
            if (market.material_price_trend === "RISING")
                riskScore += 1; // Cost volatility
        }
        if (riskScore >= 4)
            return "HIGH";
        if (riskScore >= 2)
    ...
    async analyzePricingScenarios(baseFactors, market) {
    ...
    async analyzeHistoricalPricing(orders) {
        ...
        const avgAcceptedPrice = acceptedOrders.reduce((sum, o) => sum + o.quoted_price, 0) /
            acceptedOrders.length;
        const avgRejectedPrice = orders
            .filter(o => !o.accepted)
            .reduce((sum, o) => sum + o.quoted_price, 0) /
            (totalOrders - acceptedOrders.length || 1);
        const priceElasticity = ((avgRejectedPrice - avgAcceptedPrice) / avgAcceptedPrice) * 100;
        // Find optimal margin range
        const acceptedMargins = acceptedOrders.map(o => ((o.quoted_price - o.actual_cost) / o.quoted_price) * 100);
        ...
        insights.push(`🎯 Optimal margin range: ${optimalMarginRange.min.toFixed(1)}% - ${optimalMarginRange.max.toFixed(1)}%`);
        return {
...
exports.DynamicPricingAI = DynamicPricingAI;
// Export singleton
exports.dynamicPricingAI = new DynamicPricingAI();

...
exports.predictiveMaintenanceAI = exports.PredictiveMaintenanceAI = void 0;
class PredictiveMaintenanceAI {
    // Predict machine failure probability
    async predictFailure(data) {
        // Calculate individual risk factors
        const runtimeRisk = this.calculateRuntimeRisk(data.runtime_hours, data.last_maintenance);
        ...
        const failureProbability = runtimeRisk * weights.runtime +
        ...
        const predictedFailureDate = this.estimateFailureDate(failureProbability, data.runtime_hours);
        // Generate recommendations
        const recommendedActions = this.generateRecommendations(failureProbability, {
            runtime: runtimeRisk,
    ...
    async predictAllAssets(assets) {
        const predictions = await Promise.all(assets.map(asset => this.predictFailure(asset)));
    ...
    calculateRuntimeRisk(hours, lastMaintenance) {
        const daysSinceLastMaintenance = (Date.now() - new Date(lastMaintenance).getTime()) /
            (1000 * 60 * 60 * 24);
    ...
    calculateVibrationRisk(vibration) {
    ...
    estimateFailureDate(probability, currentHours) {
        if (probability < 25)
            return null;
    ...
    generateRecommendations(probability, risks) {
        const recommendations = [];
        if (probability >= 75) {
            recommendations.push("⚠️ URGENT: Schedule immediate maintenance inspection");
            recommendations.push("Consider temporary shutdown to prevent catastrophic failure");
        }
    ...
    calculateConfidence(data) {
        ...
        if (!data.temperature || data.temperature === 0)
            confidence -= 20;
        if (!data.vibration_level || data.vibration_level === 0)
            confidence -= 20;
        if (!data.noise_level || data.noise_level === 0)
            confidence -= 15;
        if (data.failure_history.length === 0)
            confidence -= 15;
        // Check data recency
        const daysSinceLastMaintenance = (Date.now() - new Date(data.last_maintenance).getTime()) /
            (1000 * 60 * 60 * 24);
    ...
    async optimizeSchedule(predictions, maintenanceCrew = 2) {
        // Sort by risk and predicted failure date
        const sorted = predictions
            .filter(p => p.risk_level !== "LOW")
            .sort((a, b) => {
            if (a.risk_level === b.risk_level) {
                return ((a.predicted_failure_date?.getTime() || 0) -
            ...
            if (currentDayHours + hoursPerMaintenance > dailyCapacity) {
                ...
                if (currentDayAssets.length > 0) {
                    schedule.push({
                        ...
                        priority: sorted.find(p => currentDayAssets.includes(p.asset_id))
                            ?.risk_level || "MEDIUM",
        ...
        if (currentDayAssets.length > 0) {
            schedule.push({
                date: currentDate,
                assets: currentDayAssets,
                priority: sorted.find(p => currentDayAssets.includes(p.asset_id))?.risk_level ||
                    "MEDIUM",
                estimated_hours: currentDayHours,
            });
        }
        ...
        const totalCost = schedule.reduce((sum, s) => sum + s.estimated_hours * costPerMaintenanceHour, 0);
...
exports.PredictiveMaintenanceAI = PredictiveMaintenanceAI;
// Export singleton
exports.predictiveMaintenanceAI = new PredictiveMaintenanceAI();

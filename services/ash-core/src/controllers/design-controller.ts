...
export const getDesignAssets = async (req: AuthenticatedRequest, res: Response) => {
  try {
...
export const getDesignAsset = async (req: AuthenticatedRequest, res: Response) => {
  try {
...
export const uploadDesignAssets = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    logger.info('Design assets uploaded', {
    ...
    logger.error('Upload design assets error:', error)
    res.status(500).json({
      error: 'Internal Server Error',
...
export const approveDesignAsset = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    const updatedAsset = await prisma.designAsset.update({
      ...
      data: {
        ...
        metadata: {
          ...
          approval_history: [
            ...((asset.metadata as any)?.approval_history || []),
            {
    ...
    logger.info('Design asset approval updated', {
    ...
    logger.error('Approve design asset error:', error)
    res.status(500).json({
      error: 'Internal Server Error',
...
export const createDesignVersion = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    const [updatedAsset, newVersion] = await prisma.$transaction(async (tx) => {
      // Create new version
...
export const getClientApprovalLink = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const asset = await prisma.designAsset.findFirst({
      where: {
        id: req.params.id,
    ...
    logger.error('Get client approval link error:', error)
    res.status(500).json({
      error: 'Internal Server Error',

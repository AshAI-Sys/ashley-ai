...
import { DashboardQueries, withQueryTiming } from '../utils/query-optimizer'

export class DashboardController {
  // Real-time production overview with caching
  async getProductionOverview(req: Request, res: Response) {
    try {
      const workspaceId = req.user?.workspace_id
      const cacheKey = PERFORMANCE_CONFIG.CACHE.KEYS.DASHBOARD_OVERVIEW(workspaceId!)
      ...
      const response = await withCache(
        ...
        async () => {
          ...
          const [
            ...
            DashboardQueries.getOrderStats(workspaceId!),
            DashboardQueries.getProductionMetrics(workspaceId!),
            DashboardQueries.getUpcomingDeadlines(workspaceId!, 7),

            // Machine/asset utilization (optimized)
            withQueryTiming('dashboard-machine-utilization', async () => {
            ...
            withQueryTiming('dashboard-quality-metrics', async () => {
              const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
              return prisma.qcCheck.groupBy({
                by: ['status', 'stage'],
                where: { 
          ...
          return this.formatDashboardResponse(
            activeOrders,
            productionStats,
            upcomingDeadlines,
            machineUtilization,
        ...
        PERFORMANCE_CONFIG.CACHE.TTL.DASHBOARD_DATA
      )
  ...
  async getProductionFloorStatus(req: Request, res: Response) {
    try {
      const workspaceId = req.user?.workspace_id
      const cacheKey = PERFORMANCE_CONFIG.CACHE.KEYS.PRODUCTION_FLOOR(workspaceId!)
      ...
      const response = await withCache(
        ...
        async () => {
          ...
          const [bundles, workOrders, employees] = await Promise.all([
            ...
            withQueryTiming('production-floor-bundles', async () => {
            ...
            withQueryTiming('production-floor-work-orders', async () => {
            ...
            withQueryTiming('production-floor-employees', async () => {
      ...
      logger.error('Production floor status error:', error)
  ...
  async getAdvancedAnalytics(req: Request, res: Response) {
    try {
      ...
      const [
        ...
        prisma.sewingOperation.groupBy({
          by: ['employee_id'],
          where: {
            workspace_id: workspaceId,
            completed_at: { gte: startDate }
      ...
      const analytics = {
        trends: {
          ...
          quality: qualityTrends.reduce((acc, curr) => {
            ...
            if (!acc[date]) acc[date] = { passed: 0, failed: 0, total: 0 }
    ...
    return {
      ...
      quality: {
        ...
        pass_rate: qualityMetrics.reduce((acc, curr) => {
          if (curr.status === 'approved') acc.passed += curr._count.id
  ...
  private formatProductionFloorResponse(bundles: any[], workOrders: any[], employees: any[]) {
    return {
      bundles: bundles.map(bundle => ({
        ...
        progress: {
          ...
          printing: bundle.printing_orders.every((po: any) => po.status === 'completed') ? 100 : 
                   bundle.printing_orders.some((po: any) => po.status === 'in_progress') ? 50 : 0,

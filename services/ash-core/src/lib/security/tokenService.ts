...
export class TokenService {
  ...
  async validateApprovalToken(token: string): Promise<TokenValidationResult> {
    try {
  ...
  generateAccessCode(approvalId: string): string {
    ...
    const hash = crypto.createHash('sha256').update(code + approvalId).digest('hex')
  ...
  async checkRateLimit(clientId: string, operation: string): Promise<boolean> {
  ...
  async revokeToken(jti: string, reason: string = 'manual_revocation') {

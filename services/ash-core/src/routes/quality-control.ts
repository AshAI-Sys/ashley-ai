...
const CreateDefectCodeSchema = z.object({
  code: z.string().min(1),
  name: z.string().min(1),
  category: z.enum(['FABRIC', 'SEWING', 'PRINTING', 'FINISHING']),
...
const CreateChecklistSchema = z.object({
  name: z.string().min(1),
  type: z.enum(['INLINE_PRINTING', 'INLINE_SEWING', 'FINAL']),
  category: z.enum(['VISUAL', 'MEASUREMENTS', 'FUNCTIONAL']),
...
const CreateInspectionSchema = z.object({
  order_id: z.string(),
  routing_step_id: z.string().optional(),
  bundle_id: z.string().optional(),
  checklist_id: z.string(),
  inspector_id: z.string(),
  inspection_type: z.enum(['INLINE_PRINTING', 'INLINE_SEWING', 'FINAL']),
  inspection_level: z.enum(['GENERAL_I', 'GENERAL_II', 'GENERAL_III']).optional(),
  aql_critical: z.number().min(0).optional(),
  aql_major: z.number().min(0).optional(), 
  aql_minor: z.number().min(0).optional(),
  lot_size: z.number().int().min(1),
...
function calculateSampleSize(lotSize: number): { sampleSize: number, ac: number, re: number } {
...
function getAcceptanceNumbers(aql: number, sampleSize: number) {
  const aqlStr = aql.toString()
  const base = AQL_ACCEPTANCE_NUMBERS[aqlStr as keyof typeof AQL_ACCEPTANCE_NUMBERS] || { ac: 0, re: 1 }
...
function determineInspectionResult(
  criticalFound: number, majorFound: number, minorFound: number,
  aqlCritical: number, aqlMajor: number, aqlMinor: number,
  sampleSize: number
): 'ACCEPT' | 'REJECT' | 'PENDING_REVIEW' {
...
router.get('/defect-codes', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching defect codes:', error)
    res.status(500).json({ error: 'Failed to fetch defect codes' })
  }
})

// Create defect code
router.post('/defect-codes', async (req: Request, res: Response) => {
  try {
    const workspaceId = req.headers['x-workspace-id'] as string
    ...
    logger.info(`Defect code created: ${defectCode.code}`)
    res.status(201).json(defectCode)
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ error: 'Invalid data', details: error.errors })
    }
    logger.error('Error creating defect code:', error)
    res.status(500).json({ error: 'Failed to create defect code' })
  }
})

// Update defect code
router.put('/defect-codes/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params
    const workspaceId = req.headers['x-workspace-id'] as string
    ...
    logger.error('Error updating defect code:', error)
...
router.get('/inspections', async (req: Request, res: Response) => {
  try {
    ...
    const where = {
      ...
      ...(inspector_id && { inspector_id: inspector_id as string }),
      ...(inspection_type && { inspection_type: inspection_type as string }),
...
router.post('/inspections', async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`QC inspection created: ${inspection.id} for order ${inspection.order?.order_number}`)
...
router.post('/inspections/:id/start', async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Inspection started: ${inspection.id}`)
    res.json(inspection)
  } catch (error) {
...
router.post('/inspections/:id/complete', async (req: Request, res: Response) => {
  try {
    ...
    const result = determineInspectionResult(
      currentInspection.critical_found,
      currentInspection.major_found, 
      currentInspection.minor_found,
      currentInspection.aql_critical,
      currentInspection.aql_major,
      currentInspection.aql_minor,
      currentInspection.sample_size
    )
    ...
    logger.info(`Inspection completed: ${inspection.id} - Result: ${result}`)
    res.json(inspection)
  } catch (error) {
...
router.post('/inspections/:id/defects', async (req: Request, res: Response) => {
  try {
    const { id } = req.params
    const workspaceId = req.headers['x-workspace-id'] as string
    ...
    logger.info(`Defect recorded: ${defect.defect_code?.code} in inspection ${id}`)
    res.status(201).json(defect)
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ error: 'Invalid data', details: error.errors })
    }
    logger.error('Error recording defect:', error)
...
router.post('/aql/calculate-sample-size', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error calculating sample size:', error)
    res.status(500).json({ error: 'Failed to calculate sample size' })
  }
})

// Evaluate inspection result based on defects found
router.post('/aql/evaluate-result', async (req: Request, res: Response) => {
  try {
    const {
      critical_found = 0,
      major_found = 0, 
    ...
    logger.error('Error evaluating AQL result:', error)

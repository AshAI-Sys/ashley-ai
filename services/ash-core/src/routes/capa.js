...
// VALIDATION SCHEMAS
// ================================
const CreateCAPASchema = zod_1.z.object({
    title: zod_1.z.string().min(1),
    type: zod_1.z.enum(['CORRECTIVE', 'PREVENTIVE']),
    priority: zod_1.z.enum(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']),
    source_type: zod_1.z.enum(['QC_INSPECTION', 'CUSTOMER_COMPLAINT', 'INTERNAL_AUDIT', 'OTHER']),
    source_id: zod_1.z.string().optional(),
    inspection_id: zod_1.z.string().optional(),
    defect_id: zod_1.z.string().optional(),
    root_cause: zod_1.z.string().optional(),
    corrective_action: zod_1.z.string().optional(),
    preventive_action: zod_1.z.string().optional(),
    assigned_to: zod_1.z.string().optional(),
    due_date: zod_1.z.string().datetime().optional(),
    created_by: zod_1.z.string()
});
const UpdateCAPASchema = zod_1.z.object({
    title: zod_1.z.string().optional(),
    priority: zod_1.z.enum(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']).optional(),
    status: zod_1.z.enum(['OPEN', 'IN_PROGRESS', 'PENDING_VERIFICATION', 'CLOSED']).optional(),
    root_cause: zod_1.z.string().optional(),
    corrective_action: zod_1.z.string().optional(),
    preventive_action: zod_1.z.string().optional(),
    assigned_to: zod_1.z.string().optional(),
...
const CloseCAPASchema = zod_1.z.object({
...
router.get('/', async (req, res) => {
    try {
        ...
        const { status, priority, type, source_type, assigned_to, created_by, due_from, due_to, page = '1', limit = '50', sort_by = 'created_at', sort_order = 'desc' } = req.query;
        ...
        shared_1.logger.error('Error fetching CAPA tasks:', error);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        shared_1.logger.error('Error fetching CAPA task:', error);
...
router.post('/', async (req, res) => {
    try {
        ...
        shared_1.logger.info(`CAPA task created: ${capaTask.capa_number}`);
        ...
        shared_1.logger.error('Error creating CAPA task:', error);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        shared_1.logger.info(`CAPA task updated: ${capaTask.capa_number}`);
        res.json(capaTask);
    }
    catch (error) {
        if (error instanceof zod_1.z.ZodError) {
        ...
        shared_1.logger.error('Error updating CAPA task:', error);
...
router.post('/:id/start', async (req, res) => {
    try {
        ...
        shared_1.logger.info(`CAPA task started: ${capaTask.capa_number}`);
        res.json(capaTask);
    }
    catch (error) {
        shared_1.logger.error('Error starting CAPA task:', error);
...
router.post('/:id/verify', async (req, res) => {
    try {
        ...
        shared_1.logger.info(`CAPA task marked for verification: ${capaTask.capa_number}`);
        res.json(capaTask);
    }
    catch (error) {
        shared_1.logger.error('Error marking CAPA task for verification:', error);
...
router.post('/:id/close', async (req, res) => {
    try {
        ...
        shared_1.logger.info(`CAPA task closed: ${capaTask.capa_number} - Effectiveness: ${validatedData.effectiveness}`);
        res.json(capaTask);
    }
    catch (error) {
        if (error instanceof zod_1.z.ZodError) {
        ...
        shared_1.logger.error('Error closing CAPA task:', error);
...
router.post('/:id/reopen', async (req, res) => {
    try {
        ...
        shared_1.logger.info(`CAPA task reopened: ${capaTask.capa_number}`);
        res.json(capaTask);
    }
    catch (error) {
        shared_1.logger.error('Error reopening CAPA task:', error);
...
router.get('/analytics/summary', async (req, res) => {
    try {
        ...
        shared_1.logger.error('Error fetching CAPA analytics:', error);
...
router.get('/analytics/trends', async (req, res) => {
    try {
        ...
        shared_1.logger.error('Error fetching CAPA trends:', error);

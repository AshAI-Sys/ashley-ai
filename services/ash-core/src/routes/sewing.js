...
router.get('/operations', [
    (0, auth_1.requirePermission)('sewing.view'),
    (0, express_validator_1.query)('product_type').optional().isString()
], async (req, res) => {
    try {
...
router.post('/operations', [
    (0, auth_1.requirePermission)('sewing.manage'),
    (0, express_validator_1.body)('product_type').isString().notEmpty(),
    (0, express_validator_1.body)('name').isString().notEmpty(),
    (0, express_validator_1.body)('standard_minutes').isFloat({ min: 0.01 }),
    (0, express_validator_1.body)('piece_rate').optional().isFloat({ min: 0 }),
    (0, express_validator_1.body)('depends_on').optional().isArray()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const operation = await prisma.sewingOperation.create({
            ...
            //   newValues: { product_type, name, standard_minutes }
            // })
            logger: shared_1.logger, : .info('Sewing operation created', { operationId: operation.id, product_type, name }),
            res, : .status(201).json({ success: true, data: operation })
        });
        try { }
        catch (error) {
            shared_1.logger.error('Error creating sewing operation', error);
...
router.get('/piece-rates', [
    ...
    (0, express_validator_1.query)('operation_name').optional().isString()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const { brand_id, operation_name } = req.query;
        const where = {
            workspace_id: req.user.workspace_id
        };
        if (brand_id)
        ...
        if (operation_name)
            where.operation_name = { contains: operation_name };
        ...
        try { }
            ...
            shared_1.logger.error('Error fetching piece rates', error);
...
router.post('/piece-rates', [
    (0, auth_1.requirePermission)('sewing.manage'),
    (0, express_validator_1.body)('operation_name').isString().notEmpty(),
    (0, express_validator_1.body)('rate').isFloat({ min: 0.01 }),
    (0, express_validator_1.body)('brand_id').optional().isUUID(),
    (0, express_validator_1.body)('effective_from').optional().isISO8601(),
    (0, express_validator_1.body)('effective_to').optional().isISO8601()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const pieceRate = await prisma.pieceRate.create({
            ...
            //   newValues: { operation_name, rate, brand_id }
            // })
            logger: shared_1.logger, : .info('Piece rate created', { rateId: pieceRate.id, operation_name, rate }),
            res, : .status(201).json({ success: true, data: pieceRate })
        });
        try { }
        catch (error) {
            shared_1.logger.error('Error creating piece rate', error);
...
router.get('/runs', [
    ...
    (0, express_validator_1.query)('operator_id').optional().isUUID(),
    (0, express_validator_1.query)('bundle_id').optional().isUUID(),
    (0, express_validator_1.query)('status').optional().isIn(['CREATED', 'IN_PROGRESS', 'DONE']),
    ...
    try {
        ...
        const { order_id, operator_id, bundle_id, status, limit = 50, offset = 0 } = req.query;
        const where = {};
        ...
        const runs = await prisma.sewingRun.findMany({
            ...
            // })
            // Filter by workspace (since we're going through relations)
            const: filteredRuns = runs.filter(run => run.order.workspace_id === req.user.workspace_id),
            res, : .json({ success: true, data: filteredRuns })
        });
        try { }
        catch (error) {
            shared_1.logger.error('Error fetching sewing runs', error);
...
router.post('/runs', [
    ...
    (0, express_validator_1.body)('operation_name').isString().notEmpty(),
    (0, express_validator_1.body)('operator_id').isUUID(),
    ...
    try {
        ...
        const order = await prisma.order.findUnique({
            ...
            // })
            if(, order) { }
        } || order.workspace_id !== req.user.workspace_id), { return: res, status };
        (404).json({ success: false, error: 'Order not found' });
    }
    ...
    const operation = await prisma.sewingOperation.findFirst({
        ...
        if(, pieceRate) {
            ...
            const sewingRun = await prisma.sewingRun.create({
                ...
                //   newValues: { order_id, operation_name, operator_id, bundle_id }
                // })
                logger: shared_1.logger, : .info('Sewing run created', {
                    runId: sewingRun.id,
                    order_id,
                    operation_name,
                    operator_id
                    ...
                    shared_1.logger.error('Error creating sewing run', error);
            ...
            router.post('/runs/:runId/start', [
                ...
                try {
                    ...
                    const run = await prisma.sewingRun.findUnique({
                        ...
                        // })
                        if(, run) { }
                    } || run.order.workspace_id !== req.user.workspace_id), { return: res };
                }
                finally { }
            }).status(404).json({ success: false, error: 'Sewing run not found' });
        },
        if(run) { }, : .status !== 'CREATED'
    }), { return: res, status };
    (400).json({ success: false, error: 'Run is not in CREATED status' });
});
const updatedRun = await prisma.sewingRun.update({
    ...
    //   newValues: { status: 'IN_PROGRESS', started_at: updatedRun.started_at }
    // })
    logger: shared_1.logger, : .info('Sewing run started', { runId, operator: run.operator.first_name }),
    res, : .json({ success: true, data: updatedRun })
});
try { }
catch (error) {
    shared_1.logger.error('Error starting sewing run', error);
...
router.post('/runs/:runId/complete', [
    ...
    (0, express_validator_1.body)('reject_reason').optional().isString(),
    (0, express_validator_1.body)('reject_photo_url').optional().isURL()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const run = await prisma.sewingRun.findUnique({
            ...
            // })
            if(, run) { }
        } || run.order.workspace_id !== req.user.workspace_id), { return: res, status };
        (404).json({ success: false, error: 'Sewing run not found' });
    }
    finally {
    }
    ...
    const operation = await prisma.sewingOperation.findFirst({
        ...
        if(, pieceRate) {
            ...
            , 
            // })
            shared_1.logger.info('Sewing run completed', {
                runId,
                qty_good,
                qty_reject,
                efficiency_pct: efficiencyPct
            ...
            try { }
                ...
                shared_1.logger.error('Error completing sewing run', error);
    ...
    router.get('/dashboard', [
        ...
        try {
            ...
            const allRuns = await prisma.sewingRun.findMany({
                ...
                ,
                // })
                const: workspaceRuns = allRuns.filter(run => run.order.workspace_id === req.user.workspace_id),
                const: activeRuns = workspaceRuns.filter(run => run.status === 'IN_PROGRESS').length,
                const: todaysRuns = workspaceRuns.filter(run => run.created_at >= today && run.created_at < tomorrow).length
                // Operator efficiency (today)
                ,
                // Operator efficiency (today)
                const: todaysCompleted = workspaceRuns.filter(run => run.status === 'DONE' &&
                    run.ended_at &&
                    run.ended_at >= today &&
                    run.ended_at < tomorrow),
                const: operatorStats = todaysCompleted.reduce((acc, run) => {
                    if (!run.operator_id || !run.efficiency_pct)
                        return acc;
                ...
                const: topOperators = Object.values(operatorStats)
                    ...
                    const: recentRejects = workspaceRuns
                        ...
                        shared_1.logger.error('Error fetching sewing dashboard', error);
            ...
            const operatorRuns = await prisma.sewingRun.findMany({
                ...
                ,
                // })
                const: workspaceOperatorRuns = operatorRuns.filter(r => r.order.workspace_id === req.user.workspace_id),
            ...
            ('Consider reviewing work method for this operation');
        ...
        shared_1.logger.info('Ashley AI efficiency analysis completed', {
            runId,
        ...
        try { }
            ...
            shared_1.logger.error('Error running efficiency analysis', error);
    ...
    router.get('/operators/efficiency', [
        ...
        try {
            ...
            const { operator_id, date_from, date_to, operation_name } = req.query;
            ...
            const allRuns = await prisma.sewingRun.findMany({
                ...
                ,
                // Group by operator
                const: operatorStats = workspaceRuns.reduce((acc, run) => {
                ...
                Object, : .values(operatorStats).forEach((stats) => {
                    ...
                    Object.values(stats.operations).forEach((op) => {
                        op.avg_efficiency = op.actual_minutes > 0
                            ? (op.earned_minutes / op.actual_minutes) * 100
                            : 0;
                        // })
                        // Daily efficiency
                        Object.values(stats.daily_breakdown).forEach((day) => {
                            ...
                            const report = {
                                ...
                                summary: {
                                    ...
                                    avg_efficiency: Object.values(operatorStats).reduce((sum, stats) => sum + stats.overall_efficiency, 0) / Object.keys(operatorStats).length || 0
                                }
                            };
                        ...
                        try { }
                            ...
                            shared_1.logger.error('Error generating efficiency report', error);
                    ...
                    router.post('/operators/payroll', [
                        (0, auth_1.requirePermission)('hr.manage'), // Requires HR permissions
                        (0, express_validator_1.body)('period_start').isISO8601(),
                        (0, express_validator_1.body)('period_end').isISO8601(),
                        (0, express_validator_1.body)('operator_ids').optional().isArray(),
                        (0, express_validator_1.body)('include_base_salary').optional().isBoolean()
                    ], async (req, res) => {
                        try {
                            const errors = (0, express_validator_1.validationResult)(req);
                            if (!errors.isEmpty()) {
                            ...
                            const allRuns = await prisma.sewingRun.findMany({
                                ...
                                ,
                                // Group by operator for payroll calculation
                                const: operatorPayroll = workspaceRuns.reduce((acc, run) => {
                                    const opId = run.operator_id;
                                    if (!acc[opId]) {
                                        acc[opId] = {
                                            operator_id: opId,
                                    ...
                                    Object.values(operatorPayroll).forEach((payroll) => {
                                        payroll.gross_earnings = payroll.piece_work_earnings + payroll.base_salary_portion;
                                        payroll.avg_efficiency = payroll.runs_detail.reduce((sum, run) => sum + (run.efficiency_pct || 0), 0) / payroll.runs_detail.length || 0;
                                        ...
                                        const payrollReport = {
                                            ...
                                            summary: {
                                                ...
                                                total_piece_earnings: Object.values(operatorPayroll).reduce((sum, p) => sum + p.piece_work_earnings, 0),
                                                total_base_salary: Object.values(operatorPayroll).reduce((sum, p) => sum + p.base_salary_portion, 0),
                                                total_gross_earnings: Object.values(operatorPayroll).reduce((sum, p) => sum + p.gross_earnings, 0),
                                                total_pieces_produced: Object.values(operatorPayroll).reduce((sum, p) => sum + p.total_pieces, 0),
                                                avg_efficiency: Object.values(operatorPayroll).reduce((sum, p) => sum + p.avg_efficiency, 0) / Object.keys(operatorPayroll).length || 0
                                            }
                                        };
                                        // await logAudit({
                                        //   workspaceId: req.user!.workspace_id,
                                        ...
                                        // })
                                        shared_1.logger.info('Sewing payroll calculated', {
                                        ...
                                        try { }
                                            ...
                                            shared_1.logger.error('Error calculating payroll', error);
                                    ...
                                    router.get('/operators/:operatorId/performance', [
                                        (0, auth_1.requirePermission)('sewing.view'),
                                        (0, express_validator_1.param)('operatorId').isUUID(),
                                        ...
                                        try {
                                            ...
                                            const runs = await prisma.sewingRun.findMany({
                                                ...
                                                // })
                                                // Filter by workspace
                                                const: workspaceRuns = runs.filter(run => run.order.workspace_id === req.user.workspace_id),
                                                if(workspaceRuns) { }, : .length === 0
                                            }), { return: res };
                                        }
                                        finally { }
                                    }).status(404).json({ success: false, error: 'No performance data found for this operator' });
                                })
                                // Daily performance tracking
                                ,
                                // Daily performance tracking
                                const: dailyPerformance = workspaceRuns.reduce((acc, run) => {
                                    const day = run.ended_at?.toISOString().split('T')[0];
                                    if (!day)
                                        return acc;
                                    if (!acc[day]) {
                                ...
                                Object, : .values(dailyPerformance).forEach((day) => {
                                    ...
                                    const performance = {
                                        ...
                                        daily_performance: Object.values(dailyPerformance).sort((a, b) => a.date.localeCompare(b.date)),
                                        trends: {
                                            avg_efficiency: Object.values(dailyPerformance).reduce((sum, day) => sum + day.efficiency, 0) / Object.keys(dailyPerformance).length,
                                            avg_daily_pieces: Object.values(dailyPerformance).reduce((sum, day) => sum + day.pieces, 0) / Object.keys(dailyPerformance).length,
                                            avg_daily_earnings: Object.values(dailyPerformance).reduce((sum, day) => sum + day.earnings, 0) / Object.keys(dailyPerformance).length,
                                            quality_trend: Object.values(dailyPerformance).reduce((sum, day) => sum + day.quality_percentage, 0) / Object.keys(dailyPerformance).length
                                        },
                                        summary: {
                                            total_runs: workspaceRuns.length,
                                            total_pieces: workspaceRuns.reduce((sum, run) => sum + run.qty_good, 0),
                                            total_earnings: workspaceRuns.reduce((sum, run) => sum + (run.piece_rate_pay || 0), 0),
                                            best_day_efficiency: Math.max(...Object.values(dailyPerformance).map((day) => day.efficiency)),
                                            operations_performed: [...new Set(workspaceRuns.map(run => run.operation_name))]
                                        }
                                    };
                                    res.json({ success: true, data: performance });
                                }), catch(error) {
                                    shared_1.logger.error('Error fetching operator performance', error);

...
router.get('/summary', [
    ...
    try {
        ...
        const [activeOrders, completedOrders, totalBundles, pendingQC, routing_steps, employees] = await Promise.all([
            ...
            database_1.prisma.bundle.count({
                where: {
                    workspace_id: workspaceId,
                    created_at: { gte: startDate }
                }
            }),
            database_1.prisma.qcInspection.count({
                where: {
                    workspace_id: workspaceId,
                    pass_fail: false,
            ...
            database_1.prisma.routingStep.findMany({
                where: {
                    workspace_id: workspaceId,
                    status: 'in_progress'
                },
        ...
        const departmentActivity = routing_steps.reduce((acc, step) => {
        ...
        shared_1.logger.error('Production summary error:', error);
        res.status(500).json({
...
router.get('/bundles', [
    ...
    (0, express_validator_1.query)('status').optional().isIn(['pending', 'in_progress', 'completed', 'on_hold']),
    (0, express_validator_1.query)('current_stage').optional().isString()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const current_stage = req.query.current_stage;
        ...
        const [bundles, total] = await Promise.all([
            ...
            database_1.prisma.bundle.count({ where })
        ]);
        res.json({
            data: bundles,
            pagination: {
        ...
        shared_1.logger.error('Get bundles error:', error);
        res.status(500).json({
            error: 'Internal Server Error',
...
router.get('/bundles/:id', [
    ...
    try {
        ...
        shared_1.logger.error('Get bundle error:', error);
        res.status(500).json({
            error: 'Internal Server Error',
...
router.put('/bundles/:id', [
    ...
    (0, express_validator_1.param)('id').isUUID(),
    (0, express_validator_1.body)('status').optional().isIn(['pending', 'in_progress', 'completed', 'on_hold']),
    (0, express_validator_1.body)('current_stage').optional().isString(),
    ...
    try {
        ...
        const existingBundle = await database_1.prisma.bundle.findFirst({
            where: {
                id: req.params.id,
                workspace_id: req.user.workspace_id
            }
        ...
        shared_1.logger.info('Bundle updated', {
        ...
        shared_1.logger.error('Update bundle error:', error);
        res.status(500).json({
            error: 'Internal Server Error',
...
router.get('/routing', [
    (0, auth_1.requirePermission)('production:read'),
    (0, express_validator_1.query)('order_id').optional().isUUID(),
    (0, express_validator_1.query)('status').optional().isIn(['pending', 'in_progress', 'completed']),
    ...
    try {
        ...
        const routing_steps = await database_1.prisma.routingStep.findMany({
            where,
            include: {
                order: {
                    select: {
        ...
        shared_1.logger.error('Get routing steps error:', error);
        res.status(500).json({
            error: 'Internal Server Error',
...
router.put('/routing/:id', [
    ...
    (0, express_validator_1.param)('id').isUUID(),
    (0, express_validator_1.body)('status').optional().isIn(['pending', 'in_progress', 'completed', 'on_hold']),
    (0, express_validator_1.body)('assigned_to').optional().isString(),
    (0, express_validator_1.body)('actual_hours').optional().isDecimal(),
    (0, express_validator_1.body)('notes').optional().isString()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        ...
        const existingStep = await database_1.prisma.routingStep.findFirst({
            where: {
                id: req.params.id,
                workspace_id: req.user.workspace_id
            }
        ...
        const updatedStep = await database_1.prisma.routingStep.update({
            where: { id: req.params.id },
            data: updateData,
            include: {
                order: {
        ...
        shared_1.logger.info('Routing step updated', {
        ...
        shared_1.logger.error('Update routing step error:', error);
        res.status(500).json({
            error: 'Internal Server Error',

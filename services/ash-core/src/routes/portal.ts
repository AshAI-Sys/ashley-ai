...
router.post('/approval/:token/approve', [
  ...
  body('approver_name').notEmpty().trim(),
  ...
  try {
    ...
    logger.info('Design approved via portal', {
    ...
    logger.error('Design approval error:', error)
    res.status(500).json({
...
router.post('/approval/:token/changes', [
  ...
  try {
    ...
    logger.info('Design changes requested via portal', {
      approval_id: approval.id,
      asset_id: approval.asset_id,
      approver_email,
      workspace_id: approval.design_asset.workspace_id
    ...
    logger.error('Design changes request error:', error)
    res.status(500).json({
      error: 'Internal Server Error',
  ...
  try {
    ...
    res.json({
      ...
      pending_approvals: orders.reduce((count, order) => {
        return count + order.design_assets.filter(asset =>
          asset.approvals.some(approval => approval.status === 'SENT')
        ).length
      }, 0)
    })

...
describe('Finance API Integration Tests', () => {
  ...
  describe('GET /api/finance/invoices', () => {
    ...
    it('should support pagination for invoices', async () => {
      if (!authToken) {
        console.log('⏭️  Skipping: No auth token')
        return
      }

      const response = await fetch(`${API_BASE}/api/finance/invoices?page=1&limit=20`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json',
        },
    ...
    it('should filter invoices by status', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/finance/invoices?status=OPEN`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json',
        },
  ...
  describe('POST /api/finance/invoices', () => {
    ...
    it('should calculate invoice totals correctly', async () => {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        expect(invoice.subtotal || invoice.sub_total).toBe(1000.00)
      }
    }, TEST_TIMEOUT)
  ...
  describe('GET /api/finance/payments', () => {
    ...
    it('should filter payments by method', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/finance/payments?method=BANK_TRANSFER`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json',
        },
  ...
  describe('Performance', () => {
    it('should return finance stats within 2 seconds', async () => {
      if (!authToken) {
        console.log('⏭️  Skipping: No auth token')
        return
      }

...
describe('Printing Operations API Integration Tests', () => {
  ...
  describe('GET /api/printing/runs', () => {
    ...
    it('should filter runs by method', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/printing/runs?method=SILKSCREEN`, {
        headers: {
          'Authorization': `Bearer ${authToken}`
        }
      })
  ...
  describe('POST /api/printing/runs/[id]/start', () => {
    it('should start print run', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/printing/runs/test-run-123/start`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
  ...
  describe('POST /api/printing/runs/[id]/pause', () => {
    it('should pause print run', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/printing/runs/test-run-123/pause`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
  ...
  describe('POST /api/printing/runs/[id]/complete', () => {
    it('should complete print run', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/printing/runs/test-run-123/complete`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
  ...
  describe('GET /api/printing/machines', () => {
    ...
    it('should filter machines by status', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/printing/machines?status=OPERATIONAL`, {
        headers: {
          'Authorization': `Bearer ${authToken}`
        }
      })
  ...
  describe('GET /api/printing/ai/dashboard-insights', () => {
    it('should return AI dashboard insights', async () => {
      ...
      const response = await fetch(`${API_BASE}/api/printing/ai/dashboard-insights`, {
        headers: {
          'Authorization': `Bearer ${authToken}`
        }
      })

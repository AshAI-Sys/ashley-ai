...
describe("Real API Integration Tests", () => {
  ...
  describe("Authentication API", () => {
    ...
    it(
      "should accept login with valid credentials",
      async () => {
        if (!serverRunning) {
          console.log("   ⏭️  Skipping: Server not running");
          return;
    ...
    it(
      ...
      async () => {
        ...
        const response = await fetch(
          `${API_BASE}/api/nonexistent-endpoint-${Date.now()}`
        );
  ...
  describe("Performance", () => {
    it(
      "should respond to health check within 500ms",
      async () => {
        if (!serverRunning) {
          console.log("   ⏭️  Skipping: Server not running");
          return;
    ...
    it(
      "should respond to API endpoints within 2 seconds",
      async () => {
        if (!serverRunning || !authToken) {
          console.log(
            "   ⏭️  Skipping: Server not running or not authenticated"
          );
          return;
        }

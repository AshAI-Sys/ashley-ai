...
function verifyMagicBytes(
  ...
  const hex = buffer
    .toString("hex", 0, Math.min(buffer.length, 12))
    .toUpperCase();
  ...
  if (!expectedSignatures) {
    return {
      valid: true,
      warnings: ["No magic byte signature available for verification"],
    };
  }
...
function performSecurityChecks(fileInfo: FileInfo): ValidationResult {
  ...
  const scriptPatterns = [/<script/i, /javascript:/i, /onerror=/i, /onclick=/i];
  ...
  if (fileInfo.buffer) {
    const content = fileInfo.buffer.toString(
      "utf-8",
      0,
      Math.min(fileInfo.buffer.length, 1024)
    );
  ...
  describe("validateExtension() - Extension Validation", () => {
    ...
    it("should reject double extension attack", () => {
      ...
      expect(result.error).toMatch(
        /not allowed for security reasons|suspicious double extension/
      );
    });
  ...
  describe("verifyMagicBytes() - File Signature Verification", () => {
    ...
    it("should handle unknown MIME types gracefully", () => {
      ...
      expect(result.warnings).toContain(
        "No magic byte signature available for verification"
      );
    });
  });

...
class RateLimiter {
  ...
  isAllowed(request: NextRequest | MockRequest): { allowed: boolean; resetTime?: number; remaining?: number } {
...
function createMockRequest(ip: string = '127.0.0.1', userAgent: string = 'test-agent'): MockRequest {
  return {
    headers: {
      get(name: string): string | null {
...
describe('Rate Limiter Utility', () => {
  ...
  describe('Performance', () => {
    it('should handle many unique clients efficiently', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        const request = createMockRequest(`10.0.${Math.floor(i / 256)}.${i % 256}`)
        limiter.isAllowed(request as any)
      }

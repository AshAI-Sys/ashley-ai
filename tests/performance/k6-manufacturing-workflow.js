...
export default function () {
  ...
  group('Stage 1: Order Management', () => {
    ...
    if (!check(res, { 'orders loaded': (r) => r.status === 200 || r.status === 401 })) {
  ...
  group('Stage 2: Cutting', () => {
    ...
    // Scan bundle
    res = http.post(`${BASE_URL}/api/cutting/bundles/scan`, JSON.stringify({
      qr_code: 'BUNDLE-TEST-123',
      station: 'CUTTING'
    }), { headers });
    check(res, { 'bundle scanned': (r) => [200, 201, 400, 401, 404].includes(r.status) });
  ...
  group('Stage 3: Printing', () => {
    ...
    check(res, { 'machines loaded': (r) => r.status === 200 || r.status === 401 });

    // Start print run
    res = http.post(`${BASE_URL}/api/printing/runs/test-run-123/start`, JSON.stringify({
      started_by: 'operator-1',
      machine_id: 'machine-1'
    }), { headers });
    check(res, { 'run started': (r) => [200, 201, 400, 401, 404].includes(r.status) });
  ...
  group('Stage 4: QC', () => {
    ...
    const res = http.post(`${BASE_URL}/api/mobile/qc/submit`, JSON.stringify({
    ...
    check(res, { 'qc submitted': (r) => [200, 201, 400, 401, 404].includes(r.status) });
  ...
  group('Stage 6: Finance', () => {
    ...
    check(res, { 'invoices loaded': (r) => r.status === 200 || r.status === 401 });

...
function login() {
  ...
  const success = check(res, {
    ...
    'login has token': (r) => r.json('token') !== undefined || r.json('accessToken') !== undefined,
  });
...
export function smokeTest() {
  ...
  endpoints.forEach((endpoint) => {
    ...
    const success = check(res, {
      [`${endpoint} status is 200 or 401`]: (r) => [200, 401, 404].includes(r.status),
    });
...
export function loadTest() {
  ...
  check(res, { 'orders list loaded': (r) => r.status === 200 || r.status === 401 });

  sleep(1);

  // 2. Check cutting operations
  res = http.get(`${BASE_URL}/api/cutting/lays`, { headers });
  check(res, { 'cutting lays loaded': (r) => r.status === 200 || r.status === 401 });

  sleep(0.5);

  // 3. Check printing operations
  res = http.get(`${BASE_URL}/api/printing/runs`, { headers });
  check(res, { 'printing runs loaded': (r) => r.status === 200 || r.status === 401 });
...
export function stressTest() {
  ...
  check(res, {
    'stress test response': (r) => r.status === 200 || r.status === 401 || r.status === 404,
  });
...
export function spikeTest() {
  ...
  responses.forEach((res, i) => {
    check(res, {
      [`batch request ${i} success`]: (r) => r.status === 200 || r.status === 401 || r.status === 404,
    });
  });

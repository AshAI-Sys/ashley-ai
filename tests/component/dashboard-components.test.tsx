    ...
    <div data-testid="real-time-metrics">
      ...
      {metrics && (
        <div>
          ...
          <div data-testid="active-production">{metrics.activeProduction || 0}</div>
    ...
    <div data-testid="role-widgets">
      ...
      <div data-testid="widgets-list">
        {widgets[role]?.map((widget: string) => (
          <div key={widget} data-testid={`widget-${widget.toLowerCase().replace(' ', '-')}`}>
            {widget}
          </div>
        ))}
      </div>
...
const MockCustomizableDashboard = ({ layout, onLayoutChange }: any) => {
  ...
  const handleAddWidget = () => {
    const newWidget = { id: `widget-${currentLayout.length + 1}`, title: 'New Widget' }
...
describe('Dashboard Components', () => {
  describe('RealTimeMetrics Component', () => {
    ...
    it('should update when metrics change', () => {
      const { rerender } = render(<MockRealTimeMetrics metrics={{ totalOrders: 100 }} />)
  ...
  describe('RoleActivities Component', () => {
    ...
    it('should display activities list', () => {
      ...
      expect(screen.getByTestId('activity-0')).toHaveTextContent('Order #123 created')
      expect(screen.getByTestId('activity-1')).toHaveTextContent('Production started')
      expect(screen.getByTestId('activity-2')).toHaveTextContent('QC inspection passed')
    })
  ...
  describe('CustomizableDashboard Component', () => {
    it('should render with empty layout', () => {
      render(<MockCustomizableDashboard layout={[]} onLayoutChange={jest.fn()} />)
    ...
    it('should add widget when button clicked', () => {
      ...
      render(<MockCustomizableDashboard layout={[]} onLayoutChange={handleLayoutChange} />)
    ...
    it('should remove widget when remove button clicked', () => {
      ...
      render(<MockCustomizableDashboard layout={initialLayout} onLayoutChange={handleLayoutChange} />)
    ...
    it('should render initial layout', () => {
      ...
      render(<MockCustomizableDashboard layout={layout} onLayoutChange={jest.fn()} />)

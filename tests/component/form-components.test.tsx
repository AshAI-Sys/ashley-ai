...
describe("Form Components", () => {
  describe("OrderForm Component", () => {
    ...
    it("should show validation errors on submit with empty fields", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("client-name-error")).toHaveTextContent(
          "Client name is required"
        );
        expect(screen.getByTestId("quantity-error")).toHaveTextContent(
          "Quantity must be greater than 0"
        );
        expect(screen.getByTestId("due-date-error")).toHaveTextContent(
    ...
    it("should update input values on change", () => {
      ...
      const clientInput = screen.getByTestId(
        "client-name-input"
      ) as HTMLInputElement;
    ...
    it("should pre-fill form with initial data", () => {
      ...
      expect(
        (screen.getByTestId("client-name-input") as HTMLInputElement).value
      ).toBe("Existing Client");
      expect(
        (screen.getByTestId("quantity-input") as HTMLInputElement).value
      ).toBe("50");
      expect(
        (screen.getByTestId("due-date-input") as HTMLInputElement).value
      ).toBe("2025-11-15");
    });

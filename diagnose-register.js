...
async function diagnose() {
  ...
  try {
    ...
    console.log(
      `   ✅ Database connected (${userCount} users, ${workspaceCount} workspaces)\n`
    );
    ...
    if (allUsers.length === 0) {
      ...
      allUsers.forEach(user => {
        console.log(
          `   - ${user.email} (${user.first_name} ${user.last_name}) - Role: ${user.role}`
        );
      });
      console.log("");
    }
    ...
    if (allWorkspaces.length === 0) {
      ...
      allWorkspaces.forEach(ws => {
        console.log(
          `   - ${ws.name} (slug: ${ws.slug}) - Active: ${ws.is_active}`
        );
      });
      console.log("");
    }
    ...
    const {
      validatePassword,
    } = require("./services/ash-admin/src/lib/password-validator.ts");
